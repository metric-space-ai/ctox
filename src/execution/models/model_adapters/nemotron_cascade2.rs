...
pub fn rewrite_success_response(
    ...
    let mut builder = engine::responses_turn_builder(
        &payload,
        fallback_model,
        "nvidia/Nemotron-Cascade-2-30B-A3B",
    );
    ...
    if let Some(choices) = payload.get("choices").and_then(Value::as_array) {
        for choice in choices {
            ...
            if let Some(text) = message
                ...
                .filter(|text| !text.is_empty())
            {
                let (inline_reasoning, visible_text) = split_reasoning_and_content(&text);
                ...
                let (plain_text, xml_tool_calls) = parse_xml_tool_calls(&visible_text);
...
mod tests {
    ...
    #[test]
    fn nemotron_exact_text_override_fills_missing_message_text() {
        let payload = json!({
            "id": "chatcmpl_test",
            "choices": [{
                "message": {
        ...
        assert_eq!(rewritten["output_text"], "CTOX_NEMOTRON_OK");
    }
}
