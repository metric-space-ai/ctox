...
pub fn rewrite_request(raw: &[u8]) -> anyhow::Result<Vec<u8>> {
    ...
    let model = payload
        ...
        .unwrap_or("mistralai/mistral-small-2603")
        .to_string();
    ...
    // Mistral M2.7 has mandatory reasoning — always enable thinking.
    ...
    serde_json::to_vec(&Value::Object(request))
        .context("failed to encode Mistral chat-completions payload")
}
...
pub fn rewrite_success_response(
    ...
    let mut builder =
        engine::responses_turn_builder(&payload, fallback_model, "mistralai/mistral-small-2603");
    if let Some(choices) = payload.get("choices").and_then(Value::as_array) {
        for choice in choices {
            ...
            // Mistral uses reasoning_content for thinking output
    ...
    serde_json::to_vec(&response_payload).context("failed to encode Mistral responses payload")
}
