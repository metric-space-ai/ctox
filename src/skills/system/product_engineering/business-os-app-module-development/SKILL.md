...
Top acceptance gate for generated apps: the final module must have requested-domain UI, helper logic, automation payload, and tests. Generic scaffold strings such as `New record`, `Search records`, `Select a record`, `Title`, `Owner`, `Due date`, `Open`, `Blocked`, `Done`, `record.title`, `due_at_ms`, `summary.total`, `summary.open`, and `summary.blocked` are allowed only as temporary scaffold internals before the first app-file write. They must not remain in final `index.html`, `index.js`, locales, or tests unless the requested business domain literally uses those exact words.

The first implementation must be a vertical requested-domain slice, not a helper phase. Rewrite `index.html`, `index.js`, `core/records.mjs`, `core/automation.mjs`, one locale file, and one `tests/*.test.mjs` together before validation, tests, syntax checks, or any completion claim. The first requested-domain app-artifact write must include `index.js` and a `tests/*.test.mjs` rewrite, either in the same command as the helper/UI files or as the immediately completing paired write before any validation or cosmetic/contract edit. Do not edit `module.json`, `collections.schema.json`, `schema.js`, `index.css`, or `icon.svg` before `index.js` and tests have been rewritten for the requested domain; those scaffold files are already structurally valid, and early schema/CSS/icon/manifest edits are trace failures. If a tool-call limit forces separate writes, the immediately next app-artifact write must complete missing UI/JS/test/helper parity; do not keep polishing helpers, add schema, edit CSS/icon, or validate while `index.js` or tests still contain scaffold fields. If `core/records.mjs` changes from generic records to requested-domain records, rewrite `index.html`, `index.js`, one locale file, `core/automation.mjs`, and `tests/*.test.mjs` in the same implementation pass so visible fields, filters, summaries, fixture facts, and automation copy all match. A helper-only rewrite is a failed app.

Do not add new first-pass `data-action` values such as `restock`, `reorder`, `renew`, `attention`, `bulk`, `export`, `ai`, or status-only actions. Use only the scaffold action surface: form submit plus `data-action="new"`, `data-action="delete"`, and `data-action="follow-up"`. Put domain-specific work into the follow-up command payload or normal form fields. Tests must not keep stale scaffold `assert.deepEqual` expectations for `total/open/blocked/done` after helpers return domain aggregates; assert named requested-domain fields deliberately.
...
writes. If you read this skill inside CTOX, run exactly `ctox skills system
show business-os-app-module-development --body` once with no pipe, no
redirection, no `head`, no `tail`, and no `sed`. After that optional complete
skill-body read, the first app-artifact tool action must set `MODULE_DIR` and
write bounded requested-domain content directly to files under `$MODULE_DIR/`,
including `core/records.mjs`, `core/automation.mjs`, `index.html`, `index.js`,
one locale file, and one `tests/*.test.mjs` file. Keep `module.json`,
`collections.schema.json`, `schema.js`, `index.css`, and `icon.svg` untouched
until that entry/test vertical slice exists and is domain-specific.
...
2. Treat the existing scaffold as already structurally valid. The required file inventory is the fixed scaffold contract: `module.json`, `collections.schema.json`, `schema.js`, `index.html`, `index.css`, `index.js`, `icon.svg`, `core/automation.mjs`, `core/records.mjs`, `locales/de.json`, `locales/en.json`, and `tests/*.test.mjs`. This list is not permission to run `ls`, `find`, `tree`, `stat`, or file-by-file readback before requested-domain edits. Inspect only the exact export, selector, or failing snippet needed after a concrete validator/test/syntax failure names it. Preserve the technical contracts as the baseline: mount wiring, helper export names, commandBus dispatch facade, locales, and test harness. Do not preserve generic visible labels, generic field names, generic summaries, or generic fixture expectations as the final app. Validating the untouched scaffold before requested-domain edits is a task failure, not progress. Do not run `ctox business-os app repair-missing` or `ctox business-os app scaffold ... --repair-missing` from an App Creator worker turn. If validation names a missing required file, restore that exact file directly at `$MODULE_DIR/<file>` and reconnect it to the current helpers, UI, locales, and tests before validating again. Do not touch `module.json`, `collections.schema.json`, `schema.js`, `index.css`, or `icon.svg` as the first implementation move; first make `index.js`, tests, helpers, UI, and one locale domain-specific.
3. Inspect exactly three shipped source modules for patterns, preferably `customers`, `shiftflow`, and `outbound`, unless a CTOX App Creator prompt has already embedded those three few-shot patterns. Use exact files that exist in normal CTOX installs, such as `src/apps/business-os/modules/customers/module.json`, `src/apps/business-os/modules/customers/index.js`, `src/apps/business-os/modules/shiftflow/index.js`, `src/apps/business-os/modules/outbound/index.js`, and `src/apps/business-os/modules/outbound/core/audience.js`; do not run `ls`, `find`, `rg`, `grep`, `wc -l`, or line-count sweeps over `src/apps/business-os/modules/`, any source-module directory, or the selected few-shot files. Use `notes` only as a supplemental simple-CRUD reference; do not treat it as a scaffold-helper template. Do not inspect `src/apps/business-os/app.js`, `shared/`, `router/loader` source, `src/core/business_os/`, `src/skills`, or validator/checker source while building a generated app. Inside a CTOX App Creator turn, a full `ctox skills system show business-os-app-module-development --body` read is acceptable at most once only when it is the exact unpiped command; partial reads through `head`, `tail`, or `sed`, redirection to scratch files, and source-skill inspection are failures.
4. Preserve the scaffold helper API exactly, then change its implementation. `core/records.mjs` must still export `MODULE_ID`, `COLLECTION_NAME`, `createRecord`, `normalizeStatus`, `visibleRecords`, and `summarizeRecords`; add requested-domain helpers such as `needsAttention` only after these exports remain present. `core/automation.mjs` must still export `buildFollowUpCommand(record = {})`. For first-pass runtime apps, treat `buildFollowUpCommand(record = {})` as a stable compatibility facade: change its implementation to build the requested-domain `business_os.chat.task`, but keep the export present, keep both `type` and `command_type`, and keep `payload.record_snapshot`. Do not replace it with only plan helpers, move the command builder into `core/records.mjs`, or import domain helpers from `core/records.mjs` before exporting them there.
5. For first-pass App Creator apps, preserve the scaffold's single primary module-owned collection and helper API by default. Translate the requested domain into fields, statuses, calculations, fixture records, labels, filters, and automation facts on that collection. The scaffold collection key is `<snake_case_module_id>_records`; for example, `bench_subscriptions_r104_20260619t175320z` owns `bench_subscriptions_r104_20260619t175320z_records`. Keep that exact collection key in `module.json.collections`, `collections.schema.json.collections`, `schema.js`, `core/records.mjs` as `COLLECTION_NAME`, `index.js`, and tests. Do not rename the scaffold collection to domain nouns such as `subscriptions`, `items`, `locations`, `batches`, `projects`, `contracts`, `findings`, `notes`, or `pick_lines`. `schema.js` must not export `collections = { subscriptions: ... }` for a subscriptions app; it must export the scaffold collection key. Do not invent extra collections unless the user explicitly needs separate persisted objects and you update `module.json`, `collections.schema.json`, `schema.js`, `core/records.mjs`, `core/automation.mjs`, `index.js`, locales, and every `tests/*.test.mjs` in the same edit before validation. Runtime helpers must not define constants for undeclared collections. Runtime files must not reference `business_commands`; the only automation write path is `ctx.commandBus.dispatch(...)`.
6. Use direct bounded exact-path file edits only. Shell heredocs or redirects are acceptable only when they write the final artifact directly to `$MODULE_DIR/<file>` or the exact prompted module path. Do not write app artifacts to `/tmp`, split generated module fragments under `/tmp`, test them from `/tmp`, copy/move them from `/tmp`, stage `/tmp/*.patch` files, `cd /tmp`/`cd "$TMPDIR"` for Node import/stat/readFileSync probes, or treat scratch output as implementation work. If a shell write fails because of quoting, history expansion, command length, or escaping, do not switch to Python, base64, Node writer scripts, generated writer scripts, `/tmp` scratch files copied into the module, shell-level `apply_patch` discovery/invocation, module scratch/probe files, `cat >>` append chunks against app artifacts, huge `printf`/`echo`/`tee`/`cat` payload rewrites, `sed -i`/`perl -pi` line surgery, or data-URL workarounds. Shorten the file, keep simple DOM wiring in `index.js`, put pure record logic in `core/records.mjs`, put command payload logic in `core/automation.mjs`, and rewrite the smaller affected file directly at its final module path.
7. Required artifacts are canonical files, not shell patterns. Write `module.json`, `collections.schema.json`, `schema.js`, `index.html`, `index.css`, `index.js`, `icon.svg`, `locales/en.json`, `locales/de.json`, `core/records.mjs`, `core/automation.mjs`, and tests by exact path only. For first-pass runtime-installed App Creator apps, do not create extra `core/*.mjs` layers such as `ui.mjs`, `render.mjs`, `runtime.mjs`, or `panel.mjs`; those are refactor work, not one-shot app creation. For runtime-installed App Creator apps, the module root may contain only those root files plus `core/`, `locales/`, and `tests/`. Do not create or leave temporary schema/manifest aliases, typo files such as `m.json`, `m`, or `modul.json`, scratch notes, copied app roots, glob artifacts, or ad hoc helper directories in the module root. For runtime-installed manifests, preserve `title`, `entry="installed-modules/<id>/index.html"`, `install_scope="installed"`, SemVer `version` without a `v` prefix, `collections=["business_commands", "<snake_case_module_id>_records"]`, `layout.shell="full-workspace"`, `store.distribution="ctox-runtime-installed-module"`, and `store.installable=false`; never replace the manifest with a generic web-app manifest using `name`, `permissions`, `collection_ownership`, `layout.shell="single"`, source/store fields such as `entry="index.html"`, `entry="modules/.../index.html"`, `install_scope="store"`, `source="local"`, `store.source_path="modules/..."`, `store.distribution="ctox-repo-module"`, `store.installable=true`, `layout.icon_svg`, or any `layout.right`/right-resizer metadata.
...
8. `schema.js` is invariant. Do not rename it, delete it, replace it with `schema.mjs`, or leave a root-level `schema.mjs`/`schema.cjs` alias next to it. Do not rewrite `schema.js` as an isolated first domain step. If domain fields change, keep the existing scaffold collection key and update `schema.js`, `collections.schema.json`, `core/records.mjs`, UI, and tests in one lockstep pass. Put reusable schema fragments in `core/*.mjs` and re-export them from `schema.js`. `collections.schema.json` is a native-readable JSON schema contract, not an ESM/JSON-module wrapper: it must be `{ "schema_format": "ctox-business-os-module-collections-v1", "collections": { "<snake_case_module_id>_records": { "version": 0, "primaryKey": "id", "type": "object", "properties": ..., "required": ... } } }`. Do not use `format`, `schemaFormat`, `primary_key`, or nested `json_schema`.
9. Tests must prove useful behavior, not only syntax. At minimum they must import both `../core/records.mjs` and `../core/automation.mjs`, cover record visibility/summary logic, and assert a `business_os.chat.task` command payload whose title/instruction/prompt/record_snapshot contain actual facts from the fixture records. Keep fixture expectations hand-computed and synchronized with the helper's exported shape. If `summarizeRecords()` or another helper returns extra fields, either assert the full updated shape or assert individual fields intentionally; do not leave a stale partial `deepEqual` that fails only because the helper grew. For generated App Creator tests, do not write `assert.deepEqual(summarizeRecords(...), {...})` directly; assign the helper result to a variable and assert named fields deliberately, unless the expected object includes every returned key. Do not write placeholder tests, tautologies such as `assert.equal(1, 1)` or `assert.equal(1 + 1, 2)`, automation-only type checks, or tests that pass without importing local app helpers. Keep fixture status values aligned with `normalizeStatus()` and other normalizers. Do not write negative anti-pattern tests or assertion messages that contain forbidden layout/data/dependency literals; validators own those checks. All tests in `tests/*.test.mjs` must pass; adding a new green test while an old scaffold test imports a missing export is still red.
9a. Keep visible UI and helper output in lockstep. If `index.js` reads aggregate fields such as `summary.total`, `summary.open`, `summary.blocked`, `summary.mrr`, or `summary.renewals_30d`, `core/records.mjs` must visibly return those exact fields from `summarizeRecords()` and tests must assert them from concrete fixture records. Do not leave the generic Title/Status/Open/Blocked/Done/Search-records scaffold in `index.html` or matching `record.title`, `due_at_ms`, and `summary.open` branches in `index.js` after changing helpers to a requested domain. Before the first validation call, `index.html`, `index.js`, `core/records.mjs`, `core/automation.mjs`, one locale file, and one test must all contain concrete nouns from the user's prompt, not only the module id or app title.
10. Do not run validation, tests, `node --check`, scaffold repair, directory listings, or generated scaffold readback as the first action on a complete fresh scaffold or as the first action in a validation-rework turn that says the validator is green but no complete required domain artifact set was written after the scaffold baseline. After an optional single complete unpiped `ctox skills system show business-os-app-module-development --body` read, the first app-artifact tool action must be a direct bounded requested-domain write under `MODULE_DIR`. First make the smallest vertical requested-domain slice across `core/records.mjs`, `core/automation.mjs`, `index.html`, `index.js`, one locale file, and one `tests/*.test.mjs` file so they contain concrete requested-domain fixture facts before validation, tests, syntax checks, or completion. The first requested-domain write must include `index.js` and tests, or the immediately next app-artifact write must add them before any validation, schema, manifest, CSS, icon, or polish action. Do not edit `module.json`, `collections.schema.json`, `schema.js`, `index.css`, or `icon.svg` as an isolated first domain step; those files are scaffold contract surfaces and should stay frozen until the requested-domain entry/test slice exists. Keep helper exports, schema fields, collection names, status values, command payload fields, fixture records, labels, filters, UI selectors, and HTML `data-action` values in lockstep with every importer, every concrete `index.js` handler/branch/action-map entry, and every test before running validation; partial helper rewrites are not a valid checkpoint. First-pass runtime apps must keep the scaffold's existing action surface: form submit plus `data-action="new"`, `data-action="delete"`, and `data-action="follow-up"`. Prefer changing labels, filters, selected-record facts, and the `buildFollowUpCommand()` payload over adding visible action buttons. Do not add `data-action="restock"`, `data-action="reorder"`, `data-action="renew"`, `data-action="attention"`, bulk, export, AI, or status-only buttons even when you could add a handler; remove extra buttons instead of leaving or implementing decorative controls. If `index.html` changes after the scaffold baseline, `index.js` and tests must also change after the baseline so selectors, form fields, render output, summary fields, and actions match the new fragment. Then run `ctox business-os app validate <id> --installed` before claiming success and repair every bullet exactly. Validation completion is accepted only when `core/records.mjs`, `core/automation.mjs`, `index.html`, `index.js`, one locale file, and one `tests/*.test.mjs` file all changed after the scaffold baseline. A locale-only label change, icon change, helper-only rewrite, HTML-only rewrite, or green helper test does not count as visible domain UI and must not be treated as a completed app while `index.html`, `index.js`, or tests remain scaffold-generic. If validation reports early, partial validation, "no complete required domain app artifact set was written after CTOX recorded the scaffold baseline", "no required app artifact was written after CTOX recorded the scaffold baseline", stale `summary.total/open/blocked` reads, `data-*` selector drift, unallowed `data-action`, "first requested-domain app-artifact write did not rewrite index.js and tests", "manifest, schema, CSS, or icon artifact was edited before index.js and tests were rewritten", or "generic App Creator records scaffold", immediately edit the runtime app files for the requested domain as one lockstep repair and validate again. Do not answer such a rework prompt by running the validator as the first or only command.
   If validation was green earlier and later turns it red, trust the later red result. Do not continue domain tests while any required root artifact is missing. The repair order is: restore the exact missing required file named by the validator, repair imports/handlers/selectors, rerun tests, rerun `ctox business-os app validate <id> --installed`.
11. Once `ctox business-os app validate ...` is green, stop. Do not backfill missed few-shot inspection, read checker internals, search prior bench apps, run repository-wide conformance scripts, source-wide RxDB scans, broad file dumps, cosmetic rewrites, helper refactors, or extra polishing passes. A green App Creator validator plus the required focused tests is the completion boundary. CTOX may still rework the task if the post-scaffold tool trace shows validation before direct module edits, partial skill reads, `src/skills` inspection, `/tmp` artifact staging/testing/copying, generated scaffold readback before implementation, source-module discovery/line-count sweeps, nonexistent few-shot paths, or broad source/generated-file dumps; the repair is to write bounded final files directly under `MODULE_DIR`, then run the app-specific validator once.
...
you are inside a CTOX App Creator run and are about to partially read `ctox skills system show business-os-app-module-development --body` with `head`, `tail`, `sed`, redirection, or source-file discovery, or inspect `src/skills`; only the exact unpiped `ctox skills system show business-os-app-module-development --body` command is acceptable, partial/source reads are not
...
you are about to invent a few-shot file path that does not exist in a normal CTOX install, especially core/automation.mjs under outbound or shiftflow; use real shipped paths like `outbound/index.js` or `outbound/core/audience.js`
...
you are about to read the generated App Creator scaffold back file-by-file, list it with `ls`/`find`/`tree`/`stat`, or inspect it in consecutive line-range chunks before requested-domain edits; the required file inventory is supplied by the scaffold contract, and later readback is limited to snippets needed for a concrete import, selector, syntax, test, or validator failure
you are about to run `ctox business-os app repair-missing` or `ctox business-os app scaffold ... --repair-missing` from an App Creator worker turn; restore missing required files directly at exact `$MODULE_DIR/<file>` paths instead
...
you are about to use `/tmp` or another scratch directory to write, test, stage, copy, move, patch, import, stat, or read generated app artifacts; write and check bounded payloads directly at `$MODULE_DIR/<file>` or the exact prompted module path instead
your first requested-domain app-artifact write does not include both `index.js` and one `tests/*.test.mjs`, and your immediately next app-artifact action is anything other than the missing entry/test rewrite
you are about to edit `module.json`, `collections.schema.json`, `schema.js`, `index.css`, or `icon.svg` before `index.js` and tests have been rewritten for the requested domain
...
you are building a new runtime-installed App Creator app and neither the CTOX service preflight nor your first explicit action has created a complete structural scaffold under the target directory
...
you are about to clean, reset, delete, or rewrite a structural scaffold wholesale because it looks generic; keep it as the baseline and edit the smallest domain-specific parts
you are about to treat an untouched structural scaffold as a completed requested app; a scaffold is only the baseline, and at least the requested-domain record model, labels, visible workflow, automation payload, and tests must be customized before final validation can complete the task
you are about to count locale-only label changes, icon updates, helper-only rewrites, or green helper tests as visible domain UI while index.html or index.js still show the generic records scaffold
you are about to write placeholder or tautological tests such as `test("placeholder")`, `assert.equal(1, 1)`, `assert.equal(1 + 1, 2)`, `assert.ok(true)`, or tests that only check `type`/`command_type`; tests must import both `../core/records.mjs` and `../core/automation.mjs`, exercise record helpers, and assert concrete fixture facts in the automation payload
you changed `summarizeRecords()` or domain helper output but left `index.js` reading stale aggregate fields such as `summary.total`, `summary.open`, or `summary.blocked` that the helper no longer returns; update UI aggregate labels and helper output in lockstep
...
schema.js for a first-pass runtime-installed App Creator app does not contain the exact `<snake_case_module_id>_records` collection key, or it renamed the scaffold collection to a domain noun such as subscriptions/items/projects/contracts/findings
core/records.mjs no longer exports `COLLECTION_NAME`, `createRecord`, `normalizeStatus`, `visibleRecords`, or `summarizeRecords`, while index.js or tests still import scaffold helpers
...
you are about to use Python, base64 blobs, Node writer scripts, data URLs, generated writer scripts, `/tmp` scratch files copied into the module, `/tmp/*.patch` files, `/tmp`/`$TMPDIR` as a probe working directory, shell `apply_patch` wrappers, or temporary generated file-copy wrappers to create, test, probe, or repair app files; if a file needs that, split/refactor it into smaller ESM helpers and direct bounded exact-path edits
...
you are about to rewrite core/records.mjs from scratch without preserving the scaffold exports `COLLECTION_NAME`, `createRecord`, `normalizeStatus`, `visibleRecords`, and `summarizeRecords`
...
5. Resolve the target directory and carry the required-file contract before any
   optional UI or polish. Do not create an inventory document and do not list
   the scaffold to prove this contract before requested-domain edits:
...
If validation explicitly names missing scaffold files after a partial rewrite,
restore each named required file directly at the exact `$MODULE_DIR/<file>`
path before changing domain logic. Do not run `ctox business-os app
repair-missing` and do not run `ctox business-os app scaffold ... --repair-missing`
from an App Creator worker turn. The service preflight owns scaffold creation;
the worker owns direct, bounded file repair plus helper/UI/test reconciliation.
...
slice immediately. In CTOX App Creator runs, the service preflight has already
created the scaffold, so the first app-artifact action must modify
requested-domain files in place. In external empty-target runs, the first write
action creates `MODULE_DIR` and the exact required files by direct final path.
Do not list or read back the scaffold first. Do not open or inspect
...
After the initial domain file pass, let `ctox business-os app validate <id>
--installed` prove the required file inventory before adding optional features.
If validation reports a missing required file, stop optional work and create
only the named missing file by exact final path.
...
- Do not use Python, base64, Node one-off writer scripts, data URLs, `/tmp` scratch files copied into the module, `/tmp/*.patch` files, `/tmp`/`$TMPDIR` as the current working directory for generated-module probes, shell `apply_patch` wrappers, temporary generated file-copy wrappers, or `sed -i`/`perl -pi` line surgery to recover from malformed shell writes. When a direct write is fragile, reduce the app scope, split the file into a smaller local `.mjs` helper, or rewrite the affected bounded helper/file directly with simpler literals.
