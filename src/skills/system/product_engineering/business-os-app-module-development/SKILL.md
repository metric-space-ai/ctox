...
The first implementation must be a vertical requested-domain slice, not a helper phase. Rewrite `index.html`, `index.js`, `core/records.mjs`, `core/automation.mjs`, one locale file, and one `tests/*.test.mjs` together before validation, tests, syntax checks, or any completion claim. If a tool-call limit forces separate writes, the immediately next app-artifact write must complete missing UI/JS/test/helper parity; do not keep polishing helpers, add schema, or validate while `index.js` or tests still contain scaffold fields. If `core/records.mjs` changes from generic records to requested-domain records, rewrite `index.html`, `index.js`, one locale file, `core/automation.mjs`, and `tests/*.test.mjs` in the same implementation pass so visible fields, filters, summaries, fixture facts, and automation copy all match. A helper-only rewrite is a failed app.
...
one locale file, and one `tests/*.test.mjs` file.
