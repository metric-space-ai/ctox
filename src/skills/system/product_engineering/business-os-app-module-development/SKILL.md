...
you are inside a CTOX App Creator run and are about to partially read `ctox skills system show business-os-app-module-development --body` with `head`, `tail`, `sed`, or source-file discovery, or inspect `src/skills`; a complete one-shot skill body read is acceptable, partial/source reads are not
