...
you are inside a CTOX App Creator run and are about to call `ctox skills system show business-os-app-module-development`, inspect `src/skills`, or partially read the skill with `head`, `tail`, `sed`, or source-file discovery; the App Creator rules are already embedded in the prompt
...
you are about to open a few-shot path that does not exist in a normal CTOX install, such as `src/apps/business-os/modules/outbound/core/automation.mjs`; use real shipped paths like `outbound/index.js` or `outbound/core/audience.js`
