...
you are about to run `ctox business-os app scaffold ... --repair-missing` before a validator report explicitly names a missing required file, on a complete scaffold, or as a way to discover whether files are missing, or count scaffold repair output as implementation work
...
repair non-destructively before changing domain logic:

```sh
ctox business-os app scaffold <id> --installed --repair-missing
```

This command only creates missing scaffold files. It must not be used to check
whether files are missing, and it must not be used as a reason to delete
customized app files. After repair, reconcile the existing domain files with
the restored core helpers, locales, and tests.
