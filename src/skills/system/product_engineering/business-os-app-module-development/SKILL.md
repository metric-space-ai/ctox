...
2. Inspect the existing scaffold first, but do not dump every scaffold file into context. Verify the file inventory, then inspect only the exact exports, selectors, or failing snippets needed for the next edit. Keep its mount wiring, `core/automation.mjs`, `core/records.mjs`, locales, and tests as the baseline.
...
you are about to rewrite generated app files with giant `printf`, `echo`, `tee`, or `cat` shell payloads; split domain logic into small `core/*.mjs` helpers and rewrite only the bounded helper/file that changed
...
`co*ions.*json` or `colle{ctions,ctions}.schema.json`. If a required file
...
- Avoid huge single tool calls. If a file grows large enough to risk malformed tool-call JSON, reduce scope first; do not stream it as a giant shell `printf`, `echo`, `tee`, or `cat` payload. Split domain logic into small `core/*.mjs` helpers and immediately run syntax checks.
...
- Do not dump the whole generated module back into context, including by consecutive `sed -n` ranges that reconstruct a full file. Use `sed -n` only for small line ranges tied to an exact selector/import/error, `rg -n` for exact selectors/imports, and the validator report for repair targets. Full-file `cat` loops, file-by-file scaffold rereads, and broad source scans are App Creator failure patterns.
- Do not keep working after the app-specific validator is green. The final action after a green validator is the final response; extra repo-wide checks or cosmetic rewrites are scope creep and can turn a good app into a failed bench run.
