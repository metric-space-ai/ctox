...
- Keep helper exports and test imports in lockstep. Every named import in every test must exist as a real `export` in the helper file. When replacing scaffold helpers with domain-specific helpers, replace scaffold tests in the same turn and run the exact test file before the validator.
...
phase 12 tests: tests import only local `.mjs` helpers and JSON/text files; every named import is exported by that helper; they do not import `index.js`/`schema.js` directly, do not use data: URLs, and cover schema parity, core command builders, and at least one CRUD/automation path; fixture totals/counts are hand-computed in comments or small named facts and are consistent with the implementation; scaffold core/locales/tests still exist
