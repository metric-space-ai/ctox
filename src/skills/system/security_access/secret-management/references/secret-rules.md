...
- Persist secret material only in the encrypted CTOX SQLite secret store.
- Persist only the secret handle (`scope/name`) and classification in normal operator-visible state.
