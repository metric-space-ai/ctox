...
Monitoring data is not ticket prose. Ingest it into the CTOX knowledge plane so later ticket work can load it through the normal knowledge path.

Monitoring snapshots alone do not mean the full ticket+knowledge pipeline is healthy. They are one CTOX knowledge domain, not proof that mirrored tickets, source bindings, runbooks, or desk skills exist.
