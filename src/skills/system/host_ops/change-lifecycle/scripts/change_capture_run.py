...
COLLECTOR_SCRIPT = SCRIPT_DIR / "change_collect.py"
STORE_SCRIPT = SCRIPT_DIR / "change_store.py"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run change-lifecycle collectors and persist raw captures into shared SQLite.")
    ...
    for flag, value in (("--unit", args.unit), ("--repo-root", args.repo_root), ("--config-path", args.config_path), ("--url", args.url)):
        if value:
    ...
    print(json.dumps({"skill_key": "change_lifecycle", "run_id": stored.get("run_id") or raw.get("run_id"), "capture_ids": stored.get("capture_ids", []), "collector": args.collector or "run-all", "raw": raw}, indent=2))
    return 0
