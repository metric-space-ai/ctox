...
fn main() {
    eprintln!("bench_metalpack_decode_ffn_stack is only available on macOS + Metal.");
    std::process::exit(2);
}
...
fn main() -> Result<(), String> {
    use ctox_qwen35_08b_metal_probe::metal::bench::{
        run_decode_repeated_ffn_tiled_with_weights, DecodeSkeletonBenchConfig,
    };
    ...
    if args.len() < 2 {
        return Err("usage: bench_metalpack_decode_ffn_stack <metalpack-dir> [layers] [layer-prefix] [input-token] [iterations]".to_owned());
    }
    ...
    let layers = args
        ...
        .map(|arg| {
            arg.parse::<usize>()
                .map_err(|err| format!("invalid layers argument `{arg}`: {err}"))
        })
        .transpose()?
        .unwrap_or(6);
    ...
    let result = run_decode_repeated_ffn_tiled_with_weights(
        cfg,
        layers,
        &embedding_weights,
        &norm,
    ...
    println!("qwen35-08b metalpack repeated FFN decode benchmark");
    println!("metalpack: {}", root.display());
    println!("ffn_layers: {}", layers);
    ...
    println!(
        "effective_gb_s_repeated_ffn_lm_head_pairs: {:.2}",
