...
fn main() {
    eprintln!("bench_rms_matvec_tiled is only available on macOS + Metal.");
    std::process::exit(2);
}
...
fn main() -> Result<(), String> {
    use ctox_qwen35_08b_metal_probe::metal::bench::{
        run_rms_matvec_tiled_bench, RmsMatvecTiledBenchConfig,
    };
    use ctox_qwen35_08b_metal_probe::QWEN35_08B;

    let args: Vec<String> = std::env::args().collect();
    let mut cfg = RmsMatvecTiledBenchConfig::default();
    ...
    let result = run_rms_matvec_tiled_bench(cfg)?;
    println!("qwen35-08b fused_rms_matvec_fp16_tiled_k1024 benchmark");
    println!(
        "shape: [{} x {}] @ [{}]",
    ...
    println!(
        "effective_gb_s_packed_weight_norm_input_io: {:.2}",
