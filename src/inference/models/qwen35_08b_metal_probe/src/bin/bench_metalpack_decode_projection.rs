...
fn main() {
    eprintln!("bench_metalpack_decode_projection is only available on macOS + Metal.");
    std::process::exit(2);
}
...
fn main() -> Result<(), String> {
    use ctox_qwen35_08b_metal_probe::metal::bench::{
        run_decode_one_projection_tiled_with_weights, DecodeSkeletonBenchConfig,
    };
    ...
    if args.len() < 2 {
        return Err(
            "usage: bench_metalpack_decode_projection <metalpack-dir> [projection-filter] [input-token] [iterations]"
                .to_owned(),
        );
    }

    let root = PathBuf::from(&args[1]);
    let projection_filter = args
        .get(2)
    ...
    let projection = pack
        ...
        .iter()
        .find(|entry| {
            entry.tensor.contains(projection_filter)
                && entry.dtype == "F16"
                && entry.layout == PackLayout::Fp16RowTiled
                && entry.source_shape == [QWEN35_08B.hidden_size, QWEN35_08B.hidden_size]
        })
        .ok_or_else(|| {
            format!("no F16 row-tiled [1024,1024] projection matching `{projection_filter}`")
        })?;

    validate_entry("embedding", embedding)?;
    validate_entry("lm_head", lm_head)?;
    validate_entry("projection", projection)?;
    if projection.source_shape[0] != QWEN35_08B.hidden_size {
        return Err(format!(
            "projection must output hidden size {}, got {:?}",
            QWEN35_08B.hidden_size, projection.source_shape
        ));
    }
    ...
    if embedding.row_tile != lm_head.row_tile
        || embedding.col_tile != lm_head.col_tile
        || embedding.row_tile != projection.row_tile
        || embedding.col_tile != projection.col_tile
    {
        return Err("embedding/lm_head/projection tile mismatch".to_owned());
    }
    ...
    let projection_weights = read_u16_entry(&pack, projection)?;
    ...
    let result = run_decode_one_projection_tiled_with_weights(
        cfg,
        &embedding_weights,
        &norm,
        &projection_weights,
    ...
    println!("qwen35-08b metalpack decode + projection benchmark");
    println!("metalpack: {}", root.display());
    println!("embedding: {}", embedding.tensor);
    println!("projection: {}", projection.tensor);
    ...
    println!(
        "effective_gb_s_projection_lm_head_pairs: {:.2}",
