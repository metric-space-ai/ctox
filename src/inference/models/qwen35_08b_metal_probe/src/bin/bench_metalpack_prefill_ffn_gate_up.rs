...
fn main() {
    eprintln!("bench_metalpack_prefill_ffn_gate_up is only available on macOS + Metal.");
    std::process::exit(2);
}
...
fn main() -> Result<(), String> {
    use ctox_qwen35_08b_metal_probe::metal::bench::{
        run_prefill_ffn_gate_up_with_weights, PrefillFfnGateUpBenchConfig,
    };
    ...
    if args.len() < 2 {
        return Err(
            "usage: bench_metalpack_prefill_ffn_gate_up <metalpack-dir> [layer] [tokens] [iterations]"
                .to_owned(),
        );
    }
    ...
    validate_projection("gate", gate)?;
    validate_projection("up", up)?;
    validate_norm(norm)?;
    if gate.row_tile != up.row_tile || gate.col_tile != up.col_tile {
        return Err(format!(
            "gate/up tile mismatch: gate=({}, {}) up=({}, {})",
            gate.row_tile, gate.col_tile, up.row_tile, up.col_tile
        ));
    }
    if gate.source_shape != up.source_shape {
        return Err(format!(
            "gate/up shape mismatch: {:?} vs {:?}",
            gate.source_shape, up.source_shape
        ));
    }
    ...
    let cfg = PrefillFfnGateUpBenchConfig {
        tokens,
        row_tile: gate.row_tile,
        col_tile: gate.col_tile,
        warmup: 3,
        iterations,
    };
    let result = run_prefill_ffn_gate_up_with_weights(
        cfg,
        &x_host,
        &norm_weights,
        &gate_weights,
    ...
    println!("qwen35-08b metalpack prefill FFN gate/up benchmark");
...
#[cfg(target_os = "macos")]
fn validate_projection(label: &str, entry: &MetalPackEntry) -> Result<(), String> {
