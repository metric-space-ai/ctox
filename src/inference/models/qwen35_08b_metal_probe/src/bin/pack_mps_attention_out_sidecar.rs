...
fn run() -> Result<(), String> {
    ...
    if args.len() != 3 {
        return Err(
            "usage: pack_mps_attention_out_sidecar <source.metalpack-dir> <output.mps-attention-out-dir>"
                .to_owned(),
        );
    }
    ...
    for layer in 0..QWEN35_08B.n_layers {
        if QWEN35_08B.layer_kind(layer) != LayerKind::FullAttention {
            continue;
        }
        let out = find_layer_class(&pack, layer, TensorClass::AttentionO)?;
        validate_projection(
            "attention_o",
            out,
        ...
        let out_written = write_attention_out_mps(&mut weights, out, &out_bytes)?;
        offset += out_written;
        ...
        manifest_entries.push(json!({
            ...
            "out": {
                ...
                "shape": [QWEN35_08B.attention_q_width(), QWEN35_08B.hidden_size],
    ...
    let manifest = json!({
        "format": "ctox.qwen35_08b.mps_attention_out_sidecar",
        "version": 1,
        ...
        "shape": {
            ...
            "attention_width": QWEN35_08B.attention_q_width(),
            "attention_layers": manifest_entries.len(),
        },
...
fn write_attention_out_mps(
    ...
    let mut dst = vec![0u16; attention_width * hidden];
