...
fn main() {
    eprintln!("bench_mega_pattern_ffn is only available on macOS + Metal.");
    std::process::exit(2);
}
...
fn main() -> Result<(), String> {
    use ctox_qwen35_08b_metal_probe::metal::bench::{
        run_pattern_ffn_mega_bench, PatternFfnMegaBenchConfig,
    };

    let args: Vec<String> = std::env::args().collect();
    let mut cfg = PatternFfnMegaBenchConfig::default();
    ...
    let result = run_pattern_ffn_mega_bench(cfg)?;
    println!("qwen35-08b qwen-pattern+ffn single-dispatch mega benchmark");
    println!(
        "pipeline: token -> embedding -> ([D,D,D,A]x6 + SwiGLU FFN per layer) -> lm_head_argmax"
    );
    println!(
        "shape: vocab={} hidden={} intermediate={}",
        result.vocab_rows, result.cols, result.intermediate
    );
