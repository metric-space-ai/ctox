...
fn main() -> Result<(), String> {
    ...
    if args.len() < 5 {
        return Err(
            "usage: compare_attention_raw_dump <baseline.bin> <candidate.bin> <tokens> <width>"
                .to_owned(),
        );
    }
    ...
    if width % 256 != 0 {
        return Err("width must be divisible by 256 for token/head/lane reporting".to_owned());
    }
    ...
    println!("qwen35-08b attention raw dump compare");
...
fn print_index(label: &str, idx: usize, width: usize) {
    let token = idx / width;
    let within = idx % width;
    let head = within / 256;
    let lane = within % 256;
    println!("{label}_index: {idx}");
    println!("{label}_token: {token}");
    println!("{label}_head: {head}");
    println!("{label}_lane: {lane}");
}
