...
pub fn process_responses_event(
    ...
    match event.kind.as_str() {
        ...
        "error" => {
            let mut response_error = ApiError::Stream("error event received".into());
            if let Some(error_val) = event.error {
                if let Ok(error) = serde_json::from_value::<Error>(error_val.clone()) {
                    if is_context_window_error(&error) {
                        response_error = ApiError::ContextWindowExceeded;
                    } else if is_quota_exceeded_error(&error) {
                        response_error = ApiError::QuotaExceeded;
                        ...
                        let message = error
                            .message
                            .unwrap_or_else(|| "error event received".to_string());
                        response_error = ApiError::Retryable { message, delay };
                    }
                } else if let Some(message) = error_val.get("message").and_then(Value::as_str) {
                    response_error = ApiError::Stream(message.to_string());
                }
            }
            return Err(ResponsesEventError::Api(response_error));
        }
...
mod tests {
    ...
    #[tokio::test]
    async fn top_level_error_event_maps_to_invalid_request() {
        let raw_error = r#"{"type":"error","error":{"code":"invalid_prompt","message":"Local runtime rejected the request."}}"#;
        ...
        match &events[0] {
            Err(ApiError::InvalidRequest { message }) => {
                assert_eq!(message, "Local runtime rejected the request.");
            }
            other => panic!("unexpected event: {other:?}"),
        }
    }
