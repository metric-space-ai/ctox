...
use ctox_app_server_protocol::CommandExecOutputDeltaNotification;
use ctox_app_server_protocol::CommandExecOutputStream;
use ctox_app_server_protocol::CommandExecResizeParams;
use ctox_app_server_protocol::CommandExecResizeResponse;
use ctox_app_server_protocol::CommandExecResponse;
use ctox_app_server_protocol::CommandExecTerminalSize;
use ctox_app_server_protocol::CommandExecTerminateParams;
use ctox_app_server_protocol::CommandExecTerminateResponse;
use ctox_app_server_protocol::CommandExecWriteParams;
use ctox_app_server_protocol::CommandExecWriteResponse;
use ctox_app_server_protocol::JSONRPCErrorError;
use ctox_app_server_protocol::ServerNotification;
use ctox_core::bytes_to_string_smart;
use ctox_core::config::StartedNetworkProxy;
use ctox_core::exec::DEFAULT_EXEC_COMMAND_TIMEOUT_MS;
use ctox_core::exec::ExecExpiration;
use ctox_core::exec::IO_DRAIN_TIMEOUT_MS;
use ctox_core::exec::SandboxType;
use ctox_core::sandboxing::ExecRequest;
use ctox_utils_pty::DEFAULT_OUTPUT_BYTES_CAP;
use ctox_utils_pty::ProcessHandle;
use ctox_utils_pty::SpawnedProcess;
    ...
    pub(crate) async fn start(
        ...
        if matches!(exec_request.sandbox, SandboxType::WindowsRestrictedToken) {
            ...
            tokio::spawn(async move {
                ...
                match ctox_core::sandboxing::execute_env(exec_request, /*stdout_stream*/ None).await
                {
                    Ok(output) => {
                        outgoing
                            .send_response(
        ...
        let spawned = if tty {
            ctox_utils_pty::spawn_pty_process(
                program,
                args,
                cwd.as_path(),
                &env,
            ...
            ctox_utils_pty::spawn_pipe_process(program, args, cwd.as_path(), &env, &arg0).await
        } else {
            ctox_utils_pty::spawn_pipe_process_no_stdin(program, args, cwd.as_path(), &env, &arg0)
                .await
        };
...
mod tests {
    ...
    fn windows_sandbox_exec_request() -> ExecRequest {
        ...
        ExecRequest {
            ...
            sandbox_permissions: ctox_core::sandboxing::SandboxPermissions::UseDefault,
        ...
        let err = manager
            .start(StartCommandExecParams {
                ...
                request_id: ConnectionRequestId {
                    ...
                    request_id: ctox_app_server_protocol::RequestId::Integer(42),
                },
    ...
    async fn windows_sandbox_non_streaming_exec_uses_execution_path() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: ctox_app_server_protocol::RequestId::Integer(99),
        };
    ...
    async fn cancellation_expiration_keeps_process_alive_until_terminated() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: ctox_app_server_protocol::RequestId::Integer(100),
        };
        let sandbox_policy = SandboxPolicy::ReadOnly {
            access: ReadOnlyAccess::FullAccess,
            network_access: false,
    ...
    async fn windows_sandbox_process_ids_reject_terminate_requests() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: ctox_app_server_protocol::RequestId::Integer(2),
        };
        let process_id = ConnectionProcessId {
            connection_id: request_id.connection_id,
    ...
    async fn dropped_control_request_is_reported_as_not_running() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: ctox_app_server_protocol::RequestId::Integer(3),
        };
