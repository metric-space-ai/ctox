...
use ctox_app_server_protocol::ConfigBatchWriteParams;
use ctox_app_server_protocol::ConfigReadParams;
use ctox_app_server_protocol::ConfigReadResponse;
use ctox_app_server_protocol::ConfigRequirements;
use ctox_app_server_protocol::ConfigRequirementsReadResponse;
use ctox_app_server_protocol::ConfigValueWriteParams;
use ctox_app_server_protocol::ConfigWriteErrorCode;
use ctox_app_server_protocol::ConfigWriteResponse;
use ctox_app_server_protocol::JSONRPCErrorError;
use ctox_app_server_protocol::NetworkRequirements;
use ctox_app_server_protocol::SandboxMode;
use ctox_core::AnalyticsEventsClient;
use ctox_core::ThreadManager;
use ctox_core::config::ConfigService;
use ctox_core::config::ConfigServiceError;
use ctox_core::config_loader::CloudRequirementsLoader;
use ctox_core::config_loader::ConfigRequirementsToml;
use ctox_core::config_loader::LoaderOverrides;
use ctox_core::config_loader::ResidencyRequirement as CoreResidencyRequirement;
use ctox_core::config_loader::SandboxModeRequirement as CoreSandboxModeRequirement;
use ctox_core::plugins::PluginId;
use ctox_core::plugins::collect_plugin_enabled_candidates;
use ctox_core::plugins::installed_plugin_telemetry_metadata;
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ConfigRequirements {
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(ctox_app_server_protocol::AskForApproval::from)
                .collect()
        }),
...
) -> ctox_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => ctox_app_server_protocol::ResidencyRequirement::Us,
    }
}

fn map_network_requirements_to_api(
    network: ctox_core::config_loader::NetworkRequirementsToml,
) -> NetworkRequirements {
    NetworkRequirements {
...
mod tests {
    use super::*;
    use ctox_core::AnalyticsEventsClient;
    use ctox_core::config_loader::NetworkRequirementsToml as CoreNetworkRequirementsToml;
    ...
    fn map_requirements_toml_to_api_converts_core_enums() {
        let requirements = ConfigRequirementsToml {
            ...
            allowed_web_search_modes: Some(vec![
                ctox_core::config_loader::WebSearchModeRequirement::Cached,
            ]),
            feature_requirements: Some(ctox_core::config_loader::FeatureRequirementsToml {
        ...
        assert_eq!(
            ...
            Some(vec![
                ctox_app_server_protocol::AskForApproval::Never,
                ctox_app_server_protocol::AskForApproval::OnRequest,
            ])
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some(ctox_app_server_protocol::ResidencyRequirement::Us),
        );
        assert_eq!(
            mapped.network,
    ...
    async fn batch_write_reloads_user_config_when_requested() {
        ...
        let config_api = ConfigApi::new(
            ...
            AnalyticsEventsClient::new(
                analytics_config,
                ctox_core::test_support::auth_manager_from_auth(
                    ctox_core::CodexAuth::from_api_key("test"),
                ),
            ),
        );
        ...
        let response = config_api
            .batch_write(ConfigBatchWriteParams {
                edits: vec![ctox_app_server_protocol::ConfigEdit {
                    key_path: "model".to_string(),
                    value: json!("gpt-5"),
                    merge_strategy: ctox_app_server_protocol::MergeStrategy::Replace,
                }],
        ...
        assert_eq!(
            response,
            ConfigWriteResponse {
                status: ctox_app_server_protocol::WriteStatus::Ok,
                version: response.version.clone(),
                file_path: ctox_utils_absolute_path::AbsolutePathBuf::try_from(
