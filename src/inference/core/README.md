...
Note that `ctox-core` makes some assumptions about certain helper utilities being available in the environment. Currently, this support matrix is:
...
Expects the binary containing `ctox-core` to run the equivalent of `codex sandbox linux` (legacy alias: `codex debug landlock`) when `arg0` is `ctox-linux-sandbox`. See the `ctox-arg0` crate for details.
...
Expects the binary containing `ctox-core` to simulate the virtual `apply_patch` CLI when `arg1` is `--codex-run-as-apply-patch`. See the `ctox-arg0` crate for details.
