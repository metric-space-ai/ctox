...
use ctox_protocol::protocol::GuardianRiskLevel;
...
-> anyhow::Result<()> {
    ...
    settings.bind(|| {
        assert_snapshot!(
            "ctox_core__guardian__tests__guardian_review_request_layout",
...
async fn guardian_reuses_prompt_cache_key_and_appends_prior_reviews() -> anyhow::Result<()> {
    ...
    settings.bind(|| {
        assert_snapshot!(
            "ctox_core__guardian__tests__guardian_followup_review_request_layout",
            format!(
...
fn guardian_review_session_config_preserves_parent_network_proxy() {
    ...
    let guardian_config = build_guardian_review_session_config_for_test(
        ...
        None,
        "parent-active-model",
        Some(ctox_protocol::openai_models::ReasoningEffort::Low),
    )
