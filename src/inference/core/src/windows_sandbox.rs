...
use ctox_otel::sanitize_metric_tag_value;
...
pub fn sandbox_setup_is_complete(codex_home: &Path) -> bool {
    ctox_windows_sandbox::sandbox_setup_is_complete(codex_home)
}
...
pub fn elevated_setup_failure_details(err: &anyhow::Error) -> Option<(String, String)> {
    let failure = ctox_windows_sandbox::extract_setup_failure(err)?;
    let code = failure.code.as_str().to_string();
    let message = ctox_windows_sandbox::sanitize_setup_metric_tag_value(&failure.message);
    Some((code, message))
}
...
pub fn elevated_setup_failure_metric_name(err: &anyhow::Error) -> &'static str {
    if ctox_windows_sandbox::extract_setup_failure(err).is_some_and(|failure| {
        matches!(
            failure.code,
            ctox_windows_sandbox::SetupErrorCode::OrchestratorHelperLaunchCanceled
        )
    }) {
...
pub fn run_elevated_setup(
    ...
    ctox_windows_sandbox::run_elevated_setup(
        policy,
        policy_cwd,
        command_cwd,
        env_map,
...
pub fn run_legacy_setup_preflight(
    ...
    ctox_windows_sandbox::run_windows_sandbox_legacy_preflight(
        policy,
        policy_cwd,
        codex_home,
        command_cwd,
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    ctox_windows_sandbox::run_setup_refresh_with_extra_read_roots(
        policy,
        policy_cwd,
        command_cwd,
        env_map,
