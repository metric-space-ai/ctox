...
async fn sync_thread_state_after_write(
    ...
    if new_thread_memory_mode.is_some()
        || items
            .iter()
            .any(ctox_state::rollout_item_affects_thread_metadata)
    {
...
impl From<ctox_state::ThreadsPage> for ThreadsPage {
    fn from(db_page: ctox_state::ThreadsPage) -> Self {
        let items = db_page
            .items
            .into_iter()
            .map(|item| ThreadItem {
