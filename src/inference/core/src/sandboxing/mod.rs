...
pub use ctox_protocol::models::SandboxPermissions;
...
use ctox_protocol::protocol::NetworkAccess;
...
pub(crate) struct SandboxTransformRequest<'a> {
    ...
    pub ctox_linux_sandbox_exe: Option<&'a PathBuf>,
...
pub(crate) enum SandboxTransformError {
    #[error("missing ctox-linux-sandbox executable path")]
    ...
    pub(crate) fn transform(
        ...
        let (command, sandbox_env, arg0_override) = match sandbox {
            ...
            SandboxType::LinuxSeccomp => {
                let exe = ctox_linux_sandbox_exe
                ...
                (
                    full_command,
                    HashMap::new(),
                    Some("ctox-linux-sandbox".to_string()),
                )
            }
            // On Windows, the restricted token sandbox executes in-process via the
            // ctox-windows-sandbox crate. We leave the command unchanged here and
