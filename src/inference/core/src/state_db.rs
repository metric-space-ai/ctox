...
pub(crate) async fn init(config: &Config) -> Option<StateDbHandle> {
    let runtime = match ctox_state::StateRuntime::init(
    ...
    if backfill_state.status != ctox_state::BackfillStatus::Complete {
...
pub async fn get_state_db(config: &Config) -> Option<StateDbHandle> {
    let state_path = ctox_state::state_db_path(config.sqlite_home.as_path());
    if !tokio::fs::try_exists(&state_path).await.unwrap_or(false) {
        return None;
    }
    let runtime = ctox_state::StateRuntime::init(
        config.sqlite_home.clone(),
        config.model_provider_id.clone(),
    )
    .await
...
pub async fn open_if_present(codex_home: &Path, default_provider: &str) -> Option<StateDbHandle> {
    let db_path = ctox_state::state_db_path(codex_home);
    if !tokio::fs::try_exists(&db_path).await.unwrap_or(false) {
        return None;
    }
    let runtime =
        ctox_state::StateRuntime::init(codex_home.to_path_buf(), default_provider.to_string())
            .await
            .ok()?;
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == ctox_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<ctox_state::Anchor> {
    let cursor = cursor?;
...
pub async fn list_thread_ids_db(
    context: Option<&ctox_state::StateRuntime>,
    codex_home: &Path,
    page_size: usize,
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => ctox_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => ctox_state::SortKey::UpdatedAt,
            },
