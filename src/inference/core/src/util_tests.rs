...
fn emit_feedback_request_tags_records_sentry_feedback_fields() {
    ...
    let auth_env = AuthEnvTelemetry {
        ...
        ctox_api_key_env_present: false,
        ctox_api_key_env_enabled: true,
    ...
    assert_eq!(
        tags.get("auth_env_ctox_api_key_present")
            .map(String::as_str),
        Some("false")
    );
    assert_eq!(
        tags.get("auth_env_ctox_api_key_enabled")
            .map(String::as_str),
        Some("true")
    );
    assert_eq!(
