...
use ctox_protocol::permissions::FileSystemAccessMode;
use ctox_protocol::permissions::FileSystemPath;
use ctox_protocol::permissions::FileSystemSandboxEntry;
use ctox_protocol::permissions::FileSystemSandboxPolicy;
use ctox_protocol::permissions::FileSystemSpecialPath;
...
use ctox_app_server_protocol::AppInfo;
use ctox_execpolicy::Decision;
use ctox_execpolicy::NetworkRuleProtocol;
use ctox_execpolicy::Policy;
...
use ctox_protocol::protocol::RealtimeAudioFrame;
use ctox_protocol::protocol::Submission;
...
use ctox_protocol::mcp::CallToolResult as McpCallToolResult;
...
async fn turn_context_model_context_window_prefers_config_override() {
    ...
    assert_eq!(turn_context.model_context_window(), Some(131_072));
}
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let rollout_items = vec![
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            ctox_protocol::protocol::UserMessageEvent {
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            ctox_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn set_rate_limits_retains_previous_credits() {
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(ctox_protocol::account::PlanType::Plus),
    };
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        secondary: None,
        credits: None,
        plan_type: Some(ctox_protocol::account::PlanType::Pro),
    };
    state.set_rate_limits(update.clone());
...
fn init_test_tracing() {
    ...
    INIT.call_once(|| {
        ...
        let tracer = provider.tracer("ctox-core-tests");
        let subscriber =
...
async fn session_configuration_apply_preserves_split_file_system_policy_on_cwd_only_update() {
    ...
    let docs_dir =
        ctox_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&docs_dir).expect("docs");

    session_configuration.cwd = original_cwd;
    session_configuration.sandbox_policy =
        ctox_config::Constrained::allow_any(SandboxPolicy::WorkspaceWrite {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    session
        .notify_request_permissions_response(
            "missing",
            ctox_protocol::request_permissions::RequestPermissionsResponse {
                permissions: RequestPermissionProfile {
                    network: Some(ctox_protocol::models::NetworkPermissions {
                        enabled: Some(true),
                    }),
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let expected_response = ctox_protocol::request_permissions::RequestPermissionsResponse {
        permissions: RequestPermissionProfile {
            network: Some(ctox_protocol::models::NetworkPermissions {
                enabled: Some(true),
            }),
        ...
        async move {
            session
                .request_permissions(
                    turn_context.as_ref(),
                    call_id,
                    ctox_protocol::request_permissions::RequestPermissionsArgs {
...
async fn spawn_task_turn_span_inherits_dispatch_trace_context() {
    ...
    let submission_context =
        ctox_otel::context_from_w3c_trace_context(&submission_trace).expect("submission");
    let task_context = ctox_otel::context_from_w3c_trace_context(&task_trace).expect("task trace");
