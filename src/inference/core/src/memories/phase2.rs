...
fn artifact_memories_for_phase2(selection: &ctox_state::Phase2InputSelection) -> Vec<Stage1Output> {
...
mod job {
    ...
    pub(super) async fn claim(
        ...
        let (token, watermark) = match claim {
            ctox_state::Phase2JobClaimOutcome::Claimed {
                ownership_token,
                input_watermark,
            } => {
            ...
            ctox_state::Phase2JobClaimOutcome::SkippedNotDirty => return Err("skipped_not_dirty"),
            ctox_state::Phase2JobClaimOutcome::SkippedRunning => return Err("skipped_running"),
        };
    ...
    pub(super) async fn succeed(
        ...
        selected_outputs: &[ctox_state::Stage1Output],
        reason: &'static str,
    ) {
        session.services.session_telemetry.counter(
            metrics::MEMORY_PHASE_TWO_JOBS,
...
mod agent {
    ...
    pub(super) fn get_prompt(
        config: Arc<Config>,
        selection: &ctox_state::Phase2InputSelection,
    ) -> Vec<UserInput> {
        let root = memory_root(&config.codex_home);
    ...
    pub(super) fn handle(
        ...
        claim: Claim,
        new_watermark: i64,
        selected_outputs: Vec<ctox_state::Stage1Output>,
        thread_id: ThreadId,
        phase_two_e2e_timer: Option<ctox_otel::Timer>,
    ) {
...
pub(super) fn get_watermark(
    ...
    latest_memories: &[ctox_state::Stage1Output],
) -> i64 {
    latest_memories
        .iter()
