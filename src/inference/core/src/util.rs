...
/// `ctox_feedback::CodexFeedback::metadata_layer()` is installed, these fields are captured and
...
///
/// ```rust
/// ctox_core::feedback_tags!(model = "gpt-5", cached = true);
/// ctox_core::feedback_tags!(provider = provider_id, request_id = request_id);
/// ```
#[macro_export]
...
pub(crate) fn emit_feedback_request_tags_with_auth_env(
    ...
    feedback_tags!(
        ...
        auth_env_ctox_api_key_present = auth_env.ctox_api_key_env_present,
        auth_env_ctox_api_key_enabled = auth_env.ctox_api_key_env_enabled,
