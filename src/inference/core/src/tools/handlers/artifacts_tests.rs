...
use ctox_artifacts::RuntimeEntrypoints;
use ctox_artifacts::RuntimePathEntry;
...
fn parse_freeform_args_with_pragma() {
    let args = parse_freeform_args("// ctox-artifacts: timeout_ms=45000\nconsole.log('ok');")
        .expect("parse args");
    assert_eq!(args.source, "console.log('ok');");
...
fn load_cached_runtime_reads_pinned_cache_path() {
    let codex_home = TempDir::new().expect("create temp codex home");
    let platform =
        ctox_artifacts::ArtifactRuntimePlatform::detect_current().expect("detect platform");
    ...
    let runtime = ctox_artifacts::load_cached_runtime(
        &codex_home
            .path()
            .join(ctox_artifacts::DEFAULT_CACHE_ROOT_RELATIVE),
        versions::ARTIFACT_RUNTIME,
    )
