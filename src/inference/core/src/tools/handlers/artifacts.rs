...
use ctox_artifacts::ArtifactBuildRequest;
use ctox_artifacts::ArtifactCommandOutput;
use ctox_artifacts::ArtifactRuntimeManager;
use ctox_artifacts::ArtifactRuntimeManagerConfig;
use ctox_artifacts::ArtifactsClient;
use ctox_artifacts::ArtifactsError;
...
const ARTIFACTS_PRAGMA_PREFIXES: [&str; 2] = ["// ctox-artifacts:", "// codex-artifact-tool:"];
...
fn parse_freeform_args(input: &str) -> Result<ArtifactsToolArgs, FunctionCallError> {
    if input.trim().is_empty() {
        return Err(FunctionCallError::RespondToModel(
            "artifacts expects raw JavaScript source text (non-empty) authored against the preloaded @oai/artifact-tool surface. Provide JS only, optionally with first-line `// ctox-artifacts: timeout_ms=15000` or `// codex-artifact-tool: timeout_ms=15000`."
                .to_string(),
        ));
    }
...
fn reject_json_or_quoted_source(code: &str) -> Result<(), FunctionCallError> {
    ...
    if trimmed.starts_with("```") {
        return Err(FunctionCallError::RespondToModel(
            "artifacts expects raw JavaScript source, not markdown code fences. Resend plain JS only (optional first line `// ctox-artifacts: ...` or `// codex-artifact-tool: ...`)."
                .to_string(),
        ));
    }
    ...
    match value {
        JsonValue::Object(_) | JsonValue::String(_) => Err(FunctionCallError::RespondToModel(
            "artifacts is a freeform tool and expects raw JavaScript source authored against the preloaded @oai/artifact-tool surface. Resend plain JS only (optional first line `// ctox-artifacts: ...` or `// codex-artifact-tool: ...`); do not send JSON (`{\"code\":...}`), quoted code, or markdown fences."
                .to_string(),
        )),
        _ => Ok(()),
    }
