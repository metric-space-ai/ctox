...
use ctox_protocol::models::SearchToolCallParams;
use ctox_protocol::models::ShellToolCallParams;
use ctox_protocol::models::function_call_output_content_items_to_text;
use ctox_utils_string::take_bytes_at_char_boundary;
use serde::Serialize;
    ...
    fn response_text(&self) -> String {
        ...
        if let Some(command) = &self.session_command {
            sections.push(format!(
                "Command: {}",
                ctox_shell_command::parse_command::shlex_join(command)
            ));
        }
    ...
    match response {
        ResponseInputItem::Message { content, .. } => content_items_to_code_mode_result(
            &content
                .into_iter()
                .map(|item| match item {
                    ctox_protocol::models::ContentItem::InputText { text }
                    | ctox_protocol::models::ContentItem::OutputText { text } => {
                        FunctionCallOutputContentItem::InputText { text }
                    }
                    ctox_protocol::models::ContentItem::InputImage { image_url } => {
