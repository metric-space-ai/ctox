...
use ctox_protocol::config_types::WebSearchConfig;
use ctox_protocol::config_types::WebSearchMode;
use ctox_protocol::config_types::WindowsSandboxLevel;
use ctox_protocol::dynamic_tools::DynamicToolSpec;
use ctox_protocol::models::VIEW_IMAGE_TOOL_NAME;
...
pub(crate) struct ToolsConfig {
    ...
    pub ctox_web_stack_enabled: bool,
    pub ctox_doc_stack_enabled: bool,
...
impl ToolsConfig {
    pub fn new(params: &ToolsConfigParams) -> Self {
        ...
        let include_artifact_tools =
            features.enabled(Feature::Artifact) && ctox_artifacts::can_manage_artifact_runtime();
        ...
        let shell_type = if !features.enabled(Feature::ShellTool) {
            ...
            if ctox_utils_pty::conpty_supported() {
        ...
        Self {
            ...
            ctox_web_stack_enabled: false,
            ctox_doc_stack_enabled: false,
    ...
    pub fn with_ctox_web_stack_enabled(mut self, ctox_web_stack_enabled: bool) -> Self {
        self.ctox_web_stack_enabled = ctox_web_stack_enabled;
        self
    }
...
fn create_ctox_web_search_tool() -> ToolSpec {
    let properties = BTreeMap::from([
        (
            "query".to_string(),
            JsonSchema::String {
                description: Some("Search query text.".to_string()),
            },
        ),
        (
            "domains".to_string(),
            JsonSchema::Array {
                description: Some(
                    "Optional allowed host/domain filters such as docs.rs or example.com."
                        .to_string(),
                ),
                items: Box::new(JsonSchema::String { description: None }),
            },
        ...
        (
            ...
            JsonSchema::String {
                description: Some(
                    "Optional context depth: `low`, `medium`, or `high`.".to_string(),
                ),
            },
        ),
        (
            ...
            JsonSchema::Boolean {
                description: Some(
                    "When true, require cached search results instead of live external fetches."
                        .to_string(),
                ),
            },
        ),
        (
            ...
            JsonSchema::Boolean {
                description: Some(
                    "When true, include source URLs in the structured output.".to_string(),
                ),
            },
        ),
    ]);
    ...
    ToolSpec::Function(ResponsesApiTool {
        name: "ctox_web_search".to_string(),
        description: "Runs CTOX's own web-search pipeline with evidence extraction. This is distinct from Codex-native `web_search` and is only exposed on the CTOX-local path."
            .to_string(),
        strict: false,
        defer_loading: None,
        parameters: JsonSchema::Object {
...
fn create_ctox_web_read_tool() -> ToolSpec {
    let properties = BTreeMap::from([
        (
            "url".to_string(),
            JsonSchema::String {
                description: Some("Absolute URL to open and extract.".to_string()),
            },
        ),
        (
            "query".to_string(),
        ...
        (
            ...
            JsonSchema::Array {
                description: Some(
                    "Optional strings to locate within the extracted page text.".to_string(),
                ),
                items: Box::new(JsonSchema::String { description: None }),
            },
        ),
...
fn create_ctox_doc_search_tool() -> ToolSpec {
    let properties = BTreeMap::from([
        (
            "query".to_string(),
            JsonSchema::String {
                description: Some("Document search query text.".to_string()),
            },
        ),
        (
            "limit".to_string(),
            JsonSchema::Number {
                description: Some("Optional maximum number of results.".to_string()),
                minimum: Some(1.0),
                maximum: None,
            },
        ),
        (
            ...
            JsonSchema::String {
                description: Some(
                    "Optional retrieval mode: `lexical`, `hybrid`, or `semantic`.".to_string(),
                ),
            },
        ),
    ]);
    ...
    ToolSpec::Function(ResponsesApiTool {
        name: "ctox_doc_search".to_string(),
        description: "Runs CTOX's local document retrieval pipeline over the configured document corpus. Use `semantic` or `hybrid` when embeddings are available."
            .to_string(),
        strict: false,
        defer_loading: None,
        parameters: JsonSchema::Object {
...
fn create_ctox_doc_read_tool() -> ToolSpec {
    let properties = BTreeMap::from([
        (
            "path".to_string(),
            JsonSchema::String {
                description: Some("Absolute local file path to read.".to_string()),
            },
        ),
        (
            "query".to_string(),
            JsonSchema::String {
                description: Some(
                    "Optional reading intent used to bias excerpt selection.".to_string(),
                ),
            },
        ),
        (
            ...
            JsonSchema::Array {
                description: Some(
                    "Optional strings to locate within the extracted document text.".to_string(),
                ),
                items: Box::new(JsonSchema::String { description: None }),
            },
        ),
    ...
    ToolSpec::Function(ResponsesApiTool {
        name: "ctox_doc_read".to_string(),
        description: "Reads one concrete local document through CTOX's document extraction stack and optionally runs find-in-document on the extracted text."
            .to_string(),
        strict: false,
        defer_loading: None,
        parameters: JsonSchema::Object {
...
fn create_ctox_web_scrape_tool() -> ToolSpec {
    let properties = BTreeMap::from([
        (
            "target_key".to_string(),
            JsonSchema::String {
                description: Some(
                    "Registered CTOX scrape target key to read from.".to_string(),
                ),
            },
        ),
        (
        ...
        (
            ...
            JsonSchema::String {
                description: Some(
                    "Semantic query text. Required when `mode` is `semantic`; omit it when `mode` is `latest`."
                        .to_string(),
                ),
            },
        ),
    ...
    ToolSpec::Function(ResponsesApiTool {
        ...
        parameters: JsonSchema::Object {
            properties,
            required: Some(vec!["target_key".to_string(), "mode".to_string()]),
            additional_properties: Some(false.into()),
        },
        output_schema: None,
    })
...
fn create_ctox_browser_automation_tool() -> ToolSpec {
    const CTOX_BROWSER_AUTOMATION_GRAMMAR: &str = r#"
...
"#;
    ...
    ToolSpec::Freeform(FreeformTool {
        ...
        format: FreeformToolFormat {
            ...
            definition: CTOX_BROWSER_AUTOMATION_GRAMMAR.to_string(),
        },
    })
}
...
fn create_channel_sync_tool() -> ToolSpec {
    ...
    properties.insert(
        ...
        JsonSchema::String {
            description: Some(
                "Optional absolute path to the CTOX communication SQLite database.".to_string(),
            ),
        },
    );
    properties.insert(
        "limit".to_string(),
        JsonSchema::Number {
            description: Some("Optional adapter-specific fetch limit.".to_string()),
            minimum: Some(1.0),
            maximum: None,
        },
    );
    ToolSpec::Function(ResponsesApiTool {
        name: "channel_sync".to_string(),
        description:
            "Sync one external channel adapter into the shared CTOX communication SQLite store."
                .to_string(),
        strict: false,
        defer_loading: None,
        parameters: JsonSchema::Object {
            properties,
            required: Some(vec!["channel".to_string()]),
            additional_properties: Some(false.into()),
        },
        output_schema: None,
    })
...
fn create_channel_take_tool() -> ToolSpec {
    let mut properties = BTreeMap::new();
    properties.insert(
        "channel".to_string(),
        JsonSchema::String {
    ...
    properties.insert(
        "limit".to_string(),
        JsonSchema::Number {
            description: Some("Maximum number of inbound messages to lease.".to_string()),
            minimum: Some(1.0),
            maximum: Some(100.0),
        },
    );
    properties.insert(
        ...
        JsonSchema::String {
            description: Some("Optional lease owner label written into routing state.".to_string()),
        },
    );
    ToolSpec::Function(ResponsesApiTool {
        name: "channel_take".to_string(),
        description: "Lease the next inbound messages from the shared CTOX communication store across one or all channels.".to_string(),
        strict: false,
        defer_loading: None,
        parameters: JsonSchema::Object {
            properties,
...
fn create_channel_ack_tool() -> ToolSpec {
    let mut properties = BTreeMap::new();
    properties.insert(
        "message_keys".to_string(),
        JsonSchema::Array {
            description: Some(
                "Message keys to mark as handled or otherwise acknowledged.".to_string(),
            ),
            items: Box::new(JsonSchema::String { description: None }),
        },
    );
    ...
    properties.insert(
        "status".to_string(),
        JsonSchema::String {
            description: Some("Routing status to write, typically handled or failed.".to_string()),
        },
    );
    ToolSpec::Function(ResponsesApiTool {
        name: "channel_ack".to_string(),
        description:
            "Acknowledge previously leased channel messages in the shared CTOX communication store."
                .to_string(),
        strict: false,
        defer_loading: None,
        parameters: JsonSchema::Object {
            properties,
            required: Some(vec!["message_keys".to_string()]),
            additional_properties: Some(false.into()),
        },
        output_schema: None,
    })
...
fn create_channel_send_tool() -> ToolSpec {
    let mut properties = BTreeMap::new();
    properties.insert(
        "channel".to_string(),
        JsonSchema::String {
    ...
    properties.insert(
        ...
        JsonSchema::String {
            description: Some("Stable conversation key to send into.".to_string()),
        },
    );
    properties.insert(
        "body".to_string(),
        JsonSchema::String {
            description: Some("Plain-text body to send.".to_string()),
        },
    );
    properties.insert(
        "subject".to_string(),
        JsonSchema::String {
            description: Some("Optional subject or thread label.".to_string()),
        },
    );
    properties.insert(
        "to".to_string(),
        JsonSchema::Array {
            description: Some("Recipients or remote conversation targets.".to_string()),
            items: Box::new(JsonSchema::String { description: None }),
        },
    );
    properties.insert(
        "cc".to_string(),
        JsonSchema::Array {
            description: Some("Optional CC recipients for email-like channels.".to_string()),
            items: Box::new(JsonSchema::String { description: None }),
        },
    );
    properties.insert(
        ...
        JsonSchema::String {
            description: Some("Optional sender display name override.".to_string()),
        },
    );
    properties.insert(
        ...
        JsonSchema::String {
            description: Some(
                "Optional sender address or account identifier override.".to_string(),
            ),
        },
    );
    properties.insert(
...
fn create_context_retrieve_tool() -> ToolSpec {
    let mut properties = BTreeMap::new();
    properties.insert(
        "conversation_id".to_string(),
        JsonSchema::Number {
            description: Some("Conversation id to retrieve context from. Defaults to 1 for the main CTOX chat thread.".to_string()),
            minimum: Some(1.0),
            maximum: None,
        },
    );
    properties.insert(
        "mode".to_string(),
        JsonSchema::String {
            description: Some("Retrieval mode. Supported values: current, continuity, forgotten, search, describe, expand.".to_string()),
        },
    );
    properties.insert(
        "kind".to_string(),
        JsonSchema::String {
            description: Some("Optional continuity document selector for continuity or forgotten modes. Supported values: narrative, anchors, focus.".to_string()),
        },
    );
    properties.insert(
        "query".to_string(),
        JsonSchema::String {
            description: Some(
                "Optional free-text query used by search and forgotten modes.".to_string(),
            ),
        },
    );
    properties.insert(
        ...
        JsonSchema::String {
            description: Some("Summary id required by describe and expand modes.".to_string()),
        },
    );
    properties.insert(
        "limit".to_string(),
        JsonSchema::Number {
            description: Some("Optional maximum number of search or forgotten results to inspect. Defaults to 10.".to_string()),
            minimum: Some(1.0),
            maximum: Some(100.0),
        },
    );
    properties.insert(
        "depth".to_string(),
        JsonSchema::Number {
            description: Some("Expansion depth for expand mode. Defaults to 1.".to_string()),
            minimum: Some(1.0),
            maximum: Some(8.0),
        },
    );
    properties.insert(
        ...
        JsonSchema::Boolean {
            description: Some(
                "Whether expand mode should also include underlying messages.".to_string(),
            ),
        },
    );
    properties.insert(
        "token_cap".to_string(),
        JsonSchema::Number {
            description: Some("Token cap for expand mode. Defaults to 8000.".to_string()),
            minimum: Some(128.0),
            maximum: None,
        },
    );
    ToolSpec::Function(ResponsesApiTool {
        ...
        description: "Retrieve CTOX context through one tool across the active context, continuity documents, forgotten continuity lines, search results, summary description, or summary expansion.".to_string(),
        strict: false,
        defer_loading: None,
        parameters: JsonSchema::Object {
            properties,
...
pub(crate) fn build_specs_with_discoverable_tools(
    ...
    use crate::tools::handlers::ChannelHandler;
    use crate::tools::handlers::CodeModeExecuteHandler;
    use crate::tools::handlers::CodeModeWaitHandler;
    use crate::tools::handlers::ContextRetrieveHandler;
    use crate::tools::handlers::CtoxBrowserAutomationHandler;
    use crate::tools::handlers::CtoxDocHandler;
    use crate::tools::handlers::CtoxWebHandler;
    ...
    let channel_handler = Arc::new(ChannelHandler);
    let context_retrieve_handler = Arc::new(ContextRetrieveHandler);
    ...
    if config.ctox_web_stack_enabled {
        let ctox_web_handler = Arc::new(CtoxWebHandler);
        let ctox_browser_automation_handler = Arc::new(CtoxBrowserAutomationHandler);
        push_tool_spec(
            &mut builder,
            create_ctox_web_search_tool(),
            /*supports_parallel_tool_calls*/ false,
            config.code_mode_enabled,
        );
        push_tool_spec(
            &mut builder,
            create_ctox_web_read_tool(),
            /*supports_parallel_tool_calls*/ false,
            config.code_mode_enabled,
        );
        push_tool_spec(
        ...
        builder.register_handler("ctox_browser_automation", ctox_browser_automation_handler);
    }

    if config.ctox_doc_stack_enabled {
        let ctox_doc_handler = Arc::new(CtoxDocHandler);
        push_tool_spec(
            &mut builder,
            create_ctox_doc_search_tool(),
            /*supports_parallel_tool_calls*/ false,
            config.code_mode_enabled,
        );
        push_tool_spec(
            &mut builder,
            create_ctox_doc_read_tool(),
            /*supports_parallel_tool_calls*/ false,
            config.code_mode_enabled,
        );
