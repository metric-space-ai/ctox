...
`ctox-otel` is the OpenTelemetry integration crate for Codex. It provides:

- Provider wiring for log/trace/metric exporters (`ctox_otel::OtelProvider`
  and `ctox_otel::provider`).
- Session-scoped business event emission via `ctox_otel::SessionTelemetry`.
- Low-level metrics APIs via `ctox_otel::metrics`.
- Trace-context helpers via `ctox_otel::trace_context` and crate-root re-exports.
...
```rust
use ctox_otel::config::OtelExporter;
use ctox_otel::config::OtelHttpProtocol;
use ctox_otel::config::OtelSettings;
use ctox_otel::OtelProvider;
use tracing_subscriber::prelude::*;

let settings = OtelSettings {
    environment: "dev".to_string(),
    service_name: "ctox-cli".to_string(),
...
`ctox-otel` also provides `OtelExporter::Statsig`, a shorthand for exporting OTLP/HTTP JSON metrics
to Statsig using Codex-internal defaults.

Statsig ingestion (OTLP/HTTP JSON) example:

```rust
use ctox_otel::config::{OtelExporter, OtelHttpProtocol};
