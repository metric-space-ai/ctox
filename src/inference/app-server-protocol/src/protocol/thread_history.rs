...
use ctox_protocol::protocol::AgentReasoningEvent;
use ctox_protocol::protocol::AgentReasoningRawContentEvent;
use ctox_protocol::protocol::AgentStatus;
use ctox_protocol::protocol::ApplyPatchApprovalRequestEvent;
use ctox_protocol::protocol::CompactedItem;
use ctox_protocol::protocol::ContextCompactedEvent;
use ctox_protocol::protocol::DynamicToolCallResponseEvent;
use ctox_protocol::protocol::ErrorEvent;
use ctox_protocol::protocol::EventMsg;
use ctox_protocol::protocol::ExecCommandBeginEvent;
use ctox_protocol::protocol::ExecCommandEndEvent;
use ctox_protocol::protocol::ImageGenerationBeginEvent;
use ctox_protocol::protocol::ImageGenerationEndEvent;
...
use ctox_protocol::protocol::PatchApplyBeginEvent;
use ctox_protocol::protocol::PatchApplyEndEvent;
...
use ctox_protocol::protocol::TurnAbortedEvent;
use ctox_protocol::protocol::TurnCompleteEvent;
use ctox_protocol::protocol::TurnStartedEvent;
use ctox_protocol::protocol::UserMessageEvent;
use ctox_protocol::protocol::ViewImageToolCallEvent;
use ctox_protocol::protocol::WebSearchBeginEvent;
use ctox_protocol::protocol::WebSearchEndEvent;
...
impl ThreadHistoryBuilder {
    ...
    fn handle_item_started(&mut self, payload: &ItemStartedEvent) {
        match &payload.item {
            ctox_protocol::items::TurnItem::Plan(plan) => {
            ...
            ctox_protocol::items::TurnItem::UserMessage(_)
            | ctox_protocol::items::TurnItem::AgentMessage(_)
            | ctox_protocol::items::TurnItem::Reasoning(_)
            | ctox_protocol::items::TurnItem::WebSearch(_)
            | ctox_protocol::items::TurnItem::ImageGeneration(_)
            | ctox_protocol::items::TurnItem::ContextCompaction(_) => {}
        }
    }
    ...
    fn handle_dynamic_tool_call_request(
        &mut self,
        payload: &ctox_protocol::dynamic_tools::DynamicToolCallRequest,
    ) {
    ...
    fn handle_collab_agent_spawn_begin(
        &mut self,
        payload: &ctox_protocol::protocol::CollabAgentSpawnBeginEvent,
    ) {
    ...
    fn handle_collab_agent_spawn_end(
        &mut self,
        payload: &ctox_protocol::protocol::CollabAgentSpawnEndEvent,
    ) {
    ...
    fn handle_collab_agent_interaction_begin(
        &mut self,
        payload: &ctox_protocol::protocol::CollabAgentInteractionBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_agent_interaction_end(
        &mut self,
        payload: &ctox_protocol::protocol::CollabAgentInteractionEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
    ...
    fn handle_collab_waiting_begin(
        &mut self,
        payload: &ctox_protocol::protocol::CollabWaitingBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_waiting_end(
        &mut self,
        payload: &ctox_protocol::protocol::CollabWaitingEndEvent,
    ) {
    ...
    fn handle_collab_close_begin(
        &mut self,
        payload: &ctox_protocol::protocol::CollabCloseBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_close_end(&mut self, payload: &ctox_protocol::protocol::CollabCloseEndEvent) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
        };
    ...
    fn handle_collab_resume_begin(
        &mut self,
        payload: &ctox_protocol::protocol::CollabResumeBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_resume_end(
        &mut self,
        payload: &ctox_protocol::protocol::CollabResumeEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
    ...
    fn handle_entered_review_mode(&mut self, payload: &ctox_protocol::protocol::ReviewRequest) {
        let review = payload
            .user_facing_hint
            .clone()
    ...
    fn handle_exited_review_mode(
        &mut self,
        payload: &ctox_protocol::protocol::ExitedReviewModeEvent,
    ) {
        let review = payload
            .review_output
            .as_ref()
...
pub fn convert_patch_changes(
    changes: &HashMap<std::path::PathBuf, ctox_protocol::protocol::FileChange>,
...
fn convert_dynamic_tool_content_items(
    items: &[ctox_protocol::dynamic_tools::DynamicToolCallOutputContentItem],
) -> Vec<DynamicToolCallOutputContentItem> {
    items
        .iter()
        .cloned()
        .map(|item| match item {
            ctox_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText { text } => {
                DynamicToolCallOutputContentItem::InputText { text }
            }
            ctox_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputImage {
                image_url,
...
fn map_patch_change_kind(change: &ctox_protocol::protocol::FileChange) -> PatchChangeKind {
    match change {
        ctox_protocol::protocol::FileChange::Add { .. } => PatchChangeKind::Add,
        ctox_protocol::protocol::FileChange::Delete { .. } => PatchChangeKind::Delete,
        ctox_protocol::protocol::FileChange::Update { move_path, .. } => PatchChangeKind::Update {
            move_path: move_path.clone(),
        },
    }
}

fn format_file_change_diff(change: &ctox_protocol::protocol::FileChange) -> String {
    match change {
        ctox_protocol::protocol::FileChange::Add { content } => content.clone(),
        ctox_protocol::protocol::FileChange::Delete { content } => content.clone(),
        ctox_protocol::protocol::FileChange::Update {
            unified_diff,
            move_path,
        } => {
...
mod tests {
    ...
    use ctox_protocol::items::UserMessageItem as CoreUserMessageItem;
    use ctox_protocol::models::MessagePhase as CoreMessagePhase;
    use ctox_protocol::models::WebSearchAction as CoreWebSearchAction;
    use ctox_protocol::parse_command::ParsedCommand;
    ...
    use ctox_protocol::protocol::DynamicToolCallResponseEvent;
    use ctox_protocol::protocol::ExecCommandEndEvent;
    use ctox_protocol::protocol::ExecCommandSource;
    use ctox_protocol::protocol::ItemStartedEvent;
    ...
    fn reconstructs_dynamic_tool_items_from_request_and_response_events() {
        let events = vec![
            ...
            EventMsg::DynamicToolCallRequest(
                ctox_protocol::dynamic_tools::DynamicToolCallRequest {
                    call_id: "dyn-1".into(),
                    turn_id: "turn-1".into(),
        ...
        let events = vec![
            ...
            EventMsg::PatchApplyEnd(PatchApplyEndEvent {
                ...
                changes: [(
                    ...
                    ctox_protocol::protocol::FileChange::Add {
                        content: "hello\n".into(),
                    },
                )]
                .into_iter()
    ...
    fn reconstructs_collab_resume_end_item() {
        let events = vec![
            ...
            EventMsg::CollabResumeEnd(ctox_protocol::protocol::CollabResumeEndEvent {
    ...
    fn reconstructs_collab_spawn_end_item_with_model_metadata() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentSpawnEnd(ctox_protocol::protocol::CollabAgentSpawnEndEvent {
                ...
                reasoning_effort: ctox_protocol::openai_models::ReasoningEffort::Medium,
        ...
        assert_eq!(
            ...
            ThreadItem::CollabAgentToolCall {
                ...
                reasoning_effort: Some(ctox_protocol::openai_models::ReasoningEffort::Medium),
                agents_states: [(
                    "00000000-0000-0000-0000-000000000002".into(),
                    CollabAgentState {
    ...
    fn reconstructs_interrupted_send_input_as_completed_collab_call() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentInteractionBegin(
                ctox_protocol::protocol::CollabAgentInteractionBeginEvent {
                    call_id: "send-1".into(),
            ...
            EventMsg::CollabAgentInteractionEnd(
                ctox_protocol::protocol::CollabAgentInteractionEndEvent {
                    call_id: "send-1".into(),
                    sender_thread_id: sender,
                    receiver_thread_id: receiver,
                    receiver_agent_nickname: None,
