...
impl HelperExecutable {
    fn file_name(self) -> &'static str {
        match self {
            Self::CommandRunner => "ctox-command-runner.exe",
        }
    }
...
mod tests {
    ...
    fn copy_runner_into_shared_bin_dir() {
        ...
        let runner_source = source_dir.join("ctox-command-runner.exe");
        let runner_destination = helper_bin_dir(&codex_home).join("ctox-command-runner.exe");
