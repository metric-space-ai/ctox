...
impl SkillsManager {
    ...
    pub async fn skills_for_cwd_with_extra_user_roots(
        ...
        if !skills_enabled_from_stack(&config_layer_stack) {
...
pub(crate) fn skills_enabled_from_stack(
    config_layer_stack: &crate::config_loader::ConfigLayerStack,
) -> bool {
...
pub(crate) fn bundled_skills_enabled_from_stack(
    ...
    let Some(skills) = skills_config_from_stack(config_layer_stack) else {
        return true;
    };
    if skills.enabled == Some(false) {
        return false;
    }
...
fn finalize_skill_outcome(
    ...
    outcome.disabled_paths = resolve_disabled_skill_paths(&outcome.skills, skill_config_rules);
