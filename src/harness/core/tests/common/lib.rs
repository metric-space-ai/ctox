...
use anyhow::ensure;
use ctor::ctor;
use ctox_utils_cargo_bin::CargoBinError;
use tempfile::TempDir;

use ctox_config::CloudRequirementsLoader;
use ctox_config::ConfigRequirementsToml;
use ctox_config::NetworkRequirementsToml;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    ctox_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    ctox_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = ctox_utils_cargo_bin::repo_root()
        .ok()
        .map(|root| root.join("ctox-rs"));
...
impl PathExt for Path {
    fn abs(&self) -> AbsolutePathBuf {
        assert!(self.is_absolute(), "path should already be absolute");
        AbsolutePathBuf::from_absolute_path(self).expect("path should be valid")
    }
}
...
pub fn test_tmp_path() -> AbsolutePathBuf {
    test_absolute_path_with_windows("/tmp", Some(r"C:\Users\ctox\AppData\Local\Temp"))
}
...
/// avoids clobbering a developer’s real `~/.ctox`.
pub async fn load_default_config_for_test(ctox_home: &TempDir) -> Config {
...
pub async fn load_default_config_for_test_with_cloud_requirements(
    ...
    ConfigBuilder::default()
        .codex_home(ctox_home.path().to_path_buf())
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        ctox_linux_sandbox_exe: Some(
            find_ctox_linux_sandbox_exe().expect("should find binary for ctox-linux-sandbox"),
        ),
...
#[cfg(target_os = "linux")]
pub fn find_ctox_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ctox_utils_cargo_bin::cargo_bin("ctox-linux-sandbox")
}
...
pub async fn wait_for_event<F>(
    ctox: &CodexThread,
    predicate: F,
) -> ctox_protocol::protocol::EventMsg
where
    F: FnMut(&ctox_protocol::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    wait_for_event_with_timeout(ctox, predicate, Duration::from_secs(1)).await
}

pub async fn wait_for_event_match<T, F>(ctox: &CodexThread, matcher: F) -> T
where
    F: Fn(&ctox_protocol::protocol::EventMsg) -> Option<T>,
{
    let ev = wait_for_event(ctox, |ev| matcher(ev).is_some()).await;
    ...
    F: FnMut(&ctox_protocol::protocol::EventMsg) -> bool,
{
    ...
    loop {
        ...
        let ev = timeout(wait_time.max(Duration::from_secs(10)), ctox.next_event())
            .await
...
pub fn sandbox_env_var() -> &'static str {
    ctox_core::spawn::CODEX_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    ctox_core::spawn::CODEX_SANDBOX_NETWORK_DISABLED_ENV_VAR
}
...
pub fn format_with_current_shell(command: &str) -> Vec<String> {
    ctox_core::shell::default_user_shell().derive_exec_args(command, /*use_login_shell*/ true)
}
...
pub fn format_with_current_shell_non_login(command: &str) -> Vec<String> {
    ctox_core::shell::default_user_shell().derive_exec_args(command, /*use_login_shell*/ false)
}
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    ctox_utils_cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}
...
macro_rules! skip_if_no_network {
    () => {{
        if ::std::env::var($crate::sandbox_network_env_var()).is_ok() {
            println!(
                "Skipping test because it cannot execute when network is disabled in a CTOX sandbox."
            );
            return;
        }
    }};
...
#[macro_export]
macro_rules! ctox_linux_sandbox_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_ctox_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("ctox-linux-sandbox binary not available, skipping test: {err}");
                    return;
                }
            }
        }
