...
use super::channels::{
    open_channel_db, upsert_communication_message, ensure_routing_rows_for_inbound,
    refresh_thread, UpsertMessage,
};
...
pub(crate) fn sync(
    ...
    for entry in fs::read_dir(&sessions_dir).unwrap_or_else(|_| fs::read_dir(".").unwrap()) {
        ...
        let Ok(session) = serde_json::from_str::<Value>(&contents) else { continue };
        let status = session.get("status").and_then(Value::as_str).unwrap_or("unknown");
        if status != "active" && status != "joining" && status != "running" {
            continue;
        }
        active += 1;

        let session_id = session.get("session_id").and_then(Value::as_str).unwrap_or("");
        let provider = session.get("provider").and_then(Value::as_str).unwrap_or("unknown");

        // Read chat messages from the session JSON and ingest any not yet in SQLite
        let chat_messages = session.get("chat_messages")
            .and_then(Value::as_array)
            .cloned()
            .unwrap_or_default();

        for msg in &chat_messages {
            let sender = msg.get("sender").and_then(Value::as_str).unwrap_or("Unknown");
...
pub(crate) fn send(
    ...
    if let Some(stdin_path) = stdin_path {
        ...
        match fs::OpenOptions::new().append(true).open(stdin_path) {
            Ok(mut file) => { let _ = writeln!(file, "{}", command); }
            Err(err) => {
    ...
    Ok(json!({"ok": true, "status": "sent", "session_id": request.session_id, "message_key": message_key}))
}
...
pub fn handle_meeting_command(root: &Path, args: &[String]) -> Result<()> {
    ...
    match command {
        "join" => {
            let url = args.get(1).context("usage: ctox meeting join <url> [--name <bot-name>]")?;
            let bot_name = find_flag_value(args, "--name")
        ...
        "schedule" => {
            let url = args.get(1).context("usage: ctox meeting schedule <url> --time <ISO-8601>")?;
            let time = find_flag_value(args, "--time")
                .context("--time <ISO-8601> is required")?;
            let bot_name = find_flag_value(args, "--name")
                .unwrap_or("CTOX Notetaker");
        ...
        "dump-script" => {
            let url = args.get(1).context("usage: ctox meeting dump-script <url>")?;
            let runtime = super::communication_gateway::runtime_settings_from_root(
                root,
                super::communication_gateway::CommunicationAdapterKind::Meeting,
            );
        ...
        _ => {
            println!("usage: ctox meeting <join|schedule|cancel|status> [args]");
            println!();
...
pub(crate) fn run_meeting_session(
    root: &Path,
    ...
    for line in reader.lines() {
        ...
        let event: Value = match serde_json::from_str(&line) {
            Ok(v) => v,
            Err(_) => {
                eprintln!("[meeting] non-JSON output: {}", &line[..line.len().min(200)]);
                continue;
            }
        };
        ...
        match event_type {
            ...
            "chat" => {
                let sender = event.get("sender").and_then(Value::as_str).unwrap_or("Unknown");
                ...
                if session.is_own_message(sender, text) {
                    eprintln!("[meeting] skipped own message: {}", &text[..text.len().min(60)]);
                    continue;
                }
...
fn finalize_meeting(
    ...
    let transcript_path = meeting_sessions_dir(root)
        .join(format!("{}-transcript.txt", session.session_id));
    fs::write(&transcript_path, &transcript)?;

    let chat_log_path = meeting_sessions_dir(root)
        .join(format!("{}-chatlog.txt", session.session_id));
...
impl MeetingSession {
    ...
    pub(crate) fn is_own_message(&self, sender: &str, text: &str) -> bool {
        ...
        if text_trimmed.starts_with(bot_name_lower) {
            ...
            if after_name.starts_with(':') || after_name.starts_with(" -") || after_name.starts_with(" to ") {
                return true;
            }
        }
        false
...
pub(crate) fn transcribe_audio_chunk(
    ...
    let response = super::communication_email_native::http_request(
        "POST",
        &format!("http://{}:{}/v1/audio/transcriptions", proxy_host, proxy_port),
        &headers,
        Some(&body),
    )?;
        ...
        if trimmed.starts_with("DTSTART") {
            ...
            if let Some(value) = trimmed.rsplit(':').next() {
                ...
                if value.len() >= 15 {
                    ...
                    return Some(format!(
                        "{year}-{month}-{day}T{hour}:{min}:{sec}{tz}"
                    ));
                }
            }
        }
...
pub(crate) fn cancel_meeting_join(root: &Path, meeting_url: &str) -> Result<Value> {
    ...
    if let Err(err) = super::schedule::remove_task(root, &format!("sched_{}", stable_digest(&format!("{schedule_name}:meeting:{}", MeetingProvider::detect(meeting_url).map(|p| p.as_str()).unwrap_or("unknown"))))) {
...
mod tests {
    ...
    fn mention_detection_respects_word_boundary() {
        ...
        assert!(MeetingSession::is_mention("@CTOX Notetaker hat geantwortet"));
        ...
        assert!(!MeetingSession::is_mention("https://example.com/@ctoxapi/foo"));
    }
    ...
    fn engine_reachable_check_returns_false_for_closed_port() {
        ...
        // Invalid host
        assert!(!check_engine_reachable("this-host-does-not-exist.invalid", 80));
    }
    ...
    fn detect_cancellation_and_update() {
        assert!(is_meeting_cancellation("Meeting Canceled: Sprint Review", ""));
    ...
    fn timeout_and_inactivity_detection_present_in_script() {
        ...
        assert!(script.contains("detectLoneParticipant"), "missing participant detection");
        assert!(script.contains("data-avatar-count"), "missing Google Meet badge detection");
        assert!(script.contains("detectSilence"), "missing silence detection");
        assert!(script.contains("AudioContext"), "missing AudioContext for silence");
        assert!(script.contains("ctoxMeetingEnd"), "missing meeting end callback");
        ...
        assert!(teams_script.contains("ffmpeg"), "Teams should use ffmpeg recording");
        assert!(teams_script.contains("virtual_output.monitor"), "Teams should use PulseAudio");
        assert!(teams_script.contains("--kiosk"), "Teams should use kiosk mode");
    }
}
