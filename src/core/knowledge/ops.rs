...
pub fn head(root: &Path, args: &[String]) -> Result<()> {
    const USAGE: &str = "ctox knowledge data head --domain X --key Y [--limit N] [--columns c1,c2]";
...
pub fn stats(root: &Path, args: &[String]) -> Result<()> {
    const USAGE: &str = "ctox knowledge data stats --domain X --key Y [--columns c1,c2]";
    let (_d, _k, path, _conn, _row) = resolve_table(root, args, USAGE)?;
    let columns = parse_columns(args);
...
pub fn count(root: &Path, args: &[String]) -> Result<()> {
    const USAGE: &str = "ctox knowledge data count --domain X --key Y [--where col=val] ...";
    let (_d, _k, path, _conn, _row) = resolve_table(root, args, USAGE)?;
...
pub fn append(root: &Path, args: &[String]) -> Result<()> {
    const USAGE: &str = "ctox knowledge data append --domain X --key Y --rows <json-array>";
    ...
    let parsed: Value = serde_json::from_str(raw)
        .with_context(|| format!("--rows is not valid JSON; expected a JSON array of objects"))?;
    let arr = parsed
        .as_array()
...
pub fn update(root: &Path, args: &[String]) -> Result<()> {
    ...
    if !path.exists() {
        bail!("no parquet on disk for domain={domain} key={table_key}; nothing to update");
    }
...
pub fn delete_rows(root: &Path, args: &[String]) -> Result<()> {
    const USAGE: &str = "ctox knowledge data delete-rows --domain X --key Y --where col=val";
    let (domain, table_key, path, conn, catalog_row) = resolve_table(root, args, USAGE)?;
    bail_if_archived(&catalog_row, &domain, &table_key)?;
    let wheres = collect_wheres(args);
    if wheres.is_empty() {
    ...
    if !path.exists() {
        bail!("no parquet on disk for domain={domain} key={table_key}; nothing to delete");
    }
    {
        let schema = scan_table(&path)?.collect_schema()?;
        let names = where_column_names(&wheres);
...
pub fn drop_column(root: &Path, args: &[String]) -> Result<()> {
    const USAGE: &str = "ctox knowledge data drop-column --domain X --key Y --column N";
    let (domain, table_key, path, conn, catalog_row) = resolve_table(root, args, USAGE)?;
    bail_if_archived(&catalog_row, &domain, &table_key)?;
    let column = required_flag(args, "--column", USAGE)?.to_string();

    if !path.exists() {
        bail!("no parquet on disk for domain={domain} key={table_key}; nothing to drop");
    }
...
pub fn import(root: &Path, args: &[String]) -> Result<()> {
    ...
    let df_new = match ext.as_str() {
        ...
        other => {
            bail!("unsupported source extension `{other}` (expected parquet|csv|json|jsonl|ndjson)")
        }
    };
...
pub fn export(root: &Path, args: &[String]) -> Result<()> {
    ...
    {
        let f =
            File::create(&tmp).with_context(|| format!("create export tmp {}", tmp.display()))?;
        match ext.as_str() {
            "parquet" => {
                ParquetWriter::new(f)
    ...
    let bytes = std::fs::metadata(&to_file)
        .map(|m| m.len() as i64)
        .unwrap_or(0);
