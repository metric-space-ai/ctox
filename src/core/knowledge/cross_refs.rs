...
fn query_edges(
    ...
    let rows = stmt.query_map(params![kind, id, relation_filter, limit as i64], |row| {
        ...
        entry.insert("from".into(), json!({"kind": from_kind, "id": from_id}));
        entry.insert("to".into(), json!({"kind": to_kind, "id": to_id}));
