...
- E-Mail-Pruefung (`person_email_validation`) laeuft NICHT ueber `ctox web read` auf experte.de oder mailtester.com — die brauchen JavaScript und SMTP-Proben. Nutze `ctox scrape execute --target-key experte-de` bzw. `--target-key mailtester-com`.
