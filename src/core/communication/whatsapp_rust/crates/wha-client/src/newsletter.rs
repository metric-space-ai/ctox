...
pub fn parse_newsletter_metadata(node: &Node) -> Result<NewsletterMetadata, ClientError> {
    ...
    let verification = meta
        ...
        .and_then(|c| {
            c.get_attr_str("state")
                .map(NewsletterVerificationState::from_str)
        });
...
mod tests {
    ...
    fn build_follow_iq_has_correct_attrs() {
        ...
        assert_eq!(
            ...
            Some(MUTATION_FOLLOW_NEWSLETTER)
        );
    ...
    fn build_create_newsletter_iq_carries_name_and_description() {
        ...
        assert_eq!(
            query.get_attr_str("query_id"),
            Some(MUTATION_CREATE_NEWSLETTER)
        );
    ...
    fn parse_newsletter_metadata_extracts_name_and_owner() {
        ...
        let mk_text = |tag: &str, text: &str| {
            Node::new(
                tag,
                Attrs::new(),
                Some(Value::Bytes(text.as_bytes().to_vec())),
            )
        };
        ...
        assert_eq!(md.verification, Some(NewsletterVerificationState::Verified));
        assert_eq!(
            md.picture_url.as_deref(),
    ...
    fn send_newsletter_text_builds_plaintext_envelope() {
        ...
        for k in kids {
            ...
            assert_ne!(
                k.tag, "participants",
                "channel must not carry <participants>"
            );
        }
    }
    ...
    fn send_newsletter_reaction_includes_meta_server_id() {
        ...
        let node =
            build_newsletter_message_node("REACT-MSG-ID-000", &channel, &msg, Some(server_id))
                .expect("build node");
    ...
    fn send_newsletter_message_with_media_uses_media_type() {
        ...
        let decoded: Message = prost::Message::decode(pt_bytes).expect("image plaintext decodes");
