...
pub async fn fetch_user_devices(client: &Client, jids: &[Jid]) -> Result<Vec<Jid>, ClientError> {
...
pub async fn persist_lid_mappings_from_usync(client: &Client, resp: &Node) {
    ...
    for (lid, pn) in pairs {
        if let Err(e) = client
            ...
            .await
        {
            tracing::warn!("usync: failed to persist LID↔PN mapping ({lid} ↔ {pn}): {e}");
        }
    }
}
...
mod tests {
    ...
    async fn persist_lid_mappings_from_usync_writes_pairs_to_store() {
        ...
        assert_eq!(
            client.device.lids.get_pn_for_lid(&alice_lid).await.unwrap(),
            Some(alice_pn.clone())
        );
        assert_eq!(
