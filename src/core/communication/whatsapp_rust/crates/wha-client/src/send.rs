...
pub async fn send_message(
    ...
    prost::Message::encode(&msg, &mut plaintext).map_err(|e| ClientError::Proto(e.to_string()))?;
    ...
    let ack = match tokio::time::timeout(Duration::from_secs(DEFAULT_REQUEST_TIMEOUT.as_secs()), rx)
        .await
    {
        Ok(Ok(node)) => node,
