...
pub async fn upload_pre_keys(client: &Client) -> Result<(), ClientError> {
    ...
    client
        .device
        .pre_keys
        .mark_pre_keys_uploaded(highest_id)
        .await?;
    Ok(())
}
...
mod tests {
    ...
    fn build_upload_iq_has_required_children() {
        ...
        let signed = fixed_prekey(99)
            .signed_by(&KeyPair::from_private([5u8; 32]))
            .unwrap();
        ...
        assert!(
            node.child_by_tag(&["registration"]).is_some(),
            "registration missing"
        );
        assert!(node.child_by_tag(&["type"]).is_some(), "<type> missing");
        assert!(
            node.child_by_tag(&["identity"]).is_some(),
            "identity missing"
        );
    ...
    fn prekey_id_is_3_byte_big_endian() {
        ...
        assert_eq!(id_node.content.as_bytes(), Some(&[0xAB, 0xCD, 0xEF][..]),);
    }

    #[test]
    fn registration_id_is_4_byte_big_endian() {
        assert_eq!(
            registration_id_to_bytes(0x01020304),
            [0x01, 0x02, 0x03, 0x04]
        );
        ...
        assert_eq!(reg.content.as_bytes(), Some(&[0xDE, 0xAD, 0xBE, 0xEF][..]),);
    }
    ...
    async fn refresh_does_nothing_when_count_above_min() {
        ...
        let pks = store.get_or_gen_pre_keys(n as u32).await.unwrap();
