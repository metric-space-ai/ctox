...
pub async fn handle_notification(client: &Client, node: &Node) -> Result<(), ClientError> {
    ...
    let own_jid = client.device.jid().cloned().unwrap_or_else(Jid::default);
...
fn handle_newsletter(client: &Client, node: &Node) {
    let jid = node
        ...
        .cloned()
        .unwrap_or_default();
    client.dispatch_event(Event::Newsletter {
        jid,
        raw: node.clone(),
    });
}
...
fn handle_status(client: &Client, node: &Node) {
    ...
    client.dispatch_event(Event::UserAbout {
        jid,
        status,
        timestamp,
    });
...
mod tests {
    ...
    fn parse_notification_kind_dispatches_correctly() {
        ...
        let cases = [
            ...
            (
                ...
                NotificationKind::LinkCodeCompanionReg,
            ),
            ("newsletter", NotificationKind::Newsletter),
            ("mex", NotificationKind::Mex),
            ("status", NotificationKind::Status),
            (
                "some_unknown_thing_we_have_never_seen",
                NotificationKind::Other,
            ),
        ];
    ...
    async fn picture_notification_emits_picture_changed_event() {
        ...
        notif
            .attrs
            .insert("t".into(), Value::String("1700000000".into()));
    ...
    async fn status_notification_emits_user_about_event() {
        ...
        notif
            .attrs
            .insert("t".into(), Value::String("1234567890".into()));
        let set = Node::new(
            "set",
            Attrs::new(),
