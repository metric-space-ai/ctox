...
mod tests {
    ...
    fn encrypt_media_is_deterministic() {
        ...
        assert_eq!(
            blob_a, blob_b,
            "encryption is not deterministic for fixed inputs"
        );
    ...
    fn parse_media_retry_extracts_encrypt_payload() {
        ...
        let notif = Node::new(
            "notification",
            attrs,
            Some(Value::Nodes(vec![rmr, encrypt])),
        );
    ...
    fn parse_media_retry_extracts_error_branch() {
        ...
        let notif = Node::new(
            "notification",
            attrs,
            Some(Value::Nodes(vec![rmr, err_node])),
        );
    ...
    async fn upload_blob_calls_http_with_correct_url() {
        ...
        let mc = MediaConn {
            ...
            hosts: vec![MediaConnHost {
                hostname: "mmg.example.com".into(),
            }],
        };
        let blob = vec![0u8; 32];
        let hash = [7u8; 32];
        let result = upload_blob(&mock, &mc, MediaType::Image, &blob, &hash, false)
            .await
            .unwrap();
        ...
        assert!(
            url_used.starts_with("https://mmg.example.com/mms/image/"),
            "got {url_used}"
        );
        assert!(
            url_used.contains("auth=AUTH%2BPADDING%3D%3D"),
            "auth not URL-encoded: {url_used}"
        );
    }
