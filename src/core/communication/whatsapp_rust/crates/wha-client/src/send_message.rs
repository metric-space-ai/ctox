...
use wha_proto::e2e::{ContextInfo, ExtendedTextMessage, Message, ProtocolMessage, ReactionMessage};
...
pub async fn send_text(client: &Client, to: &Jid, body: &str) -> Result<String, ClientError> {
...
pub async fn send_document(
    ...
    let doc = crate::upload::upload_document(client, document_bytes, mime_type, file_name).await?;
    let proto_msg = Message {
...
pub async fn send_revoke(
    ...
    let own_jid = client.device.id.clone().ok_or(ClientError::NotLoggedIn)?;
...
pub(crate) fn parse_user_node_to_bundle(
    to: &Jid,
    user_node: &Node,
...
impl Client {
    ...
    pub async fn send_sticker(&self, to: &Jid, webp_bytes: &[u8]) -> Result<String, ClientError> {
...
mod tests {
    ...
    fn build_text_message_with_participants_shape() {
        ...
        let msg =
            build_text_message_with_participants("ABCDEF1234567890", &to, vec![to_d1, to_d2], true);
        ...
        let msg2 = build_text_message_with_participants(
            "ABCDEF1234567890",
            &to,
            vec![build_to_node(&d1, build_enc_node("msg", vec![1]))],
            false,
        );
