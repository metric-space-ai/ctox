...
pub async fn handle_encrypted_message(
    ...
    let timestamp = node
        ...
        .unwrap_or(0);
    let participant = node.get_attr_jid("participant").cloned().or_else(|| {
        node.get_attr_str("participant")
            .and_then(|s| s.parse::<Jid>().ok())
    });
    let recipient = node.get_attr_jid("recipient").cloned().or_else(|| {
    ...
    for enc in enc_children {
        ...
        let result = match enc_type.as_str() {
            ...
            "skmsg" => {
                crate::recv_group::handle_group_message(client, &from, &sender_jid, ciphertext)
                    .await
                    .and_then(|raw| unpad_message(&raw))
            }
            other => Err(ClientError::Other(format!("unknown <enc> type `{other}`"))),
        };
    ...
    let our_one_time = match pkm.pre_key_id {
        Some(id) if id != 0 => {
            let pk = client
                ...
                .get_pre_key(id)
                .await?
                .ok_or_else(|| ClientError::Decrypt(format!("missing one-time pre-key id {id}")))?;
            Some(pk.key_pair)
        }
        _ => None,
    };
...
pub fn classify_decrypted_message(
    ...
    if let Some(pm) = msg.protocol_message.as_ref() {
        if pm.r#type == Some(wha_proto::e2e::protocol_message::Type::Revoke as i32) {
    ...
    if let Some(react) = msg.reaction_message.as_ref() {
        if let Some(key) = react.key.as_ref() {
            ...
            let target_sender = key
                ...
                .as_ref()
                .and_then(|s| s.parse::<Jid>().ok())
                .or_else(|| key.remote_jid.as_ref().and_then(|s| s.parse::<Jid>().ok()))
                .unwrap_or_else(|| dec.from.clone());
...
mod tests {
    ...
    async fn decrypt_pkmsg_round_trip_with_x3dh() {
        ...
        let saved = bob_client.device.sessions.get_session(&addr).await.unwrap();
    ...
    fn classify_reaction_emits_event_reaction() {
        ...
        blank.reaction_message.as_mut().unwrap().text = Some("".into());
    ...
    fn classify_protocol_revoke_emits_event_message_revoke() {
        ...
        let other_pm = wha_proto::e2e::Message {
            protocol_message: Some(Box::new(wha_proto::e2e::ProtocolMessage {
                ...
                r#type: Some(wha_proto::e2e::protocol_message::Type::EphemeralSetting as i32),
                ..Default::default()
            })),
            ..Default::default()
        };
    ...
    fn classify_extended_text_with_quoted_emits_message_with_quoted() {
        ...
        let inner = wha_proto::e2e::Message {
            extended_text_message: Some(Box::new(wha_proto::e2e::ExtendedTextMessage {
