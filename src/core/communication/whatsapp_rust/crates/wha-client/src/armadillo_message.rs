...
pub enum ArmadilloError {
    ...
    #[error("unsupported MessageApplication SubProtocol version {got} (expected {expected})")]
    ...
    match content {
        ...
        AppPayloadContent::SubProtocol(sub) => {
            ...
            match inner {
                ...
                AppSubProtocol::Voip(_) => Err(ArmadilloError::UnsupportedSubProtocol("voip")),
...
pub async fn fetch_newsletter_keys(
    ...
    let resp = client
        .send_iq(build_fetch_newsletter_keys_iq(channel))
        .await?;
...
mod tests {
    ...
    fn decode_consumer_application_subprotocol() {
        ...
        let bytes = encode_msg_transport(&app, &SignalMessageMetadata::default(), 0).unwrap();
    ...
    fn parse_newsletter_keys_extracts_32_bytes() {
        ...
        let nl = Node::new("newsletter", Attrs::new(), Some(Value::Nodes(vec![keys])));
