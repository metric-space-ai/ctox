...
impl Client {
    ...
    pub async fn send_fb_message(
        ...
        match tokio::time::timeout(Duration::from_secs(DEFAULT_REQUEST_TIMEOUT.as_secs()), rx).await
        {
    ...
    pub async fn send_fb_message_raw(
        ...
        let ack =
            match tokio::time::timeout(Duration::from_secs(DEFAULT_REQUEST_TIMEOUT.as_secs()), rx)
                .await
            {
                Ok(Ok(node)) => node,
...
mod tests {
    ...
    fn is_fb_recipient_classifies_msgr_jids() {
        ...
        assert!(
            is_fb_recipient(&msgr),
            "msgr server should route to FB path"
        );
    ...
    fn wrap_in_transport_round_trips_application_bytes() {
        ...
        let decoded = <MessageTransport as ProstMessage>::decode(bytes.as_slice()).expect("decode");
