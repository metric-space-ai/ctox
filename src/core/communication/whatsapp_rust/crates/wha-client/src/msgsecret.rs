...
pub fn encrypt_msg_secret_raw(
    ...
    let ciphertext =
        gcm_encrypt(&key, &iv, plaintext, aad).map_err(|e| ClientError::Crypto(e.to_string()))?;
    Ok(EncryptedMsgSecret {
        iv,
        ciphertext,
        key: None,
    })
...
mod tests {
    ...
    fn aad_mismatch_decrypt_fails() {
        ...
        let sealed =
            encrypt_msg_secret_raw(&master, MessageSecretUseCase::Reaction, pt, &aad_correct)
                .unwrap();
    ...
    fn info_strings_match_whatsmeow() {
        ...
        assert_eq!(
            MessageSecretUseCase::PollVote.as_info_str(),
            ENC_SECRET_POLL_VOTE
        );
        assert_eq!(
            MessageSecretUseCase::Reaction.as_info_str(),
            ENC_SECRET_REACTION
        );
        assert_eq!(
            MessageSecretUseCase::Comment.as_info_str(),
            ENC_SECRET_COMMENT
        );
        assert_eq!(
            MessageSecretUseCase::ReportToken.as_info_str(),
            ENC_SECRET_REPORT_TOKEN
        );
        assert_eq!(
            MessageSecretUseCase::EventResponse.as_info_str(),
            ENC_SECRET_EVENT_RESPONSE
        );
        assert_eq!(
            MessageSecretUseCase::Edit.as_info_str(),
            ENC_SECRET_EVENT_EDIT
        );
        assert_eq!(
            MessageSecretUseCase::BotMessage.as_info_str(),
            ENC_SECRET_BOT_MSG
        );
    }
