    ...
    fn into_attrs(self) -> Attrs {
        ...
        match self {
            ...
            PushToken::Apns {
                ...
                let pkey = base64::engine::general_purpose::URL_SAFE_NO_PAD.encode(msg_id_enc_key);
                a.insert("id".into(), Value::String(token));
...
pub(crate) fn build_user_info_iq(jids: &[Jid], sid: &str) -> InfoQuery {
...
fn parse_device_list(user: &Jid, devices_node: Option<&Node>) -> Vec<Jid> {
    let Some(node) = devices_node else {
        return Vec::new();
    };
    if node.tag != "devices" {
        return Vec::new();
...
pub(crate) fn build_set_privacy_iq(
...
impl Client {
    ...
    pub async fn get_user_info(&self, jids: &[Jid]) -> Result<HashMap<Jid, UserInfo>, ClientError> {
    ...
    pub async fn get_privacy_settings(&self) -> Result<PrivacySettings, ClientError> {
        ...
        let privacy_node = resp
            ...
            .ok_or_else(|| {
                ClientError::Malformed("privacy settings response missing <privacy> child".into())
            })?;
...
mod tests {
    ...
    fn build_user_info_iq_shape() {
        ...
        assert_eq!(
            node.get_attr_jid("to").unwrap().server,
            server::DEFAULT_USER
        );
    ...
    fn parse_user_info_list_basic() {
        ...
        let mut devices_attrs = Attrs::new();
        devices_attrs.insert("version".into(), Value::String("2".into()));
        let devices = Node::new("devices", devices_attrs, Some(Value::Nodes(vec![dev_list])));
    ...
    fn build_set_privacy_iq_shape() {
        let q = build_set_privacy_iq(PrivacySettingName::LastSeen, &PrivacySettingValue::Contacts);
    ...
    fn parse_profile_picture_extracts_metadata() {
        ...
        a.insert("direct_path".into(), Value::String("/v/abc".into()));
