...
pub async fn send_retry_receipt(
    ...
    let node = build_retry_receipt_node(
        client,
        original_message_id,
        sender,
        sender_device_jid,
...
pub fn parse_retry_receipt(node: &Node) -> Result<RetryReceipt, ClientError> {
    ...
    let retry_child = node
        ...
        .iter()
        .find(|c| c.tag == "retry")
        .ok_or_else(|| ClientError::Malformed("retry receipt missing <retry> child".into()))?;
    ...
    let registration_id = node
        ...
        .and_then(|reg| match &reg.content {
            Value::Bytes(b) if b.len() == 4 => Some(u32::from_be_bytes([b[0], b[1], b[2], b[3]])),
            _ => None,
        });
...
pub async fn handle_retry_receipt(client: &Client, receipt: &Node) -> Result<(), ClientError> {
    ...
    let env = match bundle {
        ...
        None => {
            ...
            let nodes =
                crate::send_encrypt::encrypt_for_recipient(client, &recipient, &plaintext).await?;
            let enc = nodes.into_iter().next().ok_or_else(|| {
                ClientError::Other("encrypt_for_recipient returned no <enc>".into())
            })?;
            ...
            let ciphertext = match enc.content {
                ...
                _ => {
                    return Err(ClientError::Other(
                        "encrypt_for_recipient: <enc> not bytes".into(),
                    ))
                }
            };
            if enc_type == "pkmsg" {
    ...
    enc_attrs.insert(
        "count".into(),
        Value::String(parsed.retry_count.to_string()),
    );
...
mod tests {
    ...
    fn build_retry_receipt(
        ...
        if let Some(reg) = registration_id {
            ...
            let keys = Node::new("keys", Attrs::new(), Some(Value::Nodes(vec![registration])));
            children.push(keys);
        }
    ...
    async fn build_retry_receipt_node_shape_first_retry() {
        ...
        let node = build_retry_receipt_node(&cli, "MSG-A", &sender, &device_jid, 1, 1714521600)
            .await
            .expect("build");
    ...
    async fn build_retry_receipt_node_shape_includes_keys_on_count_gt_one() {
        ...
        let sender = Jid::new("12345", server::DEFAULT_USER);
        let node = build_retry_receipt_node(&cli, "MSG-B", &sender, &sender, 2, 1714521700)
            .await
            .expect("build");
    ...
    async fn handle_retry_receipt_resolves_lid_to_pn() {
        ...
        let retry = Node::new("retry", retry_attrs, None);
        let receipt = Node::new("receipt", outer_attrs, Some(Value::Nodes(vec![retry])));
