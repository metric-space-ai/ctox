...
const CHECK_UPDATE_URL: &str = "https://web.whatsapp.com/check-update?version=0.0.0&platform=web";
...
pub fn current() -> WAVersionContainer {
    *RUNTIME_VERSION.get().unwrap_or(&crate::payload::WA_VERSION)
}
...
pub async fn check_for_update_async(client: Option<&Client>) -> Result<UpdateStatus, ClientError> {
    let current = current();
    let url = format!(
        "https://web.whatsapp.com/check-update?version={}&platform=web",
...
mod tests {
    ...
    async fn is_below_hard_dispatches_client_outdated_event() {
        ...
        let body =
            r#"{"isBroken":false,"isBelowSoft":true,"isBelowHard":true,"currentVersion":"99.0.0"}"#;
