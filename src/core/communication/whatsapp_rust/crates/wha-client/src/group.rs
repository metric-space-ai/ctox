        ...
        match child.tag.as_str() {
            ...
            "description" => {
                ...
                if let Some(body) = child.children().iter().find(|c| c.tag == "body") {
                    ...
                    info.topic_set_by =
                        cag.optional_jid("participant").cloned().unwrap_or_default();
                }
            }
            ...
            "ephemeral" => {
                ...
                let mut cag = child.attr_getter();
                info.disappearing_timer = cag.optional_u64("expiration").unwrap_or(0) as u32;
            }
            _ => {}
        }
    }
...
impl Client {
    ...
    pub async fn set_group_locked(&self, group: &Jid, locked: bool) -> Result<(), ClientError> {
        let resp = self
            .send_iq(build_set_group_locked_iq(group, locked))
            .await?;
        if let Some(err) = iq_error_from_response(&resp) {
            return Err(err);
        }
...
mod tests {
    ...
    fn build_create_group_iq_has_xmlns_and_subject() {
        ...
        assert_eq!(
            node.get_attr_jid("to").map(|j| j.server.as_str()),
            Some("g.us")
        );
    ...
    fn build_update_participants_add_iq() {
        ...
        assert_eq!(node.get_attr_str("type"), Some("set"));
        assert_eq!(
            node.get_attr_jid("to").map(|j| j.user.as_str()),
            Some("120-456")
        );
        assert_eq!(
            node.get_attr_jid("to").map(|j| j.server.as_str()),
    ...
    fn parse_invite_link_response_extracts_full_url() {
        ...
        let iq = Node::new("iq", Attrs::new(), Some(Value::Nodes(vec![invite])));
    ...
    fn build_set_group_announce_iq_toggles_tag() {
        ...
        assert_eq!(
            on.get_attr_jid("to").map(|j| j.user.as_str()),
            Some("120-456")
        );
    ...
    fn build_get_joined_groups_iq_has_participating_with_two_children() {
        ...
        assert_eq!(
            q.get_attr_jid("to").map(|j| j.server.as_str()),
            Some("g.us")
        );
        let participating = q
