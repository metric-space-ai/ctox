...
impl<'a> DangerousInternalClient<'a> {
    ...
    pub fn get_unified_session_id(&self) -> String {
        let now_ms = SystemTime::now()
            ...
            .unwrap_or(0);
        let unified = now_ms
            .wrapping_add(self.0.server_time_offset_ms())
            .wrapping_add(UNIFIED_OFFSET_MS);
...
mod tests {
    ...
    async fn inspector_surfaces_session_addresses_prekey_count_and_app_state_versions() {
        use wha_store::{AppStateMutationMacStore, PreKeyStore, SessionStore};
        let store = Arc::new(MemoryStore::new());
        ...
        assert_eq!(
            addrs,
            vec![
                "alice@s.whatsapp.net.1".to_string(),
                "bob@s.whatsapp.net.0".to_string()
            ]
        );
