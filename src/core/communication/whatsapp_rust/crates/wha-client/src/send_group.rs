...
pub async fn encrypt_for_group_with_participants(
    ...
    if let Some(skdm_bytes) = skdm_bytes_opt {
        ...
        let skdm_msg = Message {
            sender_key_distribution_message: Some(wha_proto::e2e::SenderKeyDistributionMessage {
        ...
        for participant in participant_devices {
            ...
            match crate::send_encrypt::encrypt_for_recipient(client, participant, &skdm_proto_bytes)
                .await
            {
                Ok(nodes) => {
                    for node in nodes {
...
pub async fn send_group_message(
    ...
    let result =
        encrypt_for_group_with_participants(client, group, &plaintext, &participant_devices)
            .await?;
...
mod tests {
    ...
    fn build_group_message_envelope_carries_required_attrs_and_children() {
        ...
        let skmsg_children: Vec<&Node> = env.children().iter().filter(|c| c.tag == "enc").collect();
