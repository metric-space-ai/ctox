    ...
    while has_more {
        ...
        if let Some(patches_node) = collection.child_by_tag(&["patches"]) {
            for patch_node in patches_node.children_by_tag("patch") {
                ...
                let mutations = decode_patch(&patch, name, &mut state, &keys, true, |im| {
...
pub async fn send_app_state_mutations(
    ...
    let raw_key = key_store
        ...
        .map_err(ClientError::Store)?
        .ok_or_else(|| {
            ClientError::Other(format!("missing app-state sync key {}", hex_short(&key_id)))
        })?;
    let keys = expand_app_state_keys(&raw_key.data)?;
    ...
    patch
        .encode(&mut patch_bytes)
        .map_err(|e| ClientError::Proto(e.to_string()))?;

    // Build the wire IQ.
    let mut patch_attrs = Attrs::new();
    let patch_node = Node::new(
        "patch",
        patch_attrs.clone(),
        Some(Value::Bytes(patch_bytes)),
    );
    patch_attrs.clear();
    ...
    for (input, m) in mutations.iter().zip(patch.mutations.iter()) {
        ...
        match input.operation {
            SyncdOperation::Set => {
                store
                    .put_app_state_mutation_mac(name.as_str(), state.version, &index_mac, value_mac)
                    .await
                    .map_err(ClientError::Store)?;
            }
            SyncdOperation::Remove => {
...
mod tests {
    ...
    async fn prefetch_prev_macs_dedups_within_patch() {
        use wha_proto::server_sync::{KeyId, SyncdIndex, SyncdMutation, SyncdRecord, SyncdValue};
    ...
    async fn send_app_state_mutations_errors_without_sync_key() {
        ...
        match r {
            ...
            other => panic!("expected Other('no app-state sync key…'), got {other:?}"),
        }
    }
