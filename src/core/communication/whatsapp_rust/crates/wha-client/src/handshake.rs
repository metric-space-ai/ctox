...
use wha_socket::{
    NoiseHandshake, NoiseSocket, FRAME_LENGTH_SIZE, NOISE_START_PATTERN, WA_CONN_HEADER,
};
...
pub fn finish_handshake(
    ...
    if static_decrypted.len() != 32 {
        return Err(ClientError::Handshake(
            "server static is not 32 bytes".into(),
        ));
    }
...
pub fn verify_server_cert(cert_decrypted: &[u8], server_static: &[u8]) -> Result<(), ClientError> {
    ...
    let leaf = chain
        .leaf
        .ok_or_else(|| ClientError::Handshake("missing leaf cert".into()))?;
    ...
    if intermediate_sig.len() != 64 {
        return Err(ClientError::Handshake(
            "intermediate sig not 64 bytes".into(),
        ));
    }
    if leaf_sig.len() != 64 {
    ...
    if intermediate_decoded.issuer_serial.unwrap_or(0) != WA_CERT_ISSUER_SERIAL {
        return Err(ClientError::Handshake(
            "unexpected intermediate issuer serial".into(),
        ));
    }
    ...
    if intermediate_key.len() != 32 {
        return Err(ClientError::Handshake(
            "intermediate key not 32 bytes".into(),
        ));
    }
    let mut ikey = [0u8; 32];
    ...
    if leaf_decoded.issuer_serial.unwrap_or(0) != intermediate_decoded.serial.unwrap_or(0) {
        return Err(ClientError::Handshake(
            "leaf/intermediate serial mismatch".into(),
        ));
    }
    ...
    if leaf_key.as_slice() != server_static {
        return Err(ClientError::Handshake(
            "cert key doesn't match decrypted static".into(),
        ));
    }
    Ok(())
}
