...
pub enum CallEvent {
    ...
    Reject { meta: BasicCallMeta, data: Node },
    ...
    RelayLatency { meta: BasicCallMeta, data: Node },
...
impl Client {
    ...
    pub async fn reject_call(&self, call_creator: &Jid, call_id: &str) -> Result<(), ClientError> {
        let own = self
            .device
            .id
            .as_ref()
...
mod tests {
    ...
    async fn handle_call_stanza_dispatches_reject_event() {
        ...
        let call = mk_call_node(
            "reject",
            child_attrs("CALL-NO", "100", server::DEFAULT_USER),
        );
        let _ = handle_call_stanza(&cli, &call).await;
