...
pub fn build_mark_read_receipt(
    ...
    attrs.insert(
        "type".into(),
        Value::String(receipt_type.as_str().to_owned()),
    );
...
impl Client {
    ...
    pub async fn send_message_receipt(
        ...
        let node =
            build_message_receipt_node(id, to, participant, recipient, &receipt_type, timestamp);
        self.send_node(&node).await
    }
...
pub async fn mark_read(
    ...
    if message_ids.is_empty() {
        return Err(ClientError::Malformed("no message IDs specified".into()));
    }
...
pub async fn send_played_receipt(
    ...
    let node = build_mark_read_receipt(&ids, timestamp, chat, Some(sender), &ReceiptType::Played);
    client.send_node(&node).await
}
...
mod tests {
    ...
    fn chat_presence_composing_audio_carries_media_attr() {
        ...
        let n =
            build_chat_presence_node(&me, &to, ChatPresence::Composing, ChatPresenceMedia::Audio);
    ...
    fn parse_receipt_basic_read() {
        ...
        attrs.insert(
            "from".into(),
            Value::Jid(Jid::new("123", server::DEFAULT_USER)),
        );
    ...
    fn build_message_receipt_carries_expected_attrs() {
        ...
        let n2 = build_message_receipt_node("X", &to, None, None, &ReceiptType::Delivered, None);
