...
static TOP_LEVEL: &[ReportingField] = &[
    ReportingField {
        field_number: 1,
        inner: InnerConfig::Scalar,
    },
    ReportingField {
        field_number: 3,
        inner: InnerConfig::Children(&[
            ReportingField {
                field_number: 2,
                inner: InnerConfig::Scalar,
            },
    ...
    ReportingField {
        field_number: 37,
        inner: InnerConfig::Children(&[ReportingField {
            field_number: 1,
            inner: InnerConfig::TopLevel,
        }]),
    },
        ...
        match wire_type {
            ...
            WIRE_BYTES => {
                ...
                if let Some(child_cfg) = recurse_into {
                    ...
                    if !sub.is_empty() {
                        let mut buf = Vec::with_capacity(tag_len + sub.len() + 5);
