...
async fn emit_qr_codes(sender: tokio::sync::mpsc::UnboundedSender<Event>, codes: Vec<String>) {
...
pub fn parse_pair_success(
    ...
    let lid = device_node.get_attr_jid("lid").cloned().unwrap_or_default();
    ...
    let is_hosted = container.account_type == Some(adv::AdvEncryptionType::Hosted as i32);
...
pub(crate) fn build_pair_success_ack(result: &PairResult) -> Node {
    ...
    device_identity_attrs.insert(
        "key-index".into(),
        Value::String(result.key_index.to_string()),
    );
