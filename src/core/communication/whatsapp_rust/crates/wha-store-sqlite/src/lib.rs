...
impl SqliteStore {
    ...
    pub async fn load_device(self: &Arc<Self>) -> Result<Option<Device>, StoreError> {
        ...
        let raw: Option<Vec<u8>> = self
            .run(|c| {
                c.query_row("SELECT blob FROM device ORDER BY jid LIMIT 1", [], |row| {
                    row.get::<_, Vec<u8>>(0)
                })
                .optional()
            })
...
impl PrivacyTokenStore for SqliteStore {
    ...
    async fn get_privacy_token(&self, jid: &Jid) -> Result<Option<(Vec<u8>, i64)>, StoreError> {
        let key = jid.to_string();
        self.run(move |c| {
            c.query_row(
...
impl NewsletterKeyStore for SqliteStore {
    async fn put_newsletter_key(&self, channel: &Jid, key: [u8; 32]) -> Result<(), StoreError> {
...
impl InspectStore for SqliteStore {
    ...
    async fn count_pre_keys(&self) -> Result<usize, StoreError> {
        self.run(|c| {
            let count: i64 = c.query_row("SELECT COUNT(1) FROM whatsmeow_pre_keys", [], |row| {
                row.get(0)
            })?;
            Ok(count as usize)
        })
...
impl Container {
    ...
    pub async fn get_first_device(&self) -> Result<Option<Arc<SqliteStore>>, StoreError> {
        let any: Option<i64> = self
            .store
            .run(|c| {
                c.query_row("SELECT 1 FROM device LIMIT 1", [], |row| {
                    row.get::<_, i64>(0)
                })
                .optional()
            })
...
mod tests {
    ...
    async fn identity_round_trip() {
        ...
        // TOFU on missing.
        assert!(s
            .is_trusted_identity("alice@s.whatsapp.net", key)
            .await
            .unwrap());
        ...
        assert!(s
            ...
            .unwrap());
        assert!(!s
            .is_trusted_identity("alice@s.whatsapp.net", [4u8; 32])
            .await
            .unwrap());

        // Overwrite.
        let new_key = [9u8; 32];
        s.put_identity("alice@s.whatsapp.net", new_key)
            .await
            .unwrap();
        assert!(s
            .is_trusted_identity("alice@s.whatsapp.net", new_key)
            .await
            .unwrap());
        ...
        assert!(s
            .is_trusted_identity("alice@s.whatsapp.net", [0u8; 32])
            .await
            .unwrap());
    }
    ...
    async fn save_then_load_round_trips_device_fields() {
        ...
        assert_eq!(
            loaded.signed_pre_key.signature,
        ...
        let fetched = store
            .get_pre_key(device.signed_pre_key.key_id)
            .await
            .unwrap();
    ...
    async fn newsletter_key_round_trip_sqlite() {
        ...
        assert_eq!(s.get_newsletter_key(&channel).await.unwrap(), Some(key1));

        // Rotate — second put overwrites the row.
        let key2 = [0x99u8; 32];
        s.put_newsletter_key(&channel, key2).await.unwrap();
        assert_eq!(s.get_newsletter_key(&channel).await.unwrap(), Some(key2));
    ...
    async fn container_open_and_new_device_round_trip() {
        ...
        assert!(
            c.list_devices().await.unwrap().is_empty(),
    ...
    async fn container_get_device_by_jid_after_pairing() {
        ...
        assert!(listed
            .iter()
            .any(|b| b.id.as_deref() == Some("1234.5:7@s.whatsapp.net")));
    }
