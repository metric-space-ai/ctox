...
impl SessionState {
    ...
    pub fn dh_ratchet_step<R: rand::RngCore>(
        ...
        let (root_after_recv, recv_chain) = self
            .root_key
            .create_chain(&their_new_ratchet_pub, &our_old_ratchet);
        ...
        let previous_counter = self.sender_chain_key.as_ref().map(|c| c.index).unwrap_or(0);
...
mod tests {
    ...
    fn bob_state_has_no_sender_chain_initially() {
        ...
        assert!(
            !bob.has_sender_chain(),
            "bob has no sender chain until first ratchet"
        );
    ...
    fn receiver_chain_ring_caps_at_5() {
        ...
        // Newest stays.
        assert!(state
            .find_receiver_chain(&peer_pubs[MAX_RECEIVER_CHAINS])
            .is_some());
        // Oldest fell off.
