...
impl SessionCipher {
    ...
    pub fn decrypt(
        ...
        if state.find_receiver_chain(&msg.sender_ratchet_key).is_none() {
            ...
            if let Some((prev_pub, prev_chain)) =
                state.receiver_chains.last().map(|(p, c)| (*p, c.clone()))
            {
...
mod cache_ext {
    ...
    pub trait SkippedKeyCacheExt {
        fn insert_for_test(&mut self, peer_ratchet_pub: [u8; 32], counter: u32, keys: MessageKeys);
    }
...
mod tests {
    ...
    fn pending_prekey_produces_pkmsg_then_clears_on_decrypt() {
        ...
        let plain = SessionCipher::decrypt(&mut bob, env.as_bytes()).expect("decrypt");
