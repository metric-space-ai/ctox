...
fn decode_body(buf: &[u8]) -> Result<SenderKeyMessageBody, SignalProtocolError> {
    ...
    Ok(SenderKeyMessageBody {
        ...
        iteration: iteration.ok_or(SignalProtocolError::InvalidMessage(
            "missing senderkey iteration",
        ))?,
        ciphertext: ciphertext.ok_or(SignalProtocolError::InvalidMessage(
            "missing senderkey ciphertext",
        ))?,
    })
}
...
impl SenderKeyMessage {
    ...
    pub fn decrypt(
        ...
        let signing_pub =
            {
                let state = record.get_sender_key_state(body.id).ok_or(
                    SignalProtocolError::InvalidMessage("no sender state for key id"),
                )?;
...
fn walk_to_iteration(
    ...
    if current > target_iteration {
        ...
        return state.remove_sender_message_key(target_iteration).ok_or(
            SignalProtocolError::DuplicateMessage {
                chain: current,
