...
impl SessionStore for MemoryStore {
    ...
    async fn delete_all_sessions(&self, phone: &str) -> Result<(), StoreError> {
        self.inner
            .write()
            .sessions
            .retain(|k, _| !k.starts_with(phone));
        Ok(())
    }
...
impl PreKeyStore for MemoryStore {
    ...
    async fn uploaded_pre_key_count(&self) -> Result<usize, StoreError> {
        ...
        Ok(r.pre_keys
            .values()
            .filter(|pk| pk.key_id <= r.pre_keys_uploaded_to)
            .count())
    }
}
...
impl SenderKeyStore for MemoryStore {
    async fn put_sender_key(
        ...
        self.inner
            .write()
            .sender_keys
            .insert((group.to_owned(), user.to_owned()), session);
        Ok(())
    }
    async fn get_sender_key(&self, group: &str, user: &str) -> Result<Option<Vec<u8>>, StoreError> {
        Ok(self
            ...
            .read()
            .sender_keys
            .get(&(group.to_owned(), user.to_owned()))
            .cloned())
    }
}
...
impl InspectStore for MemoryStore {
    ...
    async fn list_app_state_versions(&self) -> Result<HashMap<String, u64>, StoreError> {
        Ok(self
            .inner
            .read()
            .app_state_versions
...
mod tests {
    ...
    async fn prekey_generation_is_unique() {
        ...
        assert_eq!(
            store.get_pre_key(a.key_id).await.unwrap().unwrap().key_id,
            a.key_id
        );
    }
