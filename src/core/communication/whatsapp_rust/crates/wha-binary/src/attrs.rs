...
impl<'a> AttrUtility<'a> {
    ...
    pub fn optional_string(&mut self, key: &str) -> Option<&'a str> {
        match self.attrs.get(key) {
            ...
            Some(_) => {
                self.errors
                    .push(BinaryError::Attr(format!("attr `{key}` not a string")));
                None
            }
        }
    }
    ...
    pub fn string(&mut self, key: &str) -> &'a str {
        match self.attrs.get(key) {
            ...
            None => {
                self.errors
                    .push(BinaryError::Attr(format!("missing required attr `{key}`")));
                ""
            }
            Some(_) => {
                self.errors
    ...
    pub fn optional_jid(&mut self, key: &str) -> Option<&'a Jid> {
        match self.attrs.get(key) {
            ...
            Some(_) => {
                self.errors
                    .push(BinaryError::Attr(format!("attr `{key}` not a JID")));
                None
            }
        }
    }
    ...
    pub fn jid(&mut self, key: &str) -> Jid {
        match self.attrs.get(key) {
            ...
            None => {
                self.errors.push(BinaryError::Attr(format!(
                    "missing required JID attr `{key}`"
                )));
                Jid::default()
            }
            Some(_) => {
    ...
    pub fn optional_i64(&mut self, key: &str) -> Option<i64> {
        ...
        match s.parse::<i64>() {
            ...
            Err(e) => {
                self.errors
                    .push(BinaryError::Attr(format!("attr `{key}` not an i64: {e}")));
                None
            }
        }
    }
    ...
    pub fn optional_u64(&mut self, key: &str) -> Option<u64> {
        ...
        match s.parse::<u64>() {
            ...
            Err(e) => {
                self.errors
                    .push(BinaryError::Attr(format!("attr `{key}` not a u64: {e}")));
                None
            }
        }
    }
