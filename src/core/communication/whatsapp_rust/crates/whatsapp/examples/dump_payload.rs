...
async fn main() {
    ...
    let signed = PreKey::new(1, signed_pre_key_kp)
        .signed_by(&identity)
        .expect("sign");
    ...
    if let Some(dpd) = &p.device_pairing_data {
        ...
        println!(
            "e_keytype: {}",
            hex(dpd.e_keytype.as_deref().unwrap_or(&[]))
        );
        println!("e_ident: {}", hex(dpd.e_ident.as_deref().unwrap_or(&[])));
        println!(
            "e_skey_id: {}",
            hex(dpd.e_skey_id.as_deref().unwrap_or(&[]))
        );
        println!(
            "e_skey_val: {}",
            hex(dpd.e_skey_val.as_deref().unwrap_or(&[]))
        );
        println!(
            "e_skey_sig: {}",
            hex(dpd.e_skey_sig.as_deref().unwrap_or(&[]))
        );
        println!(
            "build_hash: {}",
            hex(dpd.build_hash.as_deref().unwrap_or(&[]))
        );
