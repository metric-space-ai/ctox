...
fn type_summary(m: &wha_proto::e2e::Message) -> String {
    ...
    if m.conversation.is_some() {
        parts.push("conversation");
    }
    if m.image_message.is_some() {
        parts.push("image");
    }
    if m.video_message.is_some() {
        parts.push("video");
    }
    ...
    if m.sticker_message.is_some() {
        parts.push("sticker");
    }
    if m.contact_message.is_some() {
        parts.push("contact");
    }
    if m.location_message.is_some() {
        parts.push("location");
    }
    if m.protocol_message.is_some() {
        parts.push("protocol");
    }
    if m.reaction_message.is_some() {
        parts.push("reaction");
    }
    if m.sender_key_distribution_message.is_some() {
        parts.push("skdm");
    }
    if parts.is_empty() {
        "(unknown)".into()
...
async fn run_pairing(
    ...
    log_push(
        &log,
        "Phase 1: opening connection with sqlite-backed device…",
    );
    let (mut client, mut events) = Client::new(device);

    log_push(
        &log,
        "opening websocket to wss://web.whatsapp.com/ws/chat …",
    );
    log_push(
        &log,
        "running Noise XX handshake + sending ClientPayload (registration)…",
    );
    client
        .connect()
        .await
    ...
    log_push(
        &log,
        "handshake complete; waiting for <iq><pair-device> from server…",
    );
    ...
    while let Some(evt) = events.recv().await {
        match evt {
            ...
            Event::PairSuccess { id } => {
                ...
                log_push(
                    &log,
                    format!("event: PairSuccess id={jid} (handle_pair_success done, ack sent)"),
                );
                *status.write() = PairStatus::Paired(jid);
...
async fn run_logged_in(
    ...
    log_push(
        &log,
        "Phase 2: waiting 1s for WA's post-pair disconnect to settle…",
    );
    ...
    if client.device.push_name.is_empty() {
        ...
        log_push(
            &log,
            format!(
                "using existing device.push_name = {:?}",
    ...
    log_push(
        &log,
        "Phase 2: re-opening websocket (login mode, passive=true)…",
    );
    client
        .connect()
        .await
        .map_err(|e| format!("logged-in connect: {e}"))?;
    let jid_str = client
        .device
        .jid()
        .map(|j| j.to_string())
    ...
    *status.write() = PairStatus::LoggedIn(jid_str.clone());
    log_push(
        &log,
        format!("logged in as {jid_str}; waiting for inbound <message>…"),
    );
        ...
        match evt {
            ...
            Event::UnhandledNode { node } => {
                ...
                if node.tag == "iq" && node.child_by_tag(&["pair-device"]).is_some() {
                    log_push(
                        &log,
                        "phase-2: another pair-device — handing back to handle_pair_device",
                    );
                ...
                if node.tag == "success" {
                    log_push(
                        &log,
                        "got <success>; sending <iq xmlns=passive><active/></iq>",
                    );
                    ...
                    match wha_client::prekeys::upload_pre_keys(&client).await {
                        Ok(()) => {
                            ...
                            log_push(
                                &log,
                                format!("pre-keys uploaded; store count {before}→{after}"),
                            );
                ...
                if node.tag == "iq" && node.get_attr_str("xmlns") == Some("urn:xmpp:ping") {
                ...
                if node.tag == "notification" {
                    ...
                    log_push(
                        &log,
                        format!("acked <notification type={:?}>", node.get_attr_str("type")),
                    );
                    continue;
                }
                ...
                if node.tag == "message" {
                    log_push(
                        &log,
                        format!("inbound <message> id={:?}", node.get_attr_str("id")),
                    );
                    match wha_client::recv_message::handle_encrypted_message(&client, &node).await {
                        Ok(dec) => {
                            ...
                            // display.
                            let decoded = <wha_proto::e2e::Message as prost::Message>::decode(
                            ...
                            let preview = match &decoded {
                                Ok(m) => {
                                    if let Some(c) = m.conversation.clone() {
                                        ...
                                        format!("non-text message types_set={}", type_summary(m))
                                    }
                                }
                            ...
                            *status.write() = PairStatus::GotMessage {
                            ...
                            if let Ok(m) = decoded.as_ref() {
                                ...
                                if has_media {
                                    if let Some(id) = node.get_attr_str("id") {
                                        media_ring.write().push(id.to_owned(), m.clone());
                                        log_push(
                                            &log,
                                            format!(
                        ...
                        Err(e) => {
                            log_push(&log, format!("decrypt failed for inbound message: {e}"));
                            ...
                            if let Some(id) = node.get_attr_str("id") {
                                ack_attrs
                                    .insert("id".into(), wha_binary::Value::String(id.to_owned()));
                            }
                            ack_attrs.insert(
                                "class".into(),
                                wha_binary::Value::String("message".into()),
                            );
                            if let Some(from) = node.attrs.get("from") {
                                ack_attrs.insert("to".into(), from.clone());
                            }
                ...
                if node.tag == "ib" {
                    if let Some(dirty) = node.children().iter().find(|c| c.tag == "dirty") {
                        ...
                        clean_attrs.insert("type".into(), wha_binary::Value::String(typ.clone()));
                        clean_attrs
                        ...
                        log_push(&log, format!("MarkNotDirty type={typ} ts={ts}"));
                        continue;
                    }
                }
    ...
    loop {
        ...
        tokio::spawn(async move {
            ...
            let response = match (method.as_str(), path.as_str()) {
                ...
                ("POST", "/send") => send_endpoint(&log, &client_handle, body_bytes).await,
                ("POST", "/react") => react_endpoint(&log, &client_handle, body_bytes).await,
                ("POST", "/reply") => reply_endpoint(&log, &client_handle, body_bytes).await,
                ("POST", "/revoke") => revoke_endpoint(&log, &client_handle, body_bytes).await,
...
async fn send_endpoint(log: &SharedLog, shared_client: &SharedClient, body: &[u8]) -> Vec<u8> {
...
async fn react_endpoint(log: &SharedLog, shared_client: &SharedClient, body: &[u8]) -> Vec<u8> {
    let client = match shared_client.read().clone() {
        Some(c) => c,
        None => {
            return http_response(
...
async fn reply_endpoint(log: &SharedLog, shared_client: &SharedClient, body: &[u8]) -> Vec<u8> {
    let client = match shared_client.read().clone() {
        Some(c) => c,
        None => {
            return http_response(
...
async fn revoke_endpoint(log: &SharedLog, shared_client: &SharedClient, body: &[u8]) -> Vec<u8> {
    let client = match shared_client.read().clone() {
        Some(c) => c,
        None => {
            return http_response(
