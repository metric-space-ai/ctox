...
async fn binary_codec_round_trip_for_a_complex_node() {
    ...
    let inner_jid_attr = {
        let mut a = Attrs::new();
        a.insert("to".into(), Value::Jid(Jid::new("12345", "s.whatsapp.net")));
...
async fn group_cipher_three_member_simulation() {
    ...
    let distribution_msg =
        GroupSessionBuilder::create_distribution_message(&mut alice_record, &name)
...
async fn prekey_upload_iq_well_formed() {
    ...
    let one_time = store.get_or_gen_pre_keys(5).await.expect("gen prekeys");
...
async fn pair_qr_string_format() {
    ...
    assert_eq!(
        ...
        "identity base64 must round-trip"
    );
...
async fn media_round_trip_local() {
    ...
    let ciphertext = cbc_encrypt(&keys.cipher_key, &keys.iv, &plaintext).expect("AES-CBC encrypt");
