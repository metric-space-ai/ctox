...
use whatsapp::client::pair::{account_sign_for_tests, make_qr_string, parse_pair_success};
...
fn synth_pair_device_iq(refs: &[&str], iq_id: &str) -> Node {
    let ref_nodes: Vec<Node> = refs
        ...
        .collect();
    let pair_device = Node::new("pair-device", Attrs::new(), Some(Value::Nodes(ref_nodes)));
...
fn synth_pair_success_iq(
    ...
    let signed_identity_bytes =
        encode_adv_signed_device_identity(&inner_bytes, account_kp.public.as_ref(), &sig, None);
...
async fn pair_device_iq_yields_well_formed_qr_strings() {
    ...
    assert_eq!(
        refs,
        vec!["ref-A".to_owned(), "ref-B".into(), "ref-C".into()]
    );
    ...
    assert!(
        ...
        "no event expected before any traffic"
    );
}
