...
impl Account {
    ...
    pub async fn open(path: impl AsRef<Path>) -> Result<Self> {
        ...
        let store =
            Arc::new(SqliteStore::open(path.to_str().ok_or_else(|| {
    ...
    pub async fn mark_read(
        ...
        let cli = self.require_client()?;
        Ok(
            wha_client::presence_receipt::mark_read(&cli, message_ids, timestamp, chat, sender)
                .await?,
        )
    }
}
                    ...
                    match child.tag.as_str() {
                        ...
                        "pair-success" => {
                            ...
                            let jid = device
                                .id
                                .clone()
                                .ok_or(Error::Internal("pair-success without jid".into()))?;
        ...
        while let Some(evt) = low_events.recv().await {
            match evt {
                ...
                LowEvent::UnhandledNode { node } => {
                    if let Err(e) =
                        handle_phase2_node(&client, &node, &etx, &store, &mut bootstrapped).await
                    {
    ...
    match node.tag.as_str() {
        ...
        "message" => {
            ...
            match client.decrypt_message(node).await {
                Ok(dec) => {
                    let proto = ProtoMessage::decode(dec.plaintext.as_slice()).map_err(|e| {
...
async fn send_clean_iq(client: &Arc<Client>, typ: &str, ts: &str) -> Result<()> {
    ...
    let iq = wha_binary::Node::new("iq", iq_attrs, Some(wha_binary::Value::Nodes(vec![clean])));
    client.send_node(&iq).await?;
    Ok(())
}
