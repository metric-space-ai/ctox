...
pub fn build_media_conn_iq(id: impl Into<String>) -> Node {
    ...
    attrs.insert("to".into(), Value::Jid(Jid::new("", server::DEFAULT_USER)));
    Node::new(
        "iq",
        attrs,
...
pub async fn download_encrypted_media(
    ...
    for host in &conn.hosts {
        let url =
            format!("https://{host}{direct_path}&hash={hash_b64}&mms-type={mms_type}&__wa-mms=");
...
/// `WhatsApp Image Keys`.
pub async fn download_image(conn: &MediaConn, msg: &ImageMessage) -> Result<Vec<u8>, MediaError> {
    download_attachment(
        conn,
...
/// `WhatsApp Video Keys`.
pub async fn download_video(conn: &MediaConn, msg: &VideoMessage) -> Result<Vec<u8>, MediaError> {
    download_attachment(
        conn,
        msg.direct_path.as_deref(),
        msg.media_key.as_deref(),
...
pub async fn download_audio(conn: &MediaConn, msg: &AudioMessage) -> Result<Vec<u8>, MediaError> {
    download_attachment(
        conn,
        msg.direct_path.as_deref(),
        msg.media_key.as_deref(),
...
impl UploadHttpClient for ReqwestUploadHttp {
    async fn post(
        ...
        let status = resp.status();
        if !status.is_success() {
            return Err(MediaError::Http(format!("non-success status {status}")));
        }
        let bytes = resp
            .bytes()
            .await
...
pub async fn upload_with_http(
    ...
    if conn.hosts.is_empty() {
        return Err(MediaError::Malformed("media_conn returned no hosts".into()));
    }
    ...
    let mut last_err = String::from("(no hosts attempted)");
    for host in &conn.hosts {
        let url = format!("https://{host}/mms/{mms_segment}/{token}?auth={auth_q}&token={token_q}");
            ...
            .await
        {
            Ok(body) => {
                ...
                let url_field = extract_json_string_field(body_str, "url").unwrap_or_default();
                let direct_path =
                    extract_json_string_field(body_str, "direct_path").unwrap_or_default();
                let handle = extract_json_string_field(body_str, "handle").unwrap_or_default();
...
mod tests {
    ...
    fn parse_response_extracts_auth_and_hosts() {
        ...
        h2.insert(
            "hostname".into(),
            Value::String("mmg-fna.whatsapp.net".into()),
        );
    ...
    fn image_round_trip_uses_image_info() {
        ...
        let err =
            decrypt_via_metadata(&blob, img.media_key.as_ref().unwrap(), VIDEO_INFO).unwrap_err();
    ...
    fn document_round_trip_uses_document_info() {
        ...
        let out =
            decrypt_via_metadata(&blob, d.media_key.as_ref().unwrap(), DOCUMENT_INFO).unwrap();
        assert_eq!(out, plain);
    }
    ...
    async fn upload_round_trip_with_local_decrypt() {
        ...
        let body_response =
            br#"{"url":"https://cdn.example/u","direct_path":"/v/t62/u","handle":"H1"}"#.to_vec();
    ...
    async fn media_url_path_includes_media_type_segment() {
        ...
        let json_resp = br#"{"url":"https://cdn/u","direct_path":"/v/u","handle":""}"#.to_vec();
        ...
        for (media_type, expected_segment) in [
            ...
            let _ = upload_with_http(&conn, b"data", media_type, &conn.auth, http.as_ref())
                .await
                .expect("upload ok");
            let url = http.captured_url.lock().await.clone().expect("url");
            assert!(
                url.starts_with(&format!("https://mmg.example.test/mms/{expected_segment}/")),
    ...
    async fn upload_falls_through_to_second_host_on_failure() {
            ...
            async fn post(
                ...
                *a += 1;
                if url.contains("primary") {
                    ...
                    Ok(br#"{"url":"https://x/u","direct_path":"/v/u","handle":""}"#.to_vec())
                }
            }
        }
        let conn = MediaConn {
        ...
        let result = upload_with_http(&conn, b"hi", "audio", &conn.auth, &http)
            .await
            .unwrap();
