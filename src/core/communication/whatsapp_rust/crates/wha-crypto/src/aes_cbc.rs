...
pub fn cbc_encrypt(key: &[u8], iv: &[u8], plaintext: &[u8]) -> Result<Vec<u8>, CryptoError> {
    ...
    let enc =
        Encryptor::new_from_slices(key, iv).map_err(|e| CryptoError::Internal(e.to_string()))?;
...
pub fn cbc_decrypt(key: &[u8], iv: &[u8], ciphertext: &[u8]) -> Result<Vec<u8>, CryptoError> {
    ...
    let dec =
        Decryptor::new_from_slices(key, iv).map_err(|e| CryptoError::Internal(e.to_string()))?;
...
pub fn ctr_xor(key: &[u8], iv: &[u8], data: &mut [u8]) -> Result<(), CryptoError> {
    ...
    let cipher =
        aes::Aes256::new_from_slice(key).map_err(|e| CryptoError::Internal(e.to_string()))?;
