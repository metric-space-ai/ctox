...
pub fn decode_mutation(
    ...
    let index_mac =
        record
            ...
            .ok_or(AppStateError::Protobuf(
                "mutation missing index blob".into(),
            ))?;
...
pub fn decode_snapshot(
    ...
    if validate_macs {
        let expected_snapshot_mac = generate_snapshot_mac(&working_state, name, &keys.snapshot_mac);
...
mod tests {
    ...
    fn parse_index_handles_basic_array() {
        ...
        assert_eq!(
            parsed,
            vec!["mute".to_string(), "1234@s.whatsapp.net".to_string()]
        );
    }
