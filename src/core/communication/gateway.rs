...
pub(crate) fn transcribe_audio_file(root: &Path, audio_path: &Path, model: &str) -> Result<String> {
    ...
    if let Some(base_url) = resolved
        ...
        .map(str::to_string)
    {
        return transcribe_via_http(&base_url, audio_path, &file_bytes, model);
    }
    native_stt::transcribe_audio_file(root, audio_path)
        .map(|output| output.text)
        .with_context(|| "transcription runtime is not resolved and native STT fallback failed")
}
