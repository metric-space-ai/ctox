...
pub fn discover_source_layout_paths(root: &Path) -> SourceLayoutPaths {
    ...
    let tools_root = runtime_env::env_or_config(root, "CTOX_TOOLS_ROOT")
        ...
        .unwrap_or_else(|| root.join("src/tools"));
    let default_agent_runtime_root = root.join("src/core/harness");
    let agent_runtime_root = runtime_env::env_or_config(root, "CTOX_AGENT_RUNTIME_ROOT")
        ...
        .unwrap_or_else(|| root.join("src/core/harness"));
...
mod tests {
    ...
    fn source_layout_paths_use_harness_runtime_root() {
        let deps = discover_source_layout_paths(Path::new("/tmp/ctox"));
        assert_eq!(deps.tools_root, PathBuf::from("/tmp/ctox/src/tools"));
        assert_eq!(
            deps.agent_runtime_root,
            PathBuf::from("/tmp/ctox/src/core/harness")
        );
        assert_eq!(
            deps.model_runtime_binary,
            PathBuf::from("/tmp/ctox/src/tools/model-runtime/bin/ctox-engine")
        );
    }
    ...
    fn recognizes_new_openrouter_api_chat_models() {
        ...
        assert!(is_openrouter_api_chat_model("deepseek/deepseek-v4-flash"));
        assert_eq!(
            default_api_provider_for_model("moonshotai/kimi-k2.6"),
            "openrouter"
        );
        assert_eq!(
            default_api_provider_for_model("deepseek/deepseek-v4-flash"),
            "openrouter"
        );
        assert!(api_provider_supports_model(
            "openrouter",
        ...
        assert_eq!(
            chat_model_family_for_model("deepseek/deepseek-v4-flash"),
            Some(ChatModelFamily::DeepSeek)
        );
        assert_eq!(
