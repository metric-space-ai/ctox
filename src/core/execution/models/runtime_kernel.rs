...
/// Window during which a previously computed `InferenceRuntimeKernel` is
/// reused. `resolve` iterates over every auxiliary role, and each one calls
/// `load_runtime_env_map` → `merge_credentials_into_env_map` which probes 19
/// secret keys with one SQLite open + schema parse per key. The TUI's
/// `refresh_header` runs `resolve` (twice, via `runtime_health_state`) and
/// `synthesize_runtime_telemetry` runs it again — together that was the main
/// CPU and lag source. State-changing call sites (`execute_runtime_switch`,
/// boost start/stop, settings save) must call
/// `invalidate_runtime_kernel_cache` to drop the cached value.
const RUNTIME_KERNEL_CACHE_TTL: Duration = Duration::from_secs(5);
...
fn runtime_kernel_cache() -> &'static Mutex<Option<(Instant, PathBuf, InferenceRuntimeKernel)>> {
    static CACHE: OnceLock<Mutex<Option<(Instant, PathBuf, InferenceRuntimeKernel)>>> =
        OnceLock::new();
    CACHE.get_or_init(|| Mutex::new(None))
}

/// Drop any cached `InferenceRuntimeKernel`. Call after a state-changing
/// operation so the next `resolve` reflects the change instead of returning
/// the stale snapshot.
pub fn invalidate_runtime_kernel_cache() {
    if let Ok(mut guard) = runtime_kernel_cache().lock() {
        *guard = None;
    }
}
...
impl InferenceRuntimeKernel {
    pub fn resolve(root: &Path) -> Result<Self> {
        {
            let guard = runtime_kernel_cache()
                .lock()
                .unwrap_or_else(|err| err.into_inner());
            if let Some((cached_at, cached_root, cached_kernel)) = guard.as_ref() {
                if cached_root.as_path() == root && cached_at.elapsed() < RUNTIME_KERNEL_CACHE_TTL {
                    return Ok(cached_kernel.clone());
                }
            }
        }
        let kernel = Self::resolve_uncached(root)?;
        let mut guard = runtime_kernel_cache()
            .lock()
            .unwrap_or_else(|err| err.into_inner());
        *guard = Some((Instant::now(), root.to_path_buf(), kernel.clone()));
        Ok(kernel)
    }

    fn resolve_uncached(root: &Path) -> Result<Self> {
