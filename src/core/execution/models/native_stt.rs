...
pub fn doctor_json(root: &Path) -> serde_json::Value {
    let model_root = root.join("src/core/inference/models/voxtral_mini_4b_realtime_2602");
...
pub fn transcribe_audio_file(root: &Path, audio_path: &Path) -> Result<TranscriptionResponse> {
    let audio = std::fs::read(audio_path)
        .with_context(|| format!("failed to read audio file {}", audio_path.display()))?;
    let runtime = stt_runtime_for_smoke(root)?;
    runtime.transcribe(&TranscriptionRequest {
        audio_bytes: &audio,
        response_format: "json",
        max_tokens: None,
    })
...
mod tests {
    ...
    fn doctor_reports_native_voxtral_stt_without_fake_text() {
        ...
        std::fs::create_dir_all(
            root.join(
                "src/core/inference/models/voxtral_mini_4b_realtime_2602/vendor/metal/kernels",
            ),
        )
        .unwrap();
        std::fs::create_dir_all(
            root.join(
                "src/core/inference/models/voxtral_mini_4b_realtime_2602/vendor/cuda/kernels",
            ),
        )
        .unwrap();
        std::fs::create_dir_all(
            root.join(
                "src/core/inference/models/voxtral_mini_4b_realtime_2602/vendor/wgsl/kernels",
            ),
        )
        .unwrap();
        std::fs::write(root.join("src/core/inference/models/voxtral_mini_4b_realtime_2602/vendor/metal/kernels/ctox_voxtral_stt_glue.metal"), "").unwrap();
        std::fs::write(root.join("src/core/inference/models/voxtral_mini_4b_realtime_2602/vendor/cuda/kernels/ctox_voxtral_stt_glue.cu"), "").unwrap();
        std::fs::write(root.join("src/core/inference/models/voxtral_mini_4b_realtime_2602/vendor/wgsl/kernels/ctox_voxtral_stt_glue.wgsl"), "").unwrap();
