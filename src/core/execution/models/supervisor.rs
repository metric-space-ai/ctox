...
fn resolve_managed_engine_binary(
    ...
    anyhow::bail!(
        "no local inference backend registered for `{}` and ctox-engine is not available for role `{}` at {}. Add a per-model crate under src/core/inference/models/<model>/, install the ctox-engine auxiliary runtime, or configure this role against an external API provider.",
        launch_spec.request_model,
        launch_spec.role,
        ctox_engine.display()
    )
...
fn spawn_managed_engine_backend(
    ...
    // binaries under src/core/inference/models/<model>/, build the
...
fn ensure_backend_process(
    ...
    // under src/core/inference/models/ are still being landed (embedding,
