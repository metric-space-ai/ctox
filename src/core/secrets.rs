...
/// process. `ensure_secret_schema` is a CREATE TABLE / CREATE INDEX batch
/// that takes a writer lock even when the tables already exist — running it
/// on every `get_secret_value` (which the TUI does ~150 times per refresh
/// via `merge_credentials_into_env_map`) was a major source of stalls when
/// the long-running daemon held the writer.
fn initialized_secret_paths() -> &'static Mutex<HashSet<PathBuf>> {
...
fn ensure_secret_schema(conn: &Connection) -> Result<()> {
    // Schema verification is idempotent on the file but still requires a
    // writer lock per call. Skip on subsequent calls for the same DB path
    // in this process — the daemon and TUI each pay the cost exactly once.
    let conn_path = conn.path().map(PathBuf::from);
    if let Some(ref path) = conn_path {
        if initialized_secret_paths()
            .lock()
...
///
/// Performance: this runs on the TUI's hot refresh path. The previous
/// implementation looped over `SECRET_KEYS` and called `get_credential` for
/// each one, which opened a fresh SQLite connection per secret. SQLite
/// re-parses the schema from `sqlite_master` on the first query of every new
/// connection — so 19 keys × 1 schema parse each became the dominant CPU
/// cost (visible in `sqlite3LocateTable → sqlite3InitOne` in profiling).
///
/// This version issues a single bulk SELECT against the secret store, then
/// decrypts each matching row. One connection open, one schema parse.
pub fn merge_credentials_into_env_map(
    ...
    if let Ok(records) = load_credentials_bulk(root, &missing) {
        let key_material = ensure_secret_master_key(root).ok().map(|(key, _)| key);
        if let Some(key_bytes) = key_material {
            for (name, nonce_b64, ciphertext_b64) in records {
                if let Ok(plaintext) = decrypt_secret_value(&key_bytes, &nonce_b64, &ciphertext_b64)
                {
                    if let Ok(value) = std::str::from_utf8(&plaintext) {
                        if !value.trim().is_empty() {
                            env_map.insert(name, value.to_string());
                        }
                    }
                }
            }
...
/// Bulk-load credential rows for the supplied names in a single SQLite
/// query. Returns `(secret_name, nonce_b64, ciphertext_b64)` for each row
/// that matched; missing names are silently omitted (caller falls back to
/// whatever was already in the env map).
fn load_credentials_bulk(
    root: &Path,
    names: &[&'static str],
    ...
    let placeholders = (1..=names.len())
        ...
        .join(", ");
    let sql = format!(
        "SELECT secret_name, nonce_b64, ciphertext_b64
         FROM ctox_secret_records
         WHERE scope = ?1 AND secret_name IN ({placeholders})"
    );
    let mut stmt = conn.prepare(&sql)?;
    let mut params: Vec<&dyn rusqlite::ToSql> = Vec::with_capacity(names.len() + 1);
    let scope: &str = CREDENTIAL_SCOPE;
    params.push(&scope);
    for name in names {
        params.push(name);
    }
