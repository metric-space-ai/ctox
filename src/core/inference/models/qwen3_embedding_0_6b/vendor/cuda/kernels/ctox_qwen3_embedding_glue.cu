...
extern "C" __global__ void ctox_qwen35_27b_rms_norm_bf16_kernel(
...
extern "C" __global__ void ctox_qwen35_27b_dense_matmul_bf16_kernel(
    const __nv_bfloat16* x,
...
extern "C" __global__ void ctox_qwen35_27b_add_bf16_kernel(
...
extern "C" __global__ void ctox_qwen35_27b_mul_bf16_kernel(
    const __nv_bfloat16* a,
    const __nv_bfloat16* b,
    __nv_bfloat16* y,
    int n
...
extern "C" __global__ void ctox_qwen35_27b_silu_bf16_kernel(
    const __nv_bfloat16* x,
    __nv_bfloat16* y,
    int n
) {
...
extern "C" __global__ void ctox_qwen35_27b_argmax_bf16_kernel(
    const __nv_bfloat16* x,
    int32_t* out,
    int vocab,
    int rows
...
extern "C" __global__ void ctox_qwen35_27b_argmax_f32_kernel(
    const float* x,
    int32_t* out,
    int vocab,
    int rows
...
extern "C" __global__ void ctox_qwen35_27b_copy_hidden_slot_bf16_kernel(
...
extern "C" __global__ void ctox_qwen35_27b_repeat_hidden_slots_bf16_kernel(
    const __nv_bfloat16* src,
    __nv_bfloat16* dst,
    int hidden,
    int dst_slots
...
extern "C" __global__ void ctox_qwen35_27b_fill_positions4_i32_kernel(
    int32_t* out,
    int start_pos,
    int n_tokens
) {
...
extern "C" __global__ void ctox_qwen35_27b_causal_mask_f16_kernel(
    __half* out,
    int kv_start,
    int n_tokens,
    int kv_len,
...
extern "C" __global__ void ctox_qwen35_27b_kv_store_bf16_kernel(
    const __nv_bfloat16* src,
    __nv_bfloat16* cache,
...
extern "C" __global__ void ctox_qwen35_27b_sdpa_decode_bf16_kernel(
...
extern "C" __global__ void ctox_qwen35_27b_dequant_q4_k_bf16_kernel(
...
extern "C" __global__ void ctox_qwen35_27b_q4_k_matvec_bf16_kernel(
...
extern "C" cudaError_t ctox_qwen35_27b_rms_norm_bf16_launch(
    const void* x,
    const void* weight,
    void* y,
    int d,
    ...
    ctox_qwen35_27b_rms_norm_bf16_kernel<<<rows, 256, 0, stream>>>(
...
extern "C" cudaError_t ctox_qwen35_27b_dense_matmul_bf16_launch(
    const void* x,
    const void* w,
    void* y,
    int rows,
    ...
    ctox_qwen35_27b_dense_matmul_bf16_kernel<<<grid, threads, 0, stream>>>(
        static_cast<const __nv_bfloat16*>(x),
...
extern "C" cudaError_t ctox_qwen35_27b_add_bf16_launch(
    const void* a,
    const void* b,
    void* y,
    int n,
    ...
    ctox_qwen35_27b_add_bf16_kernel<<<blocks, threads, 0, stream>>>(
...
extern "C" cudaError_t ctox_qwen35_27b_mul_bf16_launch(
    const void* a,
    const void* b,
    void* y,
    int n,
    ...
    const int threads = 256;
    const int blocks = (n + threads - 1) / threads;
    ctox_qwen35_27b_mul_bf16_kernel<<<blocks, threads, 0, stream>>>(
        static_cast<const __nv_bfloat16*>(a),
        static_cast<const __nv_bfloat16*>(b),
        static_cast<__nv_bfloat16*>(y),
        n
...
extern "C" cudaError_t ctox_qwen35_27b_silu_bf16_launch(
    const void* x,
    void* y,
    int n,
    cudaStream_t stream
    ...
    const int threads = 256;
    const int blocks = (n + threads - 1) / threads;
    ctox_qwen35_27b_silu_bf16_kernel<<<blocks, threads, 0, stream>>>(
        static_cast<const __nv_bfloat16*>(x),
        static_cast<__nv_bfloat16*>(y),
        n
    );
...
extern "C" cudaError_t ctox_qwen35_27b_argmax_bf16_launch(
    const void* x,
    int32_t* out,
    int vocab,
    int rows,
    ...
    ctox_qwen35_27b_argmax_bf16_kernel<<<rows, 1, 0, stream>>>(
        static_cast<const __nv_bfloat16*>(x),
        out,
        vocab,
        rows
...
extern "C" cudaError_t ctox_qwen35_27b_argmax_f32_launch(
    const void* x,
    int32_t* out,
    int vocab,
    int rows,
    ...
    ctox_qwen35_27b_argmax_f32_kernel<<<rows, 256, 0, stream>>>(
...
extern "C" cudaError_t ctox_qwen35_27b_copy_hidden_slot_bf16_launch(
    const void* src,
    void* dst,
    int src_row,
    int dst_slot,
    ...
    ctox_qwen35_27b_copy_hidden_slot_bf16_kernel<<<blocks, threads, 0, stream>>>(
...
extern "C" cudaError_t ctox_qwen35_27b_repeat_hidden_slots_bf16_launch(
    const void* src,
    void* dst,
    int hidden,
    int dst_slots,
    ...
    const int n = hidden * dst_slots;
    const int blocks = (n + threads - 1) / threads;
    ctox_qwen35_27b_repeat_hidden_slots_bf16_kernel<<<blocks, threads, 0, stream>>>(
        static_cast<const __nv_bfloat16*>(src),
        static_cast<__nv_bfloat16*>(dst),
        hidden,
        dst_slots
...
extern "C" cudaError_t ctox_qwen35_27b_fill_positions4_i32_launch(
    int32_t* out,
    int start_pos,
    int n_tokens,
    cudaStream_t stream
    ...
    const int n = n_tokens * 4;
    const int blocks = (n + threads - 1) / threads;
    ctox_qwen35_27b_fill_positions4_i32_kernel<<<blocks, threads, 0, stream>>>(
        out,
        start_pos,
        n_tokens
    );
...
extern "C" cudaError_t ctox_qwen35_27b_causal_mask_f16_launch(
    void* out,
    int kv_start,
    int n_tokens,
    int kv_len,
    ...
    const int n = n_tokens * q_stride;
    const int blocks = (n + threads - 1) / threads;
    ctox_qwen35_27b_causal_mask_f16_kernel<<<blocks, threads, 0, stream>>>(
...
extern "C" cudaError_t ctox_qwen35_27b_kv_store_bf16_launch(
    const void* src,
    void* cache,
    const int32_t* positions4,
    int n_tokens,
    ...
    const int n = n_tokens * n_kv_heads * head_dim;
    const int blocks = (n + threads - 1) / threads;
    ctox_qwen35_27b_kv_store_bf16_kernel<<<blocks, threads, 0, stream>>>(
        static_cast<const __nv_bfloat16*>(src),
...
extern "C" cudaError_t ctox_qwen35_27b_sdpa_decode_bf16_launch(
    const void* q,
    const void* k_cache,
    const void* v_cache,
    void* out,
    ...
    ctox_qwen35_27b_sdpa_decode_bf16_kernel<<<n_q_heads, 256, 0, stream>>>(
...
extern "C" cudaError_t ctox_qwen35_27b_dequant_q4_k_bf16_launch(
    const void* x,
    void* y,
    int n_blocks,
    cudaStream_t stream
) {
    ctox_qwen35_27b_dequant_q4_k_bf16_kernel<<<n_blocks, 32, 0, stream>>>(
...
extern "C" cudaError_t ctox_qwen35_27b_q4_k_matvec_bf16_launch(
    const void* w,
    const void* x,
    void* y,
    int in_dim,
    ...
    ctox_qwen35_27b_q4_k_matvec_bf16_kernel<<<out_dim, 1, 0, stream>>>(
...
extern "C" __global__ void ctox_qwen3_embedding_last_token_pool_bf16_kernel(
    ...
    const int hidden_index = (row * seq_len + (seq_len - 1)) * dim + col;
    out[row * dim + col] = __bfloat162float(hidden[hidden_index]);
}

extern "C" __global__ void ctox_qwen3_embedding_l2_normalize_f32_kernel(
    float* vectors,
    int batch,
    int dim
) {
    ...
    for (int col = tid; col < dim; col += blockDim.x) {
        const float value = vectors[row * dim + col];
        sum += value * value;
    }
    partial[tid] = sum;
    __syncthreads();
    ...
    for (int col = tid; col < dim; col += blockDim.x) {
        vectors[row * dim + col] /= norm;
    }
}

extern "C" cudaError_t ctox_qwen3_embedding_last_token_pool_bf16_launch(
    const void* hidden,
    float* out,
    int batch,
    int seq_len,
    ...
    const int threads = 256;
    const dim3 grid(batch, (dim + threads - 1) / threads, 1);
    ctox_qwen3_embedding_last_token_pool_bf16_kernel<<<grid, threads, 0, stream>>>(
...
extern "C" cudaError_t ctox_qwen3_embedding_l2_normalize_f32_launch(
    float* vectors,
    int batch,
    int dim,
    cudaStream_t stream
    ...
    ctox_qwen3_embedding_l2_normalize_f32_kernel<<<batch, 256, 0, stream>>>(
        vectors,
        batch,
        dim
    );
