...
fn main() {
    eprintln!("bench_metalpack_decode_attention_steps is only available on macOS + Metal.");
    std::process::exit(2);
}
...
fn main() -> Result<(), String> {
    use ctox_qwen35_08b_metal_probe::metal::bench::{
        run_decode_attention_tiled_sequence_with_weights, DecodeSkeletonBenchConfig,
    };
    ...
    if args.len() < 2 {
        return Err("usage: bench_metalpack_decode_attention_steps <metalpack-dir> [layer-prefix] [input-token] [steps] [iterations] [max-context]".to_owned());
    }
    ...
    let result = run_decode_attention_tiled_sequence_with_weights(
        cfg,
        steps,
        &embedding_weights,
        &norm,
    ...
    println!("qwen35-08b metalpack decode + attention multi-step benchmark");
    ...
    println!(
        "effective_gb_s_attention_sequence_lm_head_pairs: {:.2}",
        result.effective_gb_s
    );
    println!("tokens: {:?}", result.tokens);
    println!("last_score: {:.6}", result.last_score);
    Ok(())
}
...
fn validate_hidden_in(label: &str, entry: &MetalPackEntry) -> Result<(), String> {
    ...
    if entry.source_shape.len() != 2 || entry.source_shape[1] != QWEN35_08B.hidden_size {
        return Err(format!(
            "{label}: expected [rows,1024] tensor, got {:?}",
            entry.source_shape
        ));
    }
    Ok(())
...
#[cfg(target_os = "macos")]
fn validate_tile(entry: &MetalPackEntry, embedding: &MetalPackEntry) -> Result<(), String> {
