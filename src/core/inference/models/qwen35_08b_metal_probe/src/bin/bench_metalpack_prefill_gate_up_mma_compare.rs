...
fn main() {
    eprintln!("bench_metalpack_prefill_gate_up_mma_compare is only available on macOS + Metal.");
    std::process::exit(2);
}
...
fn main() -> Result<(), String> {
    use ctox_qwen35_08b_metal_probe::metal::bench::{
        run_prefill_gate_up_mma_compare_with_weights, PrefillFfnBlockBenchConfig,
    };
    ...
    if args.len() < 2 {
        return Err("usage: bench_metalpack_prefill_gate_up_mma_compare <metalpack-dir> [layer] [tokens] [iterations]".to_owned());
    }
    ...
    let result = run_prefill_gate_up_mma_compare_with_weights(
        cfg,
        &x_host,
        &norm_weights,
        &gate_weights,
    ...
    println!("qwen35-08b metalpack prefill FFN gate/up MMA compare");
    ...
    println!("mma_token_tile: {}", result.mma_token_tile);
