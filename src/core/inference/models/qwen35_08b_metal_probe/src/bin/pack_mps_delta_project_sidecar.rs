...
fn run() -> Result<(), String> {
    ...
    if args.len() != 3 {
        return Err(
            "usage: pack_mps_delta_project_sidecar <source.metalpack-dir> <output.mps-delta-project-dir>"
                .to_owned(),
        );
    }
    ...
    for layer in 0..QWEN35_08B.n_layers {
        ...
        let qkv = find_layer_class(&pack, layer, TensorClass::DeltaQkv)?;
        let z = find_layer_class(&pack, layer, TensorClass::DeltaZ)?;
        validate_projection(
            "qkv",
            qkv,
        ...
        let qkv_bytes = pack
            .read_entry_bytes(qkv)
            .map_err(|err| format!("{}: {err}", qkv.tensor))?;
        let z_bytes = pack
            .read_entry_bytes(z)
            .map_err(|err| format!("{}: {err}", z.tensor))?;

        let qkvz_offset = offset;
        let qkvz_written = write_qkvz_mps(&mut weights, qkv, &qkv_bytes, z, &z_bytes)?;
        offset += qkvz_written;

        manifest_entries.push(json!({
            "layer": layer,
            "qkv_tensor": qkv.tensor,
            "z_tensor": z.tensor,
            "qkvz": {
                "layout": "mps_fp16_row_major",
                "shape": [QWEN35_08B.hidden_size, QWEN35_08B.deltanet_qkv_width() + QWEN35_08B.deltanet_width()],
                "row_bytes": (QWEN35_08B.deltanet_qkv_width() + QWEN35_08B.deltanet_width()) * 2,
                "offset": qkvz_offset,
                "bytes": qkvz_written,
            }
        }));
    ...
    let manifest = json!({
        ...
        "shape": {
            ...
            "qkv_rows": QWEN35_08B.deltanet_qkv_width(),
            "z_rows": QWEN35_08B.deltanet_width(),
    ...
    println!("qwen35-08b MPS DeltaNet project sidecar written");
...
fn write_qkvz_mps(
    ...
    let row_stride = qkv_rows + z_rows;
    let mut dst = vec![0u16; hidden * row_stride];
    copy_row_tiled_transposed(qkv, qkv_bytes, &mut dst, row_stride, 0)?;
    copy_row_tiled_transposed(z, z_bytes, &mut dst, row_stride, qkv_rows)?;
