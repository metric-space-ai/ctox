...
fn main() {
    eprintln!("bench_metalpack_prefill_ffn_block is only available on macOS + Metal.");
    std::process::exit(2);
}
...
fn main() -> Result<(), String> {
    use ctox_qwen35_08b_metal_probe::metal::bench::{
        run_prefill_ffn_block_with_weights, PrefillFfnBlockBenchConfig,
    };
    ...
    if args.len() < 2 {
        return Err(
            "usage: bench_metalpack_prefill_ffn_block <metalpack-dir> [layer] [tokens] [iterations]"
                .to_owned(),
        );
    }
    ...
    if gate.row_tile != up.row_tile
        || gate.col_tile != up.col_tile
        || gate.row_tile != down.row_tile
    {
        return Err("gate/up/down tile mismatch".to_owned());
    }
    ...
    let result = run_prefill_ffn_block_with_weights(
        cfg,
        &x_host,
        &norm_weights,
        &gate_weights,
    ...
    println!("qwen35-08b metalpack prefill FFN block benchmark");
