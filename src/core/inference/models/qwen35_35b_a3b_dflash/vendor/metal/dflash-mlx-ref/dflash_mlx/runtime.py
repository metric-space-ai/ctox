...
def generate_dflash_once(
    *,
    ...
    try:
        start_ns = time.perf_counter_ns()
        prefill_start_ns = time.perf_counter_ns()
        prefill_step_size = int(os.environ.get("DFLASH_PREFILL_STEP_SIZE", "2048"))
        prefill_logits = None
        ...
        while generated_token_count < max_new_tokens:
            ...
            if os.environ.get("DFLASH_TRACE_CYCLE_TOKENS", "").strip().lower() not in {"", "0", "false", "no"}:
                mx.eval(verify_token_ids, posterior)
                print(
                    "[dflash-python/trace] "
                    f"start={start} block_len={block_len} commit_count={1 + acceptance_len} "
                    f"accepted={acceptance_len} verify_ids={verify_token_ids.tolist()} "
