...
`ctox-network-proxy` is Codex's local network policy enforcement proxy. It runs:
...
`ctox-network-proxy` reads from Codex's merged `config.toml` (via `ctox-core` config loading).
...
```bash
cargo run -p ctox-network-proxy --
```
...
`ctox-network-proxy` can be embedded as a library with a thin API:

```rust
use ctox_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
When `ctox-network-proxy` is embedded in managed Codex runtime, policy decisions emit structured
OTEL-compatible events with `target=ctox_otel.network_proxy`.
...
This section documents the protections implemented by `ctox-network-proxy`, and the boundaries of
