...
use ctox_core::config_loader::ConfigLayerStackOrdering;
...
use ctox_app_server_protocol::TextPosition as AppTextPosition;
use ctox_app_server_protocol::TextRange as AppTextRange;
use ctox_core::ExecPolicyError;
use ctox_core::check_execpolicy_for_warnings;
use ctox_core::config_loader::ConfigLoadError;
use ctox_core::config_loader::TextRange as CoreTextRange;
use ctox_feedback::CodexFeedback;
use ctox_protocol::protocol::SessionSource;
use ctox_state::log_db;
use tokio::sync::mpsc;
...
pub async fn run_main_with_transport(
        ...
        .await
    {
        Ok(config) => {
            ...
            match effective_toml.try_into() {
                Ok(config_toml) => {
                    if let Err(err) = ctox_core::personality_migration::maybe_migrate_personality(
                        &config.codex_home,
                        &config_toml,
                    )
                    .await
    ...
    let otel = ctox_core::otel_init::build_provider(
        &config,
        env!("CARGO_PKG_VERSION"),
        Some("ctox-app-server"),
        default_analytics_enabled,
    )
    .map_err(|e| {
        std::io::Error::new(
    ...
    let log_db = ctox_state::StateRuntime::init(
