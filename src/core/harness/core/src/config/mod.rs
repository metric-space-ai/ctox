...
use ctox_app_server_protocol::Tools;
use ctox_app_server_protocol::UserSavedConfig;
use ctox_protocol::config_types::AltScreenMode;
...
use ctox_protocol::config_types::TrustLevel;
...
use ctox_utils_absolute_path::AbsolutePathBufGuard;
...
use ctox_network_proxy::NetworkProxyConfig;
...
pub mod service;
pub mod types;
pub use ctox_config::Constrained;
pub use ctox_config::ConstraintError;
pub use ctox_config::ConstraintResult;
pub use ctox_network_proxy::NetworkProxyAuditMetadata;
...
pub use ctox_git::GhostSnapshotConfig;
...
fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<PathBuf> {
    let raw = std::env::var(ctox_state::SQLITE_HOME_ENV).ok()?;
...
pub struct Config {
    ...
    /// Session-scoped override used by embedded callers that need a lean
    ...
    /// Path to the `ctox-linux-sandbox` executable. This must be set if
    /// [`crate::exec::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `ctox-linux-sandbox`.
    pub ctox_linux_sandbox_exe: Option<PathBuf>,

    /// Path to the `ctox-execve-wrapper` executable used for shell
    ...
    /// When true, the local CTOX session exposes the CTOX-owned web tool stack
    /// instead of overlapping Codex-native web/browser tools.
    pub ctox_web_stack_enabled: bool,

    /// When true, the local CTOX session exposes the CTOX-owned local
...
impl Config {
    ...
    /// in [ConfigToml], such as `cwd`, `ctox_linux_sandbox_exe`, and
...
pub struct ConfigToml {
    ...
    /// Directory where Codex writes log files, for example `ctox-tui.log`.
...
pub use ctox_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
...
pub struct ToolsToml {
    ...
    /// Enable the CTOX-owned local web tool stack (`ctox_web_search`,
    /// `ctox_web_read`, `ctox_web_scrape`, `ctox_browser_automation`) for
    /// local CTOX sessions.
    #[serde(default)]
    pub ctox_web: Option<bool>,

    /// Enable the CTOX-owned local document tool stack (`ctox_doc_search`,
    /// `ctox_doc_read`) for local CTOX sessions.
    #[serde(default)]
    pub ctox_doc: Option<bool>,
                    ...
                    if p.is_trusted() || p.is_untrusted() {
                        if cfg!(target_os = "windows")
                            && windows_sandbox_level
                                == ctox_protocol::config_types::WindowsSandboxLevel::Disabled
                        {
...
fn add_additional_file_system_writes(
    ...
    for path in additional_writable_roots {
        let exists = file_system_sandbox_policy.entries.iter().any(|entry| {
            matches!(
                &entry.path,
                ctox_protocol::permissions::FileSystemPath::Path { path: existing }
                    if existing == path && entry.access == ctox_protocol::permissions::FileSystemAccessMode::Write
            )
        });
        if !exists {
            file_system_sandbox_policy.entries.push(
                ctox_protocol::permissions::FileSystemSandboxEntry {
                    path: ctox_protocol::permissions::FileSystemPath::Path { path: path.clone() },
                    access: ctox_protocol::permissions::FileSystemAccessMode::Write,
                },
            );
        }
    }
...
fn resolve_ctox_web_stack_enabled(
    config_toml: &ConfigToml,
    config_profile: &ConfigProfile,
) -> bool {
    config_profile
        .tools
        .as_ref()
        .and_then(|tools| tools.ctox_web)
        .or_else(|| config_toml.tools.as_ref().and_then(|tools| tools.ctox_web))
        .unwrap_or(false)
}

fn resolve_ctox_doc_stack_enabled(
    config_toml: &ConfigToml,
    config_profile: &ConfigProfile,
) -> bool {
    config_profile
        .tools
        .as_ref()
        .and_then(|tools| tools.ctox_doc)
        .or_else(|| config_toml.tools.as_ref().and_then(|tools| tools.ctox_doc))
        .unwrap_or(false)
}
...
impl Config {
    ...
    pub(crate) fn load_config_with_layer_stack(
        ...
        let ConfigOverrides {
            ...
            model_context_window: model_context_window_override,
            cwd,
        ...
        if let Some(model_context_window) = model_context_window_override {
            cfg.model_context_window = Some(model_context_window);
        }
        ...
        let ctox_web_stack_enabled = resolve_ctox_web_stack_enabled(&cfg, &config_profile);
        let ctox_doc_stack_enabled = resolve_ctox_doc_stack_enabled(&cfg, &config_profile);
        ...
        let config = Self {
            ...
            web_search_config,
            ctox_web_stack_enabled,
            ctox_doc_stack_enabled,
    ...
    pub fn skills_enabled(&self) -> bool {
        crate::skills::manager::skills_enabled_from_stack(&self.config_layer_stack)
    }
...
pub fn find_codex_home() -> std::io::Result<PathBuf> {
    ctox_utils_home_dir::find_codex_home()
}
