...
use ctox_apply_patch::ApplyPatchAction;
use ctox_apply_patch::ApplyPatchFileChange;
...
async fn effective_patch_permissions(
    ...
    ctox_protocol::permissions::FileSystemSandboxPolicy,
) {
...
impl ToolHandler for ApplyPatchHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        match ctox_apply_patch::maybe_parse_apply_patch_verified(&command, &cwd) {
            ctox_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
                    ...
                    .await
                {
                    ...
                    InternalApplyPatchInvocation::DelegateToExec(apply) => {
                        ...
                        let req = ApplyPatchRequest {
                            ...
                            timeout_ms: None,
                            codex_exe: turn.ctox_linux_sandbox_exe.clone(),
                        };
            ...
            ctox_apply_patch::MaybeApplyPatchVerified::CorrectnessError(parse_error) => {
                Err(FunctionCallError::RespondToModel(format!(
                    "apply_patch verification failed: {parse_error}"
                )))
            }
            ctox_apply_patch::MaybeApplyPatchVerified::ShellParseError(error) => {
                tracing::trace!("Failed to parse apply_patch input, {error:?}");
                Err(FunctionCallError::RespondToModel(
                    "apply_patch handler received invalid patch input".to_string(),
                ))
            }
            ctox_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => {
                Err(FunctionCallError::RespondToModel(
...
pub(crate) async fn intercept_apply_patch(
    ...
    match ctox_apply_patch::maybe_parse_apply_patch_verified(command, cwd) {
        ...
        ctox_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => Ok(None),
    }
}
