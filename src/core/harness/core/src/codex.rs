...
use chrono::Local;
use chrono::Utc;
use ctox_app_server_protocol::McpServerElicitationRequest;
use ctox_app_server_protocol::McpServerElicitationRequestParams;
use ctox_hooks::HookEvent;
use ctox_hooks::HookEventAfterAgent;
use ctox_hooks::HookPayload;
use ctox_hooks::HookResult;
use ctox_hooks::Hooks;
use ctox_hooks::HooksConfig;
use ctox_network_proxy::NetworkProxy;
use ctox_network_proxy::NetworkProxyAuditMetadata;
use ctox_network_proxy::normalize_host;
use ctox_otel::current_span_trace_id;
use ctox_otel::current_span_w3c_trace_context;
use ctox_otel::set_parent_from_w3c_trace_context;
use ctox_protocol::ThreadId;
use ctox_protocol::approvals::ElicitationRequestEvent;
use ctox_protocol::approvals::ExecApprovalRequestSkillMetadata;
use ctox_protocol::approvals::ExecPolicyAmendment;
use ctox_protocol::approvals::NetworkPolicyAmendment;
use ctox_protocol::approvals::NetworkPolicyRuleAction;
use ctox_protocol::config_types::ApprovalsReviewer;
use ctox_protocol::config_types::ModeKind;
use ctox_protocol::config_types::Settings;
use ctox_protocol::config_types::WebSearchMode;
use ctox_protocol::dynamic_tools::DynamicToolResponse;
use ctox_protocol::dynamic_tools::DynamicToolSpec;
use ctox_protocol::items::PlanItem;
...
use ctox_protocol::models::format_allow_prefixes;
use ctox_protocol::openai_models::ModelInfo;
use ctox_protocol::permissions::FileSystemSandboxPolicy;
use ctox_protocol::permissions::NetworkSandboxPolicy;
use ctox_protocol::protocol::FileChange;
use ctox_protocol::protocol::HasLegacyEvent;
use ctox_protocol::protocol::ItemCompletedEvent;
use ctox_protocol::protocol::ItemStartedEvent;
use ctox_protocol::protocol::RawResponseItemEvent;
...
use ctox_protocol::protocol::TurnAbortReason;
use ctox_protocol::protocol::TurnContextItem;
use ctox_protocol::protocol::TurnContextNetworkItem;
use ctox_protocol::protocol::W3cTraceContext;
use ctox_protocol::request_permissions::PermissionGrantScope;
use ctox_protocol::request_permissions::RequestPermissionProfile;
use ctox_protocol::request_permissions::RequestPermissionsArgs;
use ctox_protocol::request_permissions::RequestPermissionsEvent;
use ctox_protocol::request_permissions::RequestPermissionsResponse;
use ctox_protocol::request_user_input::RequestUserInputArgs;
use ctox_protocol::request_user_input::RequestUserInputResponse;
use ctox_rmcp_client::ElicitationResponse;
use ctox_rmcp_client::OAuthCredentialsStoreMode;
use ctox_utils_stream_parser::AssistantTextChunk;
use ctox_utils_stream_parser::AssistantTextStreamParser;
use ctox_utils_stream_parser::ProposedPlanSegment;
use ctox_utils_stream_parser::extract_proposed_plan_text;
use ctox_utils_stream_parser::strip_citations;
...
use ctox_config::CONFIG_TOML_FILE;
...
use ctox_async_utils::OrCancelExt;
use ctox_otel::SessionTelemetry;
use ctox_otel::TelemetryAuthMode;
use ctox_otel::metrics::names::THREAD_STARTED_METRIC;
...
use ctox_utils_readiness::Readiness;
use ctox_utils_readiness::ReadinessFlag;
...
impl Codex {
    ...
    pub(crate) async fn spawn(args: CodexSpawnArgs) -> CodexResult<CodexSpawnOk> {
        let parent_trace = match args.parent_trace {
            Some(trace) => {
                if ctox_otel::context_from_w3c_trace_context(&trace).is_some() {
                    Some(trace)
                } else {
...
impl TurnContext {
    pub(crate) fn model_context_window(&self) -> Option<i64> {
        if let Some(configured_context_window) = self
            .config
            .model_context_window
            .filter(|context_window| *context_window > 0)
        {
            return Some(configured_context_window);
        }
...
impl Session {
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &ctox_execpolicy::Policy,
        sandbox_policy: &SandboxPolicy,
        network_policy_decider: Option<Arc<dyn ctox_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<Arc<dyn ctox_network_proxy::BlockedRequestObserver>>,
    ...
    async fn new(
        ...
        let auth_env_telemetry = collect_auth_env_telemetry(
            &session_configuration.provider,
            auth_manager.ctox_api_key_env_enabled(),
        );
        ...
        let sandbox_state = SandboxState {
            sandbox_policy: session_configuration.sandbox_policy.get().clone(),
            ctox_linux_sandbox_exe: config.ctox_linux_sandbox_exe.clone(),
        ...
        let session_start_source = match &initial_history {
            ...
            InitialHistory::New | InitialHistory::Forked(_) => {
                ctox_hooks::SessionStartSource::Startup
            }
        };
    ...
    async fn active_regular_turn_context(&self) -> Option<Arc<TurnContext>> {
        let active = self.active_turn.lock().await;
        active.as_ref().and_then(|turn| {
            turn.tasks
                .values()
                .find(|task| task.kind == TaskKind::Regular)
                .map(|task| Arc::clone(&task.turn_context))
        })
    }
    ...
    pub async fn request_mcp_server_elicitation(
        ...
        let request = match params.request {
            McpServerElicitationRequest::Form {
                ...
                ctox_protocol::approvals::ElicitationRequest::Form {
                    meta,
                    message,
                    requested_schema,
                }
            ...
            McpServerElicitationRequest::Url {
                ...
                url,
                elicitation_id,
            } => ctox_protocol::approvals::ElicitationRequest::Url {
                meta,
                message,
                url,
                elicitation_id,
        ...
        let id = match request_id {
            rmcp::model::NumberOrString::String(value) => {
                ctox_protocol::mcp::RequestId::String(value.to_string())
            }
            rmcp::model::NumberOrString::Number(value) => {
                ctox_protocol::mcp::RequestId::Integer(value)
            }
        };
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<ctox_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
    ...
    async fn refresh_mcp_servers_inner(
        ...
        let sandbox_state = SandboxState {
            ...
            ctox_linux_sandbox_exe: turn_context.ctox_linux_sandbox_exe.clone(),
...
mod handlers {
    ...
    use crate::compact::InitialContextInjection;
    use crate::compact::run_inline_interrupt_compact_task;
    ...
    use ctox_protocol::custom_prompts::CustomPrompt;
    use ctox_protocol::protocol::CodexErrorInfo;
    use ctox_protocol::protocol::ErrorEvent;
    use ctox_protocol::protocol::Event;
    use ctox_protocol::protocol::EventMsg;
    use ctox_protocol::protocol::ListCustomPromptsResponseEvent;
    use ctox_protocol::protocol::ListSkillsResponseEvent;
    use ctox_protocol::protocol::McpServerRefreshConfig;
    use ctox_protocol::protocol::Op;
    use ctox_protocol::protocol::ReviewDecision;
    use ctox_protocol::protocol::ReviewRequest;
    use ctox_protocol::protocol::RolloutItem;
    use ctox_protocol::protocol::SkillsListEntry;
    use ctox_protocol::protocol::ThreadNameUpdatedEvent;
    use ctox_protocol::protocol::ThreadRolledBackEvent;
    use ctox_protocol::protocol::TurnAbortReason;
    use ctox_protocol::protocol::WarningEvent;
    use ctox_protocol::request_permissions::RequestPermissionsResponse;
    use ctox_protocol::request_user_input::RequestUserInputResponse;
    ...
    use ctox_protocol::config_types::Settings;
    use ctox_protocol::dynamic_tools::DynamicToolResponse;
    use ctox_protocol::mcp::RequestId as ProtocolRequestId;
    use ctox_protocol::user_input::UserInput;
    use ctox_rmcp_client::ElicitationAction;
    use ctox_rmcp_client::ElicitationResponse;
    use serde_json::Value;
    use std::path::PathBuf;
    use std::sync::Arc;
    ...
    pub async fn resolve_elicitation(
        ...
        decision: ctox_protocol::approvals::ElicitationAction,
        content: Option<Value>,
        meta: Option<Value>,
    ) {
        let action = match decision {
            ctox_protocol::approvals::ElicitationAction::Accept => ElicitationAction::Accept,
            ctox_protocol::approvals::ElicitationAction::Decline => ElicitationAction::Decline,
            ctox_protocol::approvals::ElicitationAction::Cancel => ElicitationAction::Cancel,
        };
            ...
            let event = Event {
                ...
                msg: EventMsg::GetHistoryEntryResponse(
                    crate::protocol::GetHistoryEntryResponseEvent {
                        offset,
                        log_id,
                        entry: entry_opt.map(|e| ctox_protocol::message_history::HistoryEntry {
    ...
    pub async fn compact(sess: &Arc<Session>, sub_id: String) {
        if let Some(turn_context) = sess.active_regular_turn_context().await {
            let _ = sub_id;
            let _ = super::run_auto_compact(
                sess,
                &turn_context,
...
pub(crate) async fn run_turn(
    ...
    loop {
        if let Some(session_start_source) = sess.take_pending_session_start_source().await {
            ...
            .to_string();
            let session_start_request = ctox_hooks::SessionStartRequest {
        ...
        .await
        {
            Ok(sampling_request_output) => {
                ...
                if !needs_follow_up {
                    ...
                    .to_string();
                    let stop_request = ctox_hooks::StopRequest {
                        session_id: sess.conversation_id,
                        turn_id: turn_context.sub_id.clone(),
                        cwd: turn_context.cwd.clone(),
                        transcript_path: sess.current_rollout_path().await,
...
async fn run_pre_sampling_compact(
    ...
    let auto_compact_limit = turn_context
        ...
        .unwrap_or(i64::MAX);
    // Compact only after the total usage tokens have exceeded the auto compact limit.
...
fn agent_message_text(item: &ctox_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
        .map(|entry| match entry {
            ctox_protocol::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect()
}
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: ctox_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(ctox_protocol::items::AgentMessageItem {
...
async fn try_run_sampling_request(
    ...
    let outcome: CodexResult<SamplingRequestResult> = loop {
        ...
        {
            Ok(event) => event,
            Err(ctox_async_utils::CancelErr::Cancelled) => break Err(CodexErr::TurnAborted),
        };
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![ctox_protocol::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
