...
use ctox_protocol::protocol::RequestUserInputEvent;
        ...
        if let Some(files) = maybe_files {
            ...
            let patch = changes
                ...
                .map(|(path, change)| match change {
                    ctox_protocol::protocol::FileChange::Add { content } => {
                        format!("*** Add File: {}\n{}", path.display(), content)
                    }
                    ctox_protocol::protocol::FileChange::Delete { content } => {
                        format!("*** Delete File: {}\n{}", path.display(), content)
                    }
...
async fn maybe_auto_review_mcp_request_user_input(
    ...
    Some(RequestUserInputResponse {
        answers: HashMap::from([(
            question.id.clone(),
            ctox_protocol::request_user_input::RequestUserInputAnswer {
...
) -> ctox_protocol::protocol::ReviewDecision
where
    F: core::future::Future<Output = ctox_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            ...
            parent_session
                .notify_approval(approval_id, ctox_protocol::protocol::ReviewDecision::Abort)
                .await;
            ctox_protocol::protocol::ReviewDecision::Abort
        }
        decision = fut => {
            decision
        }
