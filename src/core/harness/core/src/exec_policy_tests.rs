...
use ctox_protocol::protocol::GranularApprovalConfig;
...
async fn merges_requirements_exec_policy_network_rules() -> anyhow::Result<()> {
    ...
    requirements_exec_policy.add_network_rule(
        "blocked.example.com",
        ctox_execpolicy::NetworkRuleProtocol::Https,
        Decision::Forbidden,
        None,
    )?;

    let requirements = ConfigRequirements {
        exec_policy: Some(ctox_config::Sourced::new(
            ctox_config::RequirementsExecPolicy::new(requirements_exec_policy),
            ctox_config::RequirementSource::Unknown,
        )),
        ..ConfigRequirements::default()
    };
