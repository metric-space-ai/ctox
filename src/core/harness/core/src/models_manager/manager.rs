...
impl RequestTelemetry for ModelsRequestTelemetry {
    fn on_request(
        ...
        tracing::event!(
            ...
            auth.env_ctox_api_key_present = self.auth_env.ctox_api_key_env_present,
            auth.env_ctox_api_key_enabled = self.auth_env.ctox_api_key_env_enabled,
...
impl ModelsManager {
    ...
    async fn fetch_and_update_models(&self) -> CoreResult<()> {
        let _timer =
            ctox_otel::start_global_timer("codex.remote_models.fetch_update.duration_ms", &[]);
        ...
        let auth_env = collect_auth_env_telemetry(
            &self.provider,
            self.auth_manager.ctox_api_key_env_enabled(),
        );
    ...
    async fn try_load_cache(&self) -> bool {
        let _timer =
            ctox_otel::start_global_timer("codex.remote_models.load_cache.duration_ms", &[]);
        let client_version = crate::models_manager::client_version_to_whole();
