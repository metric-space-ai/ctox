...
use ctox_state::BackfillState;
use ctox_state::BackfillStats;
use ctox_state::BackfillStatus;
use ctox_state::DB_ERROR_METRIC;
use ctox_state::DB_METRIC_BACKFILL;
use ctox_state::DB_METRIC_BACKFILL_DURATION_MS;
use ctox_state::ExtractionOutcome;
use ctox_state::ThreadMetadataBuilder;
use ctox_state::apply_rollout_item;
use std::path::Path;
...
pub(crate) async fn backfill_sessions(runtime: &ctox_state::StateRuntime, config: &Config) {
    let metric_client = ctox_otel::metrics::global();
