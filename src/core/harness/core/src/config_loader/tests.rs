...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let config_error = config_error_from_io(&err);
    let _guard = ctox_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
        ctox_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
            .expect("schema error");
    assert_eq!(config_error, &expected_config_error);
}
...
fn schema_error_points_to_feature_value() {
    ...
    let _guard = ctox_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let error = ctox_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
        .expect("schema error");
    ...
    let err = load_config_layers_state(
        ...
        CloudRequirementsLoader::new(async {
            Err(CloudRequirementsLoadError::new(
                ctox_config::CloudRequirementsLoadErrorCode::RequestFailed,
                None,
...
mod requirements_exec_policy_tests {
    ...
    use ctox_config::RequirementsExecPolicyDecisionToml;
    use ctox_config::RequirementsExecPolicyParseError;
    use ctox_config::RequirementsExecPolicyPatternTokenToml;
    use ctox_config::RequirementsExecPolicyPrefixRuleToml;
    use ctox_config::RequirementsExecPolicyToml;
