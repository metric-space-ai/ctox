...
use ctox_secrets::redact_secrets;
...
async fn claim_startup_jobs(
...
) -> Option<Vec<ctox_state::Stage1JobClaim>> {
    ...
    match state_db
        .claim_stage1_jobs_for_startup(
            ...
            ctox_state::Stage1StartupClaimParams {
...
async fn run_jobs(
    session: &Arc<Session>,
    claimed_candidates: Vec<ctox_state::Stage1JobClaim>,
...
mod job {
    ...
    pub(in crate::memories) async fn run(
        session: &Session,
        claim: ctox_state::Stage1JobClaim,
        stage_one_context: &RequestContext,
    ) -> JobResult {
    ...
    mod result {
        ...
        pub(in crate::memories) async fn failed(
            session: &Session,
            thread_id: ctox_protocol::ThreadId,
            ownership_token: &str,
            reason: &str,
        ) {
