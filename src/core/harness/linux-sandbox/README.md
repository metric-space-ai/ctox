...
- a `ctox-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the Codex CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `ctox-exec` CLI can check if its arg0 is `ctox-linux-sandbox` and, if so, execute as if it were `ctox-linux-sandbox`
