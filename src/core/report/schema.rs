...
pub fn ensure_schema(conn: &Connection) -> Result<()> {
    migrate_legacy_report_runs(conn);
...
fn migrate_legacy_report_runs(conn: &Connection) {
    let exists: bool = conn
        .query_row(
            "SELECT 1 FROM sqlite_master WHERE type='table' AND name='report_runs' LIMIT 1",
            [],
            |_| Ok(true),
        )
        .unwrap_or(false);
    ...
    // `src/report/store.rs` used the same table name for the older staged
    // report pipeline. Add the deep-research columns before SCHEMA_SQL tries
    // to create indexes over them. They stay nullable so old staged-report
    // rows remain readable by their original code.
    for (column, decl) in [
        ("report_type_id", "TEXT"),
        ("domain_profile_id", "TEXT"),
        ("depth_profile_id", "TEXT"),
        ...
        ("storyline_md", "TEXT"),
        ("storyline_set_at", "TEXT"),
    ] {
        migrate_add_column(conn, "report_runs", column, decl);
    }
}
