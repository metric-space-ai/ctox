...
fn write_title_block(out: &mut String, manuscript: &Manuscript) {
    ...
    if !manuscript.scope_disclaimer.trim().is_empty() {
        out.push('\n');
...
fn resolve_xrefs(
    ...
    out.push_str(rest);
    out = out.replace("Abbildung Abbildung ", "Abbildung ");
    out = out.replace("Tabelle Tabelle ", "Tabelle ");
    out = out.replace("Figure Figure ", "Figure ");
    out = out.replace("Table Table ", "Table ");
    out
}
