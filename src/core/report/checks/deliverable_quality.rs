...
use crate::report::render::manuscript::{build_manuscript, Manuscript, ManuscriptBlockKind};
...
pub fn run_deliverable_quality_check(workspace: &Workspace<'_>) -> Result<CheckOutcome> {
    ...
    if metadata.report_type_id == "project_description" {
        let provenance_phrases = [
            "öffentlich nachvollziehbarer gesellschaftsrechtlicher rahmen",
            "oeffentlich nachvollziehbarer gesellschaftsrechtlicher rahmen",
            "eigenständiger technologie- und serviceträger",
            "eigenstaendiger technologie- und servicetraeger",
            "auf basis der recherche",
            "auf basis der quellen",
            "nach dem vorliegenden kontext",
            "die recherche zeigt",
            "die quellen zeigen",
        ];
        for phrase in provenance_phrases {
            if all_lower.contains(phrase) {
                issues.push(Issue {
                    lint_id: "PROJECT-SOURCE-PROVENANCE-PROSE",
                    instance_id: None,
                    reason: format!(
                        "Die Projektbeschreibung enthaelt sichtbare Herkunfts-/Nachweisprosa: '{phrase}'."
                    ),
                    goal: "Formuliere aus Antragstellerperspektive: konkrete Unternehmens- und Projektlogik ohne Quellen-/Recherche-/Registerauskunftssprache.".to_string(),
                });
            }
        }
    }
    ...
    if metadata.report_type_id == "project_description" {
        if !workspace
            .has_provenance_kind("project_description_agent_brief")
            .unwrap_or(false)
        {
            issues.push(Issue {
                lint_id: "PROJECT-MISSING-FOERDERVORHABEN-AGENT-BRIEF",
                instance_id: None,
                reason: "Der project_description-Lauf hat keinen nachweisbaren Foerdervorhaben-Agent-Adaptervertrag erzeugt.".to_string(),
                goal: "Fuehre vor weiterer Draft-/Revision-Arbeit `ctox report flavor-brief --run-id RUN_ID` aus und nutze den generierten Foerdervorhaben-Agent Brief als primaeren Schreibvertrag.".to_string(),
            });
        }
        for (kind, label) in [
            ("project_description_app_cli_init", "App-CLI init"),
            (
                "project_description_app_cli_workspace_snapshot",
                "App-CLI workspace snapshot",
            ),
            (
                "project_description_app_cli_asset_lookup",
                "App-CLI asset lookup",
            ),
            ("project_description_app_cli_lint_run", "App-CLI run lint"),
        ] {
            if !workspace.has_provenance_kind(kind).unwrap_or(false) {
                issues.push(Issue {
                    lint_id: "PROJECT-MISSING-APP-CLI-STAGE",
                    instance_id: None,
                    reason: format!(
                        "Der project_description-Lauf hat keine Provenienz fuer die Pflichtstufe '{label}'."
                    ),
                    goal: "Fahre den portierten Foerdervorhaben-App-CLI-Pfad deterministisch: init, workspace-snapshot, asset-lookup, writer/revision prompts, lint-run und nach DOCX-Render lint-docx/reference-fit.".to_string(),
                });
            }
        }
        ...
        let scope_block = manuscript
            ...
            .flat_map(|d| d.blocks.iter())
            .find(|b| {
                b.block_id == "project_scope_budget_timeline"
                    || b.block_id == "project_1_costs_timeline"
            });
        ...
        let project_figure_count = manuscript.structured_figures.len();
        let min_project_figures = match metadata.depth_profile_id.as_str() {
            "brief" => 2,
            _ => 5,
        };
        if project_figure_count < min_project_figures {
            issues.push(Issue {
                lint_id: "PROJECT-REFERENCE-FIT-LOW-FIGURE-DENSITY",
                instance_id: Some("doc_project__project_intro".to_string()),
                reason: format!(
                    "Echte Projektbeschreibungen aus dem Referenzkorpus sind visuell gefuehrt; vorhanden sind nur {project_figure_count} eingebettete Abbildung(en), erwartet mindestens {min_project_figures}."
                ),
                goal: "Fuege mehrere client-faehige Abbildungen per ctox report figure-add hinzu: Unternehmens-/Standort- oder Produktvisual, Produkt-/Serienuebersicht, Zielbild/Architektur, Prozess-/Servicefluss und Umsetzungs-/Nutzenlogik. Jede Abbildung muss im Fliesstext sinnvoll eingefuehrt werden.".to_string(),
            });
        }
        let broken_project_figures = manuscript
            .structured_figures
            .iter()
            .filter(|figure| !Path::new(&figure.image_path).is_file())
            .count();
        if broken_project_figures > 0 {
            issues.push(Issue {
                lint_id: "PROJECT-BROKEN-FIGURE-PATH",
                instance_id: None,
                reason: format!(
                    "{broken_project_figures} Projektabbildung(en) zeigen auf nicht vorhandene Bilddateien."
                ),
                goal: "Repariere alle figure-add-Artefakte; jede registrierte Projektabbildung muss als Bilddatei existieren und ins DOCX eingebettet werden koennen.".to_string(),
            });
        }
        if manuscript.structured_tables.len() < 2 {
            issues.push(Issue {
                lint_id: "PROJECT-REFERENCE-FIT-LOW-TABLE-DENSITY",
                instance_id: None,
                reason: format!(
                    "Projektbeschreibungen brauchen mindestens Unternehmens-/Gesellschaftstabelle und Projektumfang-/Kostentabelle; vorhanden sind {} strukturierte Tabellen.",
                    manuscript.structured_tables.len()
                ),
                goal: "Erzeuge native Tabellen fuer Gesellschaft/Unternehmensdaten und Projektumfang mit Laufzeit, Status, Budget und Kostenbloecken via project-description-sync oder table-add.".to_string(),
            });
        }
        ...
        let project_meta_leak_terms = [
            "für diese förderunterlage genügt",
            "fuer diese foerderunterlage genuegt",
            "mehr registermaterial",
            "plausibilität des vorhabens",
            "plausibilitaet des vorhabens",
            "lizenzkonstruktion",
            "tritt als klar zuordenbare",
            "projektverantwortung eindeutig",
            "rechtlicher rahmen",
            "öffentlich nachvollziehbarer gesellschaftsrechtlicher rahmen",
            "oeffentlich nachvollziehbarer gesellschaftsrechtlicher rahmen",
            "eigenständiger technologie- und serviceträger",
            "eigenstaendiger technologie- und servicetraeger",
            "damit liegt ein klarer",
            ...
            "recherchebasis",
            "evidence-register",
            "fact-transfer-ledger",
            "foerdervorhaben-agent",
            "fördervorhaben-agent",
            "asset pack",
            "asset-pack",
            "app-derived",
            "flavor",
        ...
        for term in project_meta_leak_terms {
            if all_lower.contains(term) {
                issues.push(Issue {
                    lint_id: "PROJECT-PROVENANCE-PROSE-LEAK",
                    instance_id: None,
                    reason: format!(
                        "Die Projektbeschreibung enthaelt sichtbare Provenienz-/Tool- oder Pruefsprache: '{term}'."
                    ),
                    goal: "Entferne jeden Hinweis auf Recherche-, Tool-, App-, Adapter-, Evidence- oder Prueflogik aus dem Kundendokument. Uebersetze Quellenfakten in ruhige interne Unternehmens- und Projektprosa.".to_string(),
                });
            }
        }
        ...
        .iter()
        .filter(|term| all_lower.contains(**term))
        .count();
        if legal_profile_hits < 2 {
            issues.push(Issue {
                    lint_id: "PROJECT-MISSING-LEGAL-COMPANY-PROFILE",
                    instance_id: Some("doc_company__company_legal".to_string()),
                reason: "Die Projektbeschreibung bildet die in echten Foerdervorhabenbeschreibungen uebliche Gesellschafts-/Unternehmensprofil-Ebene nicht ausreichend ab.".to_string(),
                goal: "Ergaenze die Unternehmensausgangslage um gesellschaftsrechtliche und konkrete Unternehmensdaten wie Sitz/Anschrift, Register, Geschaeftsfuehrung, Gesellschafter, Historie, Produkte/Leistungen und Kundensegmente, soweit aus Recherche oder Referenzmaterial verfuegbar. Quellen bleiben unsichtbar.".to_string(),
            });
        }

        let reference_archetype_parts: [(&str, &[&str]); 5] = [
            (
                "Vorstellung des Innovations-/Digitalisierungsprojektes",
                &[
                    "vorstellung des innovationsprojektes",
                    "vorstellung des digitalisierungsprojektes",
                    "innovationsprojekt",
                    "digitalisierungsprojekt",
                ],
            ),
            (
                "Derzeitiger Stand / Problembereich",
                &[
                    "derzeitiger stand",
                    "aktueller stand",
                    "problembereich",
        ...
        for (label, terms) in reference_archetype_parts {
            if !terms.iter().any(|term| all_lower.contains(term)) {
                issues.push(Issue {
                    lint_id: "PROJECT-MISSING-REFERENCE-ARCHETYPE-PART",
                    instance_id: None,
                    reason: format!(
                        "Die Projektbeschreibung trifft den Referenz-Archetyp nicht: '{label}' ist nicht erkennbar."
                    ),
                    goal: "Baue die Projektbeschreibung wie eine echte Foerdervorhabenbeschreibung auf: Unternehmensprofil, Projektvorstellung, Derzeitiger Stand/Problembereich, Entwicklungsziel, Herausforderungen/Massnahmen, Projektkosten, Umsetzungszeitraum und wirtschaftlicher Nutzen.".to_string(),
                });
            }
        }
        ...
        let first_person_count = count_word_hits(
            &all_lower,
            &["wir", "uns", "unser", "unsere", "unserer", "unseren"],
        );
        if first_person_count < 2 {
            issues.push(Issue {
                lint_id: "PROJECT-WEAK-APPLICANT-PERSPECTIVE",
                instance_id: None,
                reason: "Die Projektbeschreibung nutzt kaum Wir-/Unternehmensperspektive und droht dadurch wie ein externer Bericht statt wie ein Unternehmensdossier zu klingen.".to_string(),
                goal: "Schreibe, wo passend, aus ruhiger Wir-Perspektive und sonst aus direkter interner Feststellungsperspektive. Vermeide distanzierende Gutachter- und Antragstellerinnen-Prosa.".to_string(),
            });
        }
        ...
        if let Some(block) = scope_block {
            ...
            if scope_table_expected && !has_scope_table {
                issues.push(Issue {
                    ...
                    goal: "Fuege eine echte Tabelle per ctox report project-description-sync oder table-add hinzu, gebunden an project_1_costs_timeline, mit mindestens Laufzeit, Status, Budget und Kostenbloecken.".to_string(),
                });
            }
            let scope_table_has_cost_blocks = table_for_instance_contains(
                &manuscript,
                &block.instance_id,
                &[
                    "kostenbl",
            ...
            let topic_mentions_cost_blocks = metadata.raw_topic.to_lowercase().contains("kostenbl")
                || all_lower.contains("kostenbl")
                || all_lower.contains("liqid5")
                || all_lower.contains("modellwechsel")
                || all_lower.contains("service-software")
                || all_lower.contains("umfinanzierung");
            if topic_mentions_cost_blocks && !scope_table_has_cost_blocks {
                issues.push(Issue {
                    lint_id: "PROJECT-MISSING-COST-BREAKDOWN-TABLE",
                    instance_id: Some(block.instance_id.clone()),
                    reason: "Der Projektumfang nennt Kostenbloecke oder projektbezogene Kostenbestandteile, aber die native Projektumfang-Tabelle enthaelt keine Kostenblock-/Einzelpositionszeile.".to_string(),
                    goal: "Fuehre `ctox report project-description-sync --run-id RUN_ID` nach stabiler Kostenprosa erneut aus oder registriere eine native Kostentabelle, in der die Kostenbloecke/Positionen explizit aufgefuehrt sind.".to_string(),
                });
            }
        }
        ...
        if let Some(block) = benefit_block {
            ...
            if lower.contains("projektplausibilität") || lower.contains("projektplausibilitaet") {
                issues.push(Issue {
                    lint_id: "PROJECT-PLAUSIBILITY-LINE-LEAK",
                    instance_id: Some(block.instance_id.clone()),
                    reason: "Der Schlussblock enthaelt eine sichtbare Projektplausibilitaets-/Foerderlogik-Prueferlinie.".to_string(),
                    goal: "Entferne sichtbare Pruefer-/Plausibilitaetssprache. Der wirtschaftliche Nutzen muss als Kundendossier formuliert sein: konkrete Wirkmechanismen, Verwertung und Nutzen, aber keine Gate- oder Gutachterformel.".to_string(),
                });
            }
...
fn count_word_hits(text_lower: &str, words: &[&str]) -> usize {
    words
        .iter()
        .map(|word| {
...
fn table_for_instance_contains(
    ...
    for table in &manuscript.structured_tables {
        if table.instance_id.as_deref() != Some(instance_id) {
            continue;
        }
        let mut text = table.headers.join(" ");
