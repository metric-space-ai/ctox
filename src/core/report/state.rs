...
pub fn create_run(root: &Path, params: CreateRunParams) -> Result<String> {
    ...
    if report_runs_has_column(&conn, "preset")? {
        conn.execute(
            "INSERT INTO report_runs (
                 ...
                 report_type_id, domain_profile_id, depth_profile_id, style_profile_id,
                 started_at, finished_at, raw_topic, package_summary_json
             ) VALUES (
                 ?1, ?2, ?3, ?4, ?5, NULL,
                 ?6, NULL, NULL, 1, ?7, ?7,
                 ?2, ?8, ?9, ?10,
        ...
        .context("failed to insert report run into legacy-compatible table")?;
    } else {
        conn.execute(
            "INSERT INTO report_runs (
...
fn report_runs_has_column(conn: &Connection, column: &str) -> Result<bool> {
    let mut stmt = conn.prepare("PRAGMA table_info(report_runs)")?;
