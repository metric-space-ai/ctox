...
/// imported into SQLite at service start. The `src/skills/system/`
...
const EMBEDDED_SYSTEM_SKILLS: Dir<'static> =
    include_dir::include_dir!("$CARGO_MANIFEST_DIR/src/skills/system");
...
pub fn install_source_pack(root: &Path, name: &str) -> Result<PathBuf> {
    ...
    .with_context(|| {
        format!(
            "failed to write pack origin marker for {}",
            target.display()
        )
    })?;
    Ok(target)
...
fn import_skill_tree(root: &Path, base: &Path) -> Result<()> {
    ...
    while let Some(dir) = queue.pop() {
        ...
        if path_matches_prefix(&dir, &root.join("src/skills/system"))
...
fn prune_legacy_disk_system_skills(root: &Path) -> Result<usize> {
    ...
    let system_root = root.join("src/skills/system");
