...
enum ImapState {
    ...
    Selected { username: String, mailbox_id: String, mailbox_name: String },
}
...
impl ImapServer {
    ...
    async fn handle_connection(&self, mut stream: TcpStream) -> StalwartResult<()> {
        ...
        stream.write_all(b"* OK [CAPABILITY IMAP4rev1] IMAP4rev1 Server Ready\r\n").await?;
        ...
        loop {
            ...
            while let Some(pos) = line_buffer.find('\n') {
                ...
                if tokens.len() < 2 {
                    stream.write_all(format!("{} BAD Missing command\r\n", tag).as_bytes()).await?;
                    continue;
                }
                ...
                match command.as_str() {
                    "CAPABILITY" => {
                        ...
                        stream.write_all(format!("{} OK CAPABILITY completed\r\n", tag).as_bytes()).await?;
                    }
                    "NOOP" => {
                        stream.write_all(format!("{} OK NOOP completed\r\n", tag).as_bytes()).await?;
                    }
                    "LOGOUT" => {
                        stream.write_all(b"* BYE IMAP4rev1 Server logging out\r\n").await?;
                        stream.write_all(format!("{} OK LOGOUT completed\r\n", tag).as_bytes()).await?;
                        return Ok(());
                    }
                    "LOGIN" => {
                        if args.len() < 2 {
                            stream.write_all(format!("{} BAD Missing username or password\r\n", tag).as_bytes()).await?;
                            continue;
                        }
                        let username = &args[0];
                        let password = &args[1];
                        if self.store.authenticate_user(username, password)? {
                            state = ImapState::Authenticated { username: username.clone() };
                            stream.write_all(format!("{} OK LOGIN completed\r\n", tag).as_bytes()).await?;
                        } else {
                            stream.write_all(format!("{} NO LOGIN failed: bad credentials\r\n", tag).as_bytes()).await?;
                        }
                    }
                    "LIST" => {
                        if let ImapState::Authenticated { ref username } | ImapState::Selected { ref username, .. } = state {
                            let mailboxes = self.store.get_mailboxes(username)?;
                            for (_, name) in mailboxes {
                                stream.write_all(format!("* LIST (\\HasNoChildren) \"/\" \"{}\"\r\n", name).as_bytes()).await?;
                            }
                            stream.write_all(format!("{} OK LIST completed\r\n", tag).as_bytes()).await?;
                        } else {
                            stream.write_all(format!("{} NO Authenticate first\r\n", tag).as_bytes()).await?;
                        }
                    }
                    "SELECT" => {
                        if args.is_empty() {
                            stream.write_all(format!("{} BAD Missing mailbox name\r\n", tag).as_bytes()).await?;
                            continue;
                        }
                        if let ImapState::Authenticated { ref username } | ImapState::Selected { ref username, .. } = state {
                            let mailbox_name = &args[0];
                            if let Some(mailbox_id) = self.store.get_mailbox_id(username, mailbox_name)? {
                                ...
                                stream.write_all(b"* OK [UIDVALIDITY 1] UIDs valid\r\n").await?;
                                ...
                                stream.write_all(format!("{} NO SELECT failed: no such mailbox\r\n", tag).as_bytes()).await?;
                            }
                        } else {
                            stream.write_all(format!("{} NO Authenticate first\r\n", tag).as_bytes()).await?;
                        }
                    ...
                    "FETCH" => {
                        if args.len() < 2 {
                            stream.write_all(format!("{} BAD Missing sequence set or query\r\n", tag).as_bytes()).await?;
                            continue;
                        }
                        if let ImapState::Selected { ref mailbox_id, .. } = state {
                            ...
                            for idx in indices {
                                ...
                                if query_upper.contains("INTERNALDATE") {
                                    fetch_res.push(format!("INTERNALDATE \"{}\"", format_imap_date(msg.7)));
                                }
                                if query_upper.contains("RFC822.SIZE") || query_upper.contains("BODY.SIZE") {
                                    let headers_part = msg.5.as_deref().unwrap_or("");
                                    let full_raw = if headers_part.is_empty() {
                                        format!("From: {}\r\nTo: {}\r\nSubject: {}\r\n\r\n{}", msg.1, msg.2, msg.3.as_deref().unwrap_or(""), msg.4)
                                    } else {
                                ...
                                if query_upper.contains("BODY") || query_upper.contains("RFC822") {
                                    ...
                                    if query_upper.contains("HEADER") {
                                        let headers_only = if headers_part.is_empty() {
                                            format!("From: {}\r\nTo: {}\r\nSubject: {}\r\n\r\n", msg.1, msg.2, msg.3.as_deref().unwrap_or(""))
                                        } else {
                                            format!("{}\r\n", headers_part.trim_end())
                                        };
                                        fetch_res.push(format!("BODY[HEADER] {{{}}}\r\n{}", headers_only.len(), headers_only));
                                    } else if query_upper.contains("TEXT") {
                                        fetch_res.push(format!("BODY[TEXT] {{{}}}\r\n{}", msg.4.len(), msg.4));
                                    } else {
                                        fetch_res.push(format!("BODY[] {{{}}}\r\n{}", full_raw.len(), full_raw));
                                    }
                                }

                                stream.write_all(format!("* {} FETCH ({})\r\n", seq_num, fetch_res.join(" ")).as_bytes()).await?;
                            }
                            stream.write_all(format!("{} OK FETCH completed\r\n", tag).as_bytes()).await?;
                        } else {
                            stream.write_all(format!("{} NO Select a mailbox first\r\n", tag).as_bytes()).await?;
                        }
                    }
                    "STORE" => {
                        if args.len() < 3 {
                            stream.write_all(format!("{} BAD Missing parameters for STORE\r\n", tag).as_bytes()).await?;
                            continue;
                        }
                        if let ImapState::Selected { ref mailbox_id, .. } = state {
                            let sequence_set = &args[0];
                            ...
                            for idx in indices {
                                ...
                                stream.write_all(format!("* {} FETCH (FLAGS ({}) UID {})\r\n", idx + 1, flag_str, idx + 1).as_bytes()).await?;
                            }
                            stream.write_all(format!("{} OK STORE completed\r\n", tag).as_bytes()).await?;
                        } else {
                            stream.write_all(format!("{} NO Select a mailbox first\r\n", tag).as_bytes()).await?;
                        }
                    }
                    "EXPUNGE" => {
                        stream.write_all(format!("{} OK EXPUNGE completed\r\n", tag).as_bytes()).await?;
                    }
                    _ => {
                        stream.write_all(format!("{} BAD Command not implemented\r\n", tag).as_bytes()).await?;
                    }
                }
            }
        }
