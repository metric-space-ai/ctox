...
impl Agent {
    ...
    pub(crate) fn send_binding_request(
        ...
        if local_index >= self.local_candidates.len() || remote_index >= self.remote_candidates.len()
        {
            warn!(
                "[{}]: skip STUN binding request with stale candidate index local={} remote={} local_len={} remote_len={}",
                self.get_name(),
                local_index,
                remote_index,
    ...
    pub(crate) fn handle_inbound(
        ...
        if m.typ.class == CLASS_SUCCESS_RESPONSE {
	                ...
	                match prflx_candidate_config.new_candidate_peer_reflexive() {
	                    Ok(prflx_candidate) => {
	                        if let Ok(added) = self.add_remote_candidate(prflx_candidate)
	                            && added
	                        {
	                            let Some(candidate_index) = self.remote_candidates.len().checked_sub(1) else {
	                                debug!(
	                                    "[{}]: prflx candidate was removed before indexing; dropping stale binding request from {}",
	                                    self.get_name(),
	                                    remote_addr
	                                );
	                                return Ok(());
	                            };
	                            remote_candidate_index = Some(candidate_index);
	                        }
	                    }
                    Err(err) => {
                        error!(
	        ...
	        if let Some(remote_index) = remote_candidate_index {
	            if let Some(remote_candidate) = self.remote_candidates.get_mut(remote_index) {
