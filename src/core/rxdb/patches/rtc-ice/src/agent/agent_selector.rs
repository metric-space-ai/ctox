...
impl ControllingSelector for Agent {
    ...
    fn ping_candidate(&mut self, local_index: usize, remote_index: usize) {
        if local_index >= self.local_candidates.len()
            || remote_index >= self.remote_candidates.len()
        {
            warn!(
                "skip controlling candidate ping with stale indexes local={} remote={} local_len={} remote_len={}",
                local_index,
                remote_index,
...
impl ControlledSelector for Agent {
    ...
    fn ping_candidate(&mut self, local_index: usize, remote_index: usize) {
        ...
        {
            warn!(
                "skip controlled candidate ping with stale indexes local={} remote={} local_len={} remote_len={}",
                local_index,
                remote_index,
                self.local_candidates.len(),
                self.remote_candidates.len()
