...
| `src/plugins/replication-webrtc/connection-handler-simple-peer.ts` | 10999 | T1 | `src/plugins/replication_webrtc/connection_handler_simple_peer.rs` | skipped | — | wave-039: skipped — simple-peer is a JS-native WebRTC adapter; CTOX uses webrtc-rs via N5 (WebRTCRsConnectionHandler) instead. Browser side keeps the upstream JS bundle which still uses simple-peer; CTOX never executes this file's logic |
...
| Upstream | Bytes | Tier | Rust target | Status | Owner | Notes |
|---|---:|:---:|---|---|---|---|
| `src/rx-query-single-result.ts` | 3580 | T3 | `src/rx_query_single_result.rs` | done | main | wave-039: data-only stub — RxQuerySingleResult with primary_path/docs_data/count/time + docs_data()/docs_data_map()/doc_count() accessors. 3 unit tests. Deferred to phase-6: documents (RxDocument materialization via documentCreator), docs_map, getValue(throw_if_missing) which needs RxQuery.op discrimination |
...
| `src/query-cache.ts` | 4369 | T1 | `src/query_cache.rs` | done | main | wave-039: full port — QueryCache (HashMap by string_rep), QueryCacheable trait (subscriber_count/last_ensure_equal/creation_time/mark_uncached), get_by_query+uncache_rx_query+count_rx_query_subscribers, default_cache_replacement_policy_monad (closure-based) with f64-time semantics. 4 unit tests covering hit/noop/eviction/subscriber-preservation. T1 deviation: QueryCacheable trait lets RxQuery (phase-6) plug in without re-doing the cache. Deferred: trigger_cache_replacement (needs RxCollection scheduler) |
