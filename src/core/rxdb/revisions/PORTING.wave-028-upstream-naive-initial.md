...
| `src/replication-protocol/downstream.ts` | 21692 | T1 | `src/replication_protocol/downstream.rs` | wip | main | naive initial-sync (resync-only) ported: initial_checkpoint write, paginated master_changes_since loop, persist_from_master with insert-only path (existing docs skipped — conflict resolution deferred), set_checkpoint("down"). Deferred: masterChangeStream subscription for ongoing sync, addNewTask/streamQueue task pattern, meta-doc tracking (get_assumed_master_state, get_meta_write_row), conflict resolution on updates, nonPersistedFromMaster aggregation queue |
| `src/replication-protocol/upstream.ts` | 21852 | T1 | `src/replication_protocol/upstream.rs` | wip | main | naive initial-sync ported: initial_checkpoint write, paginated get_changed_documents_since_via_query on fork, persist_to_master via replication_handler.master_write (with naive assumed_master_state=None), set_checkpoint("up"). Deferred: fork.changeStream subscribe for ongoing push, masterChangeStream subscribe with active-up race protection, streamQueue.up task pattern, get_assumed_master_state + get_meta_write_row tracking, conflict-resolution retry on returned conflicts, waitBeforePersist throttle, push.batchSize strict batching |
