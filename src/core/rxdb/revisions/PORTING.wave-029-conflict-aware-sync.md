...
| `src/replication-protocol/downstream.ts` | 21692 | T1 | `src/replication_protocol/downstream.rs` | wip | main | **conflict-aware initial-sync** ported: initial_checkpoint write, paginated master_changes_since, persist_from_master with full 4-case decision (insert / skip-if-already-equal / fast-forward / true-conflict-resolve via conflict_handler.resolve), parallel fork+assumed-master reads (tokio::join!), meta-doc writes via get_meta_write_row to meta_instance, set_checkpoint("down"). Deferred: masterChangeStream subscription for ongoing sync, addNewTask/streamQueue task pattern, nonPersistedFromMaster aggregation queue |
| `src/replication-protocol/upstream.ts` | 21852 | T1 | `src/replication_protocol/upstream.rs` | wip | main | **conflict-aware initial-sync** ported: initial_checkpoint write, paginated get_changed_documents_since_via_query on fork, persist_to_master with assumed-master tracking via get_assumed_master_state, skip-if-fork-equal-to-assumed-master optimization, replication_handler.master_write with assumed_master_state populated, meta-doc writes via get_meta_write_row after successful push, conflict-id detection on returned conflicts, set_checkpoint("up") always. Deferred: fork.changeStream subscribe for ongoing push, masterChangeStream subscribe with active-up race protection, streamQueue.up task pattern, conflict-retry via resolve_conflict_error + re-push, waitBeforePersist throttle, push.batchSize strict batching |
