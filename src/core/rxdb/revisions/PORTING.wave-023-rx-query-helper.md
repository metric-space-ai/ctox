...
| N2 | `attachments_stub` — no-op `fillWriteDataForAttachmentsChange` | `src/plugins/attachments/stub.rs` | T3 | pending | — | replication-protocol imports it; parquet handled out-of-band |
...
| N5 | WebRTC connection handler on `webrtc-rs` (replaces `simple-peer` port) | `src/plugins/replication_webrtc/connection_handler_rs.rs` | T1 | pending | — | API mismatch; not a port |
| N6 | Signaling client matching browser URL contract | `src/plugins/replication_webrtc/signaling_client.rs` | T1 | pending | — | must match what business-os's signaling expects |
...
| N8 | NPM mini-deps in Rust: `custom-idle-queue`, `oblivious-set`, `array-push-at-sort-position` | `src/util/*.rs` | T2 | pending | — | used by `rx-database`, query-planner |
...
| N17 | Core type foundation (schema, document, checkpoint, hash) + phase-1 query types + **phase-2 storage types** (BulkWriteRow, RxStorageWriteError, RxStorageBulkWriteResponse, RxStorageChangeEvent, EventBulk, RxStorageQueryResult, RxStorageCountResult, RxStorageChangedDocumentsSinceResult, RxStorageInstanceCreationParams, **RxStorageInstance trait**, PreparedQuery) | `src/types/{mod,util,schema,document,checkpoint,hash,query,storage}.rs` | T1 | done | main | RxStorageInstance is async_trait + Send+Sync |
| N11 | Type stub for skipped `attachments` plugin (`RxAttachment*` types referenced by replication) | `src/plugins/attachments/types_stub.rs` | T3 | pending | — | V1 leak fix |
| N12 | Type stub for skipped `migration-schema` plugin (`RxMigrationState` type) | `src/plugins/migration_schema/types_stub.rs` | T3 | pending | — | V1 leak fix |
| N13 | Type stub for skipped `backup` plugin (`RxBackupState` type) | `src/plugins/backup/types_stub.rs` | T3 | pending | — | V1 leak fix |
| N14 | Type stub for skipped `pipeline` plugin (`RxPipeline*` types) | `src/plugins/pipeline/types_stub.rs` | T3 | pending | — | V1 leak fix |
| N15 | `event-reduce` stub returns `null` (always full re-execute) | already in phase-5 port; just port shape | T2 | pending | — | drop `event-reduce-js` dep, behaviorally correct |
...
| `src/plugins/storage-memory/rx-storage-instance-memory.ts` | 16609 | T1 | `src/plugins/storage_memory/rx_storage_instance_memory.rs` | wip | main | RxStorageInstance trait impl. bulk_write + find_documents_by_id + cleanup + change_stream + remove + close DONE. query/count return RxError::STO16 until rx_query_helper (phase-6 → get_query_matcher + get_sort_comparator) is ported. RxStorageMemory factory with collection_states. T1 deviations: synchronous persistence (no requestIdleCallback); OPEN_MEMORY_INSTANCES test-hook omitted. Also: categorize_bulk_write_rows added to rx_storage_helper.rs |
| `src/rx-storage-helper.ts` | 37030 | T1 | `src/rx_storage_helper.rs` | wip | main | partial port — essential helpers ported (get_single_document, write_single, stack_checkpoints, throw_if_is_storage_write_error, flat_clone_doc_with_meta, ensure_rx_storage_instance_params_are_correct, has_encryption, get_written_documents_from_bulk_write_response + attachment helpers). Deferred: observeSingle (RxJS pipe ops), categorizeBulkWriteRows (attachment types), getWrappedStorageInstance (RxDatabase), getChangedDocumentsSince* (rx_query_helper), randomDelayStorage (test helper) |
...
| `src/replication-protocol/index.ts` | 12372 | T1 | `src/replication_protocol/index_mod.rs` | wip | main | 4 control helpers ported: await_first_in_sync (tokio::select! over BehaviorSubjects), await_in_sync (lock all queues), await_idle (double-drain), cancel. State types massively extended (ActiveSubjects, ProcessedSubjects, FirstSyncDone, StreamQueue, ReplicationStats). Deferred: replicate_rx_storage_instance + rx_storage_instance_to_replication_handler (need upstream.rs + downstream.rs) |
...
| Upstream | Bytes | Tier | Rust target | Status | Owner | Notes |
|---|---:|:---:|---|---|---|---|
| `src/plugins/replication-webrtc/webrtc-helper.ts` | 1498 | T1 | `src/plugins/replication_webrtc/webrtc_helper.rs` | pending | — | WebRTC transport — primary (simple-peer handler replaced by webrtc-rs, see N5) |
| `src/plugins/replication-webrtc/webrtc-types.ts` | 2977 | T1 | `src/plugins/replication_webrtc/webrtc_types.rs` | pending | — | WebRTC transport — primary (simple-peer handler replaced by webrtc-rs, see N5) |
| `src/plugins/replication-webrtc/signaling-server.ts` | 5704 | T1 | `src/plugins/replication_webrtc/signaling_server.rs` | pending | — | WebRTC transport — primary (simple-peer handler replaced by webrtc-rs, see N5) |
...
| `src/plugins/replication-webrtc/index.ts` | 11079 | T1 | `src/plugins/replication_webrtc/index.rs` | pending | — | WebRTC transport — primary (simple-peer handler replaced by webrtc-rs, see N5) |

## phase-5

| Upstream | Bytes | Tier | Rust target | Status | Owner | Notes |
|---|---:|:---:|---|---|---|---|
| `src/event-reduce.ts` | 5718 | T2 | `src/event_reduce.rs` | pending | — | V1 fix — imported by rx-query; port as stub (always full re-execute, see N15) |
