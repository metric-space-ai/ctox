...
| N17 | Core type foundation (`RxJsonSchema`, `RxDocumentData`, `BulkWriteRow`, `EventBulk`, `HashFunction`, `MaybeReadonly` aliases) | `src/types/*.rs` | T1 | pending | — | upstream `types/*.d.ts` is type-only; prerequisite for phase-1 |
