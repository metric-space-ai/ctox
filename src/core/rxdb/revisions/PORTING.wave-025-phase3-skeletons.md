...
| `src/replication-protocol/downstream.ts` | 21692 | T1 | `src/replication_protocol/downstream.rs` | wip | main | skeleton-only: start_replication_downstream sets first_sync_done.down=true and warns. Real state machine (master_change_stream subscribe, paginated master_changes_since, bulk_write into fork, conflict resolve, set_checkpoint("down")) deferred to follow-up wave |
| `src/replication-protocol/upstream.ts` | 21852 | T1 | `src/replication_protocol/upstream.rs` | wip | main | skeleton-only: start_replication_upstream sets first_sync_done.up=true and warns. Real state machine (fork.changeStream subscribe, paginated initial sync, persistence queue, conflict retry, master_write) deferred to follow-up wave |
