...
| N3 | `rxjs_compat` — `Subject`/`Observable` → tokio broadcast/watch + tokio-stream | `src/rxjs_compat.rs` | T1 | pending | — | foundation for all T1 reactive modules |
...
| N16 | Minimal Mango-query Rust port (replaces upstream `mingo` NPM package) | `src/util/mango/*.rs` | T1 | pending | — | only ops actually used by storage/replication; prerequisite for phase-1 `rx-query-mingo` |
| N17 | Core type foundation (`PrimaryKey`/`CompositePrimaryKey`, `JsonSchema`, `RxJsonSchema`, `RxDocumentData`, `RxDocumentMeta`, `RxStorageDefaultCheckpoint`, `HashFunction`, `DeepReadonly`/`MaybeReadonly`/`StringKeys` aliases) | `src/types/{mod,util,schema,document,checkpoint,hash}.rs` | T1 | done | main | minimum set for phase-1; BulkWriteRow/EventBulk added when phase-2 needs them |
...
| `src/custom-index.ts` | 12134 | T3 | `src/custom_index.rs` | pending | — | required by query-planner (load-bearing for storage) |
