...
| `src/plugins/replication/index.ts` | 25090 | T1 | `src/plugins/replication/index_mod.rs` | wip | main | **skeleton port**: ReplicationOptions/PullOptions/PushOptions/PullHandlerResult types, PullHandler/PushHandler/DocumentModifier/StreamFactory closure types, RxReplicationState struct with 5 reactive subjects (received/sent/error/canceled/active), replicate_rx_collection async constructor, cancel() functional, on_cancel callback list. **start() is stub** — needs an RxStorageFactory abstraction (each storage backend exposes create_storage_instance but no shared trait yet). Deferred: retry/awaitRetry, leader-election gating actually wired, browser-visibility toggle, addConnectedStorageToCollection, real replicate_rx_storage_instance invocation |
...
| `src/plugins/replication-webrtc/index.ts` | 11079 | T1 | `src/plugins/replication_webrtc/index_mod.rs` | wip | main | **master-path skeleton**: replicate_web_rtc async fn + RxWebRTCReplicationPool struct. Token-handshake answerer, connect-stream listener, is_master_in_webrtc_replication picks role, master path spawns master_change_stream relay + message dispatcher (calls master_changes_since / master_write on local handler). Fork path is `tracing::warn!` TODO — needs replicateRxCollection from plugins/replication (T1, 25 KB, unported) |
