...
function assertRequiredMarkers() {
  ...
  assertText('src/webrtc-native.mjs', [
    ...
    ['ctox protocol response result discriminator', /hasOwnProperty\.call\(payload,\s*'result'\)/],
    ['ctox protocol response error discriminator', /hasOwnProperty\.call\(payload,\s*'error'\)/],
    ['ctox protocol response result resolution', /pending\.resolve\(payload\.result\)/],
    ['ctox protocol response error rejection', /pending\.reject\(payload\.error\)/],
