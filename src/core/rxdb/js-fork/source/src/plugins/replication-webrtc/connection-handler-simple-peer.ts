...
function sendMessage(ws: WebSocket, msg: PeerMessage): boolean {
...
function isRecoverableSignalError(error: any): boolean {
    ...
    return haystack.includes('ERR_SET_REMOTE_DESCRIPTION') ||
        haystack.includes('ERR_ADD_ICE_CANDIDATE') ||
        haystack.includes('Called in wrong state: stable') ||
        haystack.includes('remote description was null');
}

function toPeerLifecycleError(error: any): any {
    if (!isRecoverableSignalError(error)) {
        return error;
    }
    const message = String(error?.message || error || 'Recoverable WebRTC peer signal race');
    const lifecycleError: any = new Error(message);
