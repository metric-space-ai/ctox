use {
    crate::terminal_emulator::{
        ...
        TerminalStyledLine, TerminalStyledRun, DEFAULT_TERMINAL_SCROLLBACK_LINES, TERMINAL_ANSI_16,
        TERMINAL_ANSI_DIM_8, TERMINAL_BRIGHT_FG, TERMINAL_CURSOR, TERMINAL_DEFAULT_BG,
        TERMINAL_DEFAULT_FG, TERMINAL_DIM_FG,
    },
    alacritty_terminal::{
...
pub fn process_terminal_bytes(
    emulator: &Arc<Mutex<crate::terminal_emulator::TerminalEmulator>>,
...
pub(crate) fn snapshot_output(term: &Term<AlacrittyEventListener>) -> String {
    ...
    let bottom_line = top_line + grid.screen_lines() as i32 - 1;
    let start = Point::new(Line(top_line), Column(0));
    let end = Point::new(Line(bottom_line), term.last_column());
...
fn resolve_cell_color(cell: &Cell, colors: &Colors) -> StyledColor {
    ...
    if cell.flags.contains(Flags::BOLD) {
        fg = brighten_terminal_color(fg);
    }
    if cell.flags.contains(Flags::DIM) {
        fg = dim_terminal_color(fg);
    }

    StyledColor { fg, bg }
}

fn brighten_terminal_color(color: u32) -> u32 {
    for index in 0..8 {
        if color == TERMINAL_ANSI_16[index] {
            return TERMINAL_ANSI_16[index + 8];
        }
    }
    color
}
...
fn dim_terminal_color(color: u32) -> u32 {
    for index in 0..8 {
        if color == TERMINAL_ANSI_16[index] {
            return TERMINAL_ANSI_DIM_8[index];
        }
    }
    color
}
...
fn finalize_styled_line(mut cells: Vec<TerminalStyledCell>) -> TerminalStyledLine {
