...
use crate::installations::{
    InstallChannel, RemoteAccessSettings, RemoteHostTarget, RemoteInstanceSource,
};
...
fn run_inner(request: &ProvisionRequest, tx: &Sender<ProvisionEvent>) -> Result<String> {
    ...
    match request.remote.host_target {
        ...
        RemoteHostTarget::Unspecified => {
            bail!("Please choose whether CTOX should run on this computer or another host")
        }
    }
}
...
fn provision_ssh(request: &ProvisionRequest, tx: &Sender<ProvisionEvent>) -> Result<String> {
    ...
    status(
        tx,
        &format!("[1/5] Source archive ready ({archive_size} MB)"),
    );

    // ── Step 2: Upload via sshpass + scp ──
    status(
        tx,
        &format!("[2/5] Uploading {archive_size} MB to {ssh_host}..."),
    );
    run_streaming(
        Command::new("sshpass")
            .arg("-p")
    ...
    status(
        tx,
        "[4/5] Building CTOX on remote machine (this may take 15-30 minutes on a fresh system)...",
    );
...
fn run_ssh_streaming(
    ...
    let exit_status = child
        .wait()
        .with_context(|| format!("waiting for SSH: {label}"))?;
    if !exit_status.success() {
        bail!("{label} failed with exit status {exit_status}");
    }
    Ok(())
