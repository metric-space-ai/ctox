...
pub struct InstallationMessage {
    #[serde(default)]
    pub id: String,
    #[serde(default)]
    pub sender: String,
...
pub struct InstallationUser {
    ...
    #[serde(default)]
    pub email: String,
    #[serde(default = "default_installation_role")]
    pub role: String,
    #[serde(default = "default_user_active")]
    pub active: bool,
}

impl Default for InstallationUser {
    fn default() -> Self {
        Self {
            id: String::new(),
            username: String::new(),
            email: String::new(),
            role: default_installation_role(),
            active: true,
        }
    }
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct InstallationDesktopState {
    #[serde(default = "default_installation_role")]
    pub role: String,
    #[serde(default)]
    ...
    #[serde(default)]
    pub messages: Vec<InstallationMessage>,
    #[serde(default)]
    pub users: Vec<InstallationUser>,
}

impl Default for InstallationDesktopState {
    fn default() -> Self {
        Self {
            role: default_installation_role(),
...
pub struct InstallationRegistry {
    ...
    /// Desktop-only mockup state keyed by installation id. This keeps
    /// `Installation::name` as the canonical model field while allowing
    /// optional per-desktop presentation and login state to persist.
    #[serde(default)]
    pub desktop: BTreeMap<String, InstallationDesktopState>,
}
...
impl InstallationRegistry {
    pub fn load() -> Result<Self> {
        ...
        let raw = fs::read_to_string(&path).with_context(|| {
            format!(
                "failed to read installation registry from {}",
                path.display()
            )
        })?;
        let mut registry: Self = serde_json::from_str(&raw).with_context(|| {
            format!(
                "failed to parse installation registry from {}",
                path.display()
            )
        })?;
        registry.normalize_desktop_state();
        Ok(registry)
    }
    ...
    pub fn save(&self) -> Result<()> {
        ...
        fs::write(&path, format!("{raw}\n")).with_context(|| {
            format!(
                "failed to write installation registry to {}",
                path.display()
            )
        })
    }
    ...
    pub fn add_installation_path(&mut self, root_path: PathBuf) -> Result<Installation> {
        ...
        let canonical = root_path.canonicalize().unwrap_or(root_path.clone());
        ...
        self.desktop.entry(installation.id.clone()).or_default();
    ...
    pub fn remove(&mut self, installation_id: &str) {
        self.installations
            .retain(|entry| entry.id != installation_id);
        self.desktop.remove(installation_id);
    }

    fn normalize_desktop_state(&mut self) {
        let installation_ids: Vec<String> = self
            .installations
            .iter()
            .map(|installation| installation.id.clone())
            .collect();
        self.desktop
            .retain(|installation_id, _| installation_ids.contains(installation_id));
        for installation_id in installation_ids {
            self.desktop.entry(installation_id).or_default();
        }
    }
}
...
pub fn validate_installation_root(path: &Path) -> Result<()> {
    let installed_binary = path.join("bin/ctox");
    ...
    if !cargo.is_file() {
        anyhow::bail!(
            "{} is not a CTOX install root: missing bin/ctox or Cargo.toml",
            path.display()
        );
    }
    if !main_rs.is_file() {
        anyhow::bail!(
            "{} is not a CTOX install root: missing src/main.rs",
            path.display()
        );
    }
    Ok(())
...
fn default_installation_role() -> String {
    "User".to_owned()
}
