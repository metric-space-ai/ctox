...
export async function mount(ctx) {
  ...
  ctx.host.dataset.atsModule = 'consent';
  ...
  async function onSubmit(event) {
    ...
    const now = Date.now();
    const record = Object.assign({ id: 'cons_' + now + '_' + Math.round(now % 1e6), created_at_ms: now, updated_at_ms: now, _deleted: false }, { subject_id: f.subject_id, purpose: f.purpose, legal_basis: f.legal_basis || 'consent', granted_at_ms: now });
    try { await col.insert(record); formEl.reset(); await render(); }
    catch (e) { console.error('[consent] insert failed:', e); if (gateEl) gateEl.textContent = 'Fehler: ' + (e?.message || e); }
  }
