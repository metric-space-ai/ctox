...
let writes = 0, reads = 0, deny = false;
...
const dispose = await mount({
  ...
  sync: { startCollection: async () => {}, requestNative: async (method, params) => {
    reads += 1;
...
document.querySelector('#verify').addEventListener('click', () => {
  evidence.textContent = JSON.stringify(records) === baseline && writes === 0
    ? `PASS: stored fixture unchanged; zero mutations; ${reads} explicit reads.` : 'FAIL: fixture changed';
});
