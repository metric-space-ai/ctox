...
export function createSyncRuntime({ db, config, onDiagnostic }) {
  ...
  const stopAllBridges = async () => {
    ...
    httpBridges.clear();
    const httpStates = await Promise.allSettled(httpBridgePromises);
  ...
  const syncRuntime = {
    ...
    async startCollection(collection) {
      ...
      try {
        ...
        if (httpBridge) return httpBridge;
        throw error;
      }
    },
...
async function startWebRtcReplication({ db, config, collection, recordCollection, onFatalPeerError }) {
  ...
  const rxdb = db?.rxdb || await import('../rxdb/dist/ctox-rxdb-js.mjs?v=20260529-mux-phase4');
