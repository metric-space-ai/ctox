...
const APP_BUILD = '20260527-peer-filter8';
...
async function loadBusinessDbModule() {
  if (!businessDbModulePromise) {
    businessDbModulePromise = importBusinessOsModule('./shared/db.js?v=20260527-peer-filter8', 'business db')
      .then((mod) => {
...
async function loadSyncModule() {
  if (!syncModulePromise) {
    syncModulePromise = importBusinessOsModule('./shared/sync.js?v=20260527-peer-filter8', 'business sync')
      .then((mod) => {
        syncModule = mod;
        return mod;
      });
