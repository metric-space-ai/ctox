...
async function main() {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "ctox-electron-session-smoke-"));
  ...
  assert.equal(payload.betaRead.localStorage, "beta");
  assert.equal(payload.alphaRead.indexedDb, "alpha");
  assert.equal(payload.betaRead.indexedDb, "beta");
}
...
function runElectron(command, args, resultPath) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      ...
      clearInterval(resultPoll);
      child.kill("SIGKILL");
      reject(new Error(`electron session isolation smoke timed out\nstdout:\n${stdout}\nstderr:\n${stderr}`));
    }, 60000);
