    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        match tool_name.as_str() {
            ...
            "meeting_get_transcript" => {
                ...
                // Read the transcript file from the session directory
                command
                    .arg("meeting")
                    .arg("status");
                // We'll parse the session to get transcript from the JSON output
                // The CLI returns session details including transcript path
            }
            "meeting_send_chat" => {
