...
COLLECTOR_SCRIPT = SCRIPT_DIR / "security_collect.py"
STORE_SCRIPT = SCRIPT_DIR / "security_store.py"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run security-posture collectors and persist raw captures into shared SQLite.")
    ...
    for flag, value in (("--unit", args.unit), ("--cert-path", args.cert_path), ("--scan-path", args.scan_path), ("--host", args.host), ("--port", args.port)):
        if value:
    ...
    print(json.dumps({"skill_key": "security_posture", "run_id": stored.get("run_id") or raw.get("run_id"), "capture_ids": stored.get("capture_ids", []), "collector": args.collector or "run-all", "raw": raw}, indent=2))
    return 0
