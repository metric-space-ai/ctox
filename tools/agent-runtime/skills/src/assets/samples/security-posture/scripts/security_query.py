...
def main() -> int:
    parser = argparse.ArgumentParser(description="Query security_posture state from the shared SQLite kernel.")
    ...
    if args.command == "summary":
        print(json.dumps(shared.summary(conn, "security_posture"), indent=2))
        return 0
    print(json.dumps(shared.export_cytoscape(conn, "security_posture"), indent=2))
    return 0
