...
COLLECTOR_SCRIPT = SCRIPT_DIR / "ops_collect.py"
STORE_SCRIPT = SCRIPT_DIR / "ops_store.py"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run ops-insight collectors and persist raw captures into shared SQLite.")
    parser.add_argument("--db", required=True)
    parser.add_argument("--collector")
    parser.add_argument("--kernel-db")
    ...
    if args.kernel_db:
        collect.extend(["--db-path", args.kernel_db])
    ...
    print(json.dumps({"skill_key": "ops_insight", "run_id": stored.get("run_id") or raw.get("run_id"), "capture_ids": stored.get("capture_ids", []), "collector": args.collector or "run-all", "raw": raw}, indent=2))
    return 0
