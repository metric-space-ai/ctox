...
COLLECTOR_SCRIPT = SCRIPT_DIR / "incident_collect.py"
STORE_SCRIPT = SCRIPT_DIR / "incident_store.py"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run incident collectors and persist raw captures into shared SQLite.")
    ...
    if args.unit:
        collect.extend(["--unit", args.unit])
    if args.url:
        collect.extend(["--url", args.url])
    if args.host:
        collect.extend(["--host", args.host])
    if args.repo_root:
    ...
    stored = run_json(
        ...
        stdin_text=json.dumps(raw),
    )
    print(json.dumps({"skill_key": "incident_response", "run_id": stored.get("run_id") or raw.get("run_id"), "capture_ids": stored.get("capture_ids", []), "collector": args.collector or "run-all", "raw": raw}, indent=2))
    return 0
