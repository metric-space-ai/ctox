...
BOOTSTRAP_PATH = pathlib.Path(__file__).with_name("recovery_bootstrap.py")
...
class RecoveryBootstrapTests(unittest.TestCase):
    def test_build_graph_creates_recovery_entities(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            STORE_MODULE.store_capture(
                conn,
                {
                    "skill_key": "recovery_assurance",
                    "run_id": "run-recovery-001",
                    "collectors": [
                        {
                            "skill_key": "recovery_assurance",
                            "run_id": "run-recovery-001",
                            "collector": "filesystem_backup",
                            "captures": [
                                {
                                    "tool": "tar",
                                    "argv": ["tar", "-tf", "/tmp/backup.tar"],
                                    "available": True,
                                    "stdout": "etc/hosts\n",
                                    "stderr": "",
                                    "exit_code": 0,
            ...
            graph = BOOTSTRAP_MODULE.build_graph(conn, "run-recovery-001")
            kinds = {item["kind"] for item in graph["entities"]}
            self.assertIn("backup_coverage", kinds)
            self.assertIn("restore_evidence", kinds)
            self.assertIn("rpo_gap", kinds)
            self.assertIn("dr_runbook", kinds)
