...
COLLECTOR_SCRIPT = SCRIPT_DIR / "recovery_collect.py"
STORE_SCRIPT = SCRIPT_DIR / "recovery_store.py"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run recovery-assurance collectors and persist raw captures into shared SQLite.")
    parser.add_argument("--db", required=True)
    parser.add_argument("--collector")
    parser.add_argument("--backup-unit")
    ...
    for flag, value in (("--backup-unit", args.backup_unit), ("--backup-path", args.backup_path), ("--repo", args.repo), ("--dump-path", args.dump_path)):
        if value:
            collect.extend([flag, value])
    ...
    print(json.dumps({"skill_key": "recovery_assurance", "run_id": stored.get("run_id") or raw.get("run_id"), "capture_ids": stored.get("capture_ids", []), "collector": args.collector or "run-all", "raw": raw}, indent=2))
    return 0
