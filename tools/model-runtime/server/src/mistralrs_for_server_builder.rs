//! ## engine.rs instance for server builder.
...
async fn resolve_pending_isq_layers_for_pipeline(pipeline: &LoadedPipeline) -> Result<()> {
    let should_resolve = std::env::var("ENGINE_RESOLVE_PENDING_ISQ_AT_STARTUP")
        .ok()
        .map(|raw| {
    ...
    if !should_resolve {
        ...
        warn!(
            "Skipping eager pending immediate-ISQ resolution for `{pipeline_name}` due to ENGINE_RESOLVE_PENDING_ISQ_AT_STARTUP."
        );
        return Ok(());
    }
    let mut guard = pipeline.lock().await;
    let pipeline_name = guard.name();
    let resolved = guard.resolve_pending_isq_layers()?;
    if resolved > 0 {
        info!(
            "Resolved {resolved} pending immediate-ISQ layers for `{pipeline_name}` before serving."
        );
    }
    Ok(())
}
...
pub mod defaults {
    ...
    // Provides the default values used for the engine.rs instance for server.
    ...
    pub const MODEL: Option<engine_core::ModelSelected> = None;
    ...
    pub const TOKEN_SOURCE: engine_core::TokenSource = engine_core::TokenSource::CacheToken;
    pub const SEARCH_CALLBACK: Option<Arc<engine_core::SearchCallback>> = None;
    pub const PAGED_CACHE_TYPE: PagedCacheType = PagedCacheType::Auto;
}

/// A builder for creating a engine.rs instance with configured options for the engine.rs server.
///
/// ### Examples
///
/// Basic usage:
...
/// let args = Args::parse();
///
/// let engine = MistralRsForServerBuilder::new()
...
impl MistralRsForServerBuilder {
    ...
    /// use engine_server_core::engine_for_server_builder::MistralRsForServerBuilder;
    ///
    /// let builder = engine_server_core::engine_for_server_builder::MistralRsForServerBuilder::new();
    /// ```
    pub fn new() -> Self {
        Default::default()
    }
    ...
    /// Builds the configured engine.rs instance.
    ///
    /// ### Examples
    ///
    /// ```ignore
    /// use engine_server_core::engine_for_server_builder::MistralRsForServerBuilder;
    ///
    /// let shared_engine = MistralRsForServerBuilder::new()
    ///     .with_model(model)
    ...
    async fn build_single_model(mut self) -> Result<SharedMistralRsState> {
        ...
        let isq = self
            ...
            .transpose()?;
        info!("Parsed requested ISQ for single-model load: {isq:?}");
        ...
        resolve_pending_isq_layers_for_pipeline(&pipeline).await?;
        info!("Model loaded.");
    ...
    pub async fn build_multi_model(mut self) -> Result<SharedMistralRsState> {
        ...
        let isq = first_model
            ...
            .map(|isq| parse_isq_value(isq, Some(&device)).map_err(|e| anyhow::anyhow!("{e}")))
            .transpose()?;
        info!("Parsed requested ISQ for first multi-model load: {isq:?}");
        ...
        for model_config in self.models.iter().skip(1) {
            ...
            let isq = model_config
                ...
                .transpose()?;
            info!(
                "Parsed requested ISQ for multi-model load `{}`: {isq:?}",
                model_config.model_id
            );
            ...
            let engine_config = engine_core::EngineConfig {
            ...
            let mut add_model_config = engine_core::AddModelConfig::new(engine_config);
            if let Some(mcp_config) = self.mcp_client_config.clone() {
...
fn init_device(force_cpu: bool, seed: Option<u64>) -> Result<candle_core::Device> {
    let runtime_no_nccl = std::env::var("ENGINE_NO_NCCL").ok();
    let runtime_world_size = std::env::var("ENGINE_MN_LOCAL_WORLD_SIZE").ok();
    let requested_base_ordinal = std::env::var("ENGINE_BASE_DEVICE_ORDINAL")
        .ok()
    ...
    tracing::info!(
        ...
        force_cpu,
        engine_core::distributed::use_nccl(),
        cfg!(feature = "cuda"),
        cfg!(feature = "nccl"),
        cfg!(feature = "metal"),
        runtime_no_nccl,
    ...
    let device = if force_cpu {
        Device::Cpu
    } else if engine_core::distributed::use_nccl() {
        Device::Cpu
    } else {
        #[cfg(feature = "cuda")]
        {
    ...
    tracing::info!("Selected base runtime device: {device:?}");
...
fn engine_instance_info(loader: &dyn Loader) {
    info!(
...
fn configure_paged_attn(device: &Device, paged_attn: Option<bool>) -> bool {
    if engine_core::distributed::use_nccl() {
