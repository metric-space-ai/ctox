...
fn should_attach_generic_tool_matcher(has_tools: bool, chat_template_is_harmony: bool) -> bool {
    has_tools && !chat_template_is_harmony
}
...
impl Engine {
    ...
    pub(super) async fn add_request(&self, request: NormalRequest) {
        ...
        let chat_template_is_harmony = if is_chat {
            get_mut_arcmutex!(self.pipeline)
                .get_chat_template()
                .as_ref()
                .is_some_and(|chat_template| chat_template.is_harmony_format())
        } else {
            false
        };
        let matcher = if should_attach_generic_tool_matcher(has_tools, chat_template_is_harmony) {
            ...
            // Enable Harmony mode if the chat template uses Harmony format
            {
                ...
                if let Some(chat_template) = pipeline.get_chat_template() {
                    if chat_template.is_harmony_format() {
                        ...
                        seq.enable_channel_tag_mode(prompt_activates_thinking);
...
#[cfg(test)]
mod tests {
    use super::should_attach_generic_tool_matcher;

    #[test]
    fn attaches_generic_tool_matcher_for_non_harmony_chat_tools() {
        assert!(should_attach_generic_tool_matcher(true, false));
    }

    #[test]
    fn skips_generic_tool_matcher_for_harmony_chat_tools() {
        assert!(!should_attach_generic_tool_matcher(true, true));
    }

    #[test]
    fn skips_generic_tool_matcher_when_no_tools_are_present() {
        assert!(!should_attach_generic_tool_matcher(false, false));
    }
}
