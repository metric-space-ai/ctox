...
use super::{
    ...
    MistralLoader, MixtralLoader, NemotronHLoader, NormalLoaderType, Phi2Loader, Phi3Loader,
...
use crate::paged_attention::{
    calculate_cache_config, explicit_paged_cache_type, AttentionImplementation, CacheEngine,
};
...
use engine_quant::log::once_log_info;
use engine_quant::{
...
impl NormalLoaderBuilder {
    ...
    pub fn build(self, loader_tp: Option<NormalLoaderType>) -> anyhow::Result<Box<dyn Loader>> {
        let loader: Box<dyn NormalModelLoader> = match loader_tp {
            ...
            Some(NormalLoaderType::NemotronH) => Box::new(NemotronHLoader),
...
impl Loader for NormalLoader {
    ...
    fn load_model_from_path(
        ...
        let all_visible_devices = if use_nccl {
            device_map::get_all_similar_devices(&available_devices[0])?
        } else {
                ...
                if let Some(serialized) = &*self.from_uqff.read().unwrap() {
                    let weight_pack_factor = {
                        ...
                        for (_, artifact) in ser_artifacts.tensors() {
                            ...
                            let isq_type = artifact[engine_quant::UQFF_QUANT_TYPE_OFFSET];
        ...
        if use_immediate {
            let (pool, num_threads) = engine_quant::create_isq_thread_pool(immediate_ty);
            let allow_parallel_immediate = crate::utils::normal::should_parallelize_immediate_isq(
                &device,
                runtime_policy.parallelize_immediate_isq_on_discrete_cuda,
            );
            if num_threads <= 1 || !allow_parallel_immediate {
                info!("Applying immediate ISQ synchronously.");
                engine_quant::set_immediate_isq_serial(
                    immediate_ty,
                ...
                engine_quant::set_immediate_isq_with_pool(
                    immediate_ty,
                    immediate_predicates.clone(),
                    topology_overrides.clone(),
                    pool,
        ...
        let paged_attn_cache_type = explicit_paged_cache_type(paged_attn_config);
        ...
        if (should_quantize_pass || should_serialize) && self.config.from_uqff.is_none() {
            ...
            info!("ISQ/serialization phase complete.");
            ...
            info!("Loaded model tensors from UQFF artifacts.");
        }
        ...
        let (cache_config, cache_engine) = if let Some(paged_attn_config) = paged_attn_config {
            info!(
                "Initializing paged attention cache (cache_type={:?}, block_size={:?}, max_kv_tokens={max_kv_tokens:?}).",
            ...
            info!("Calculated paged attention cache configuration.");
            ...
            info!(
                "Constructing cache engine across {} layer placements.",
                layer_devices.len()
            );
        ...
        info!("Normal pipeline assembly complete.");
