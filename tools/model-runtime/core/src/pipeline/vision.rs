...
pub struct VisionPipeline {
    ...
    requested_isq: Option<IsqType>,
}

fn log_isq_diagnostics(
    model: &mut dyn VisionModel,
    model_id: &str,
    requested_isq: Option<IsqType>,
    stage: &str,
) {
    let (layers, _) = model.get_layers();
    let total_layers = layers.len();
    let mut pending_layers = 0usize;
    let mut quantized_layers = 0usize;
    let mut layer_kinds = BTreeMap::<&'static str, usize>::new();
    for (layer, _) in layers {
        let name = layer.name();
        *layer_kinds.entry(name).or_default() += 1;
        if name == "pending-isq" {
            pending_layers += 1;
        }
    ...
    let kind_summary = layer_kinds
        .into_iter()
        .map(|(name, count)| format!("{name}:{count}"))
        .collect::<Vec<_>>()
        .join(", ");
    info!(
        "ISQ diagnostics for `{model_id}` at stage `{stage}` with requested `{:?}`: total_layers={total_layers} quantized_layers={quantized_layers} pending_layers={pending_layers} layer_kinds=[{kind_summary}]",
        requested_isq
    );
}
...
impl Loader for VisionLoader {
    ...
    fn load_model_from_path(
        ...
        let all_visible_devices = if use_nccl || use_ring() {
        ...
        let (cache_config, cache_engine) = if let Some(paged_attn_config) = paged_attn_config {
            ...
            let model_config = model.model_config_like();
            ...
            let cache_engine =
                CacheEngine::new(&*model_config, &cache_config, dtype, &device, layer_devices)?;
