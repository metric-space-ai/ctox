...
impl FullAttention {
    ...
    fn load(
        ...
        let vb_sa = if loading_isq {
            // Attention projections must still resolve onto the real layer device
            // when immediate/staged ISQ keeps the outer builder CPU-backed.
            vb.pp("self_attn").set_device(layer_device.clone())
        } else {
            mapper.set_device(layer_idx, vb.pp("self_attn"), false)
        };
        ...
        let kv_shard = engine_quant::compute_kv_shard(num_kv_heads, head_dim, comm);
        ...
        Ok(Self {
            ...
            paged_attn,
            sdpa_params: SdpaParams {
                n_kv_groups: engine_quant::compute_n_kv_groups(num_kv_heads, num_heads, comm),
                softcap: None,
    ...
    fn forward(
        ...
        let mut q_gate = engine_quant::MatMul.qmethod_matmul(&x, &*self.q_proj)?;
        let mut k = engine_quant::MatMul.qmethod_matmul(&x, &*self.k_proj)?;
        let mut v = engine_quant::MatMul.qmethod_matmul(&x, &*self.v_proj)?;
        ...
        let mut y = match &self.paged_attn {
            Some(paged_attn) => match metadata {
                Some(((key_cache, value_cache), input_metadata)) => {
                    if !q.device().same_device(key_cache.device()) {
                        candle_core::bail!(
                            "qwen3.5_moe layer {} paged-attn key_cache device mismatch: q={:?} key_cache={:?}",
                            self.layer_idx,
                            q.device().location(),
                            key_cache.device().location()
                        );
                    }
                    if !q.device().same_device(value_cache.device()) {
                        candle_core::bail!(
                            "qwen3.5_moe layer {} paged-attn value_cache device mismatch: q={:?} value_cache={:?}",
                            self.layer_idx,
                            q.device().location(),
                            value_cache.device().location()
                        );
                    }
                    paged_attn.forward(
                        &q,
            ...
            None => {
                ...
                if !q.device().same_device(cache_k.device()) {
                    candle_core::bail!(
                        "qwen3.5_moe layer {} kv_cache key device mismatch: q={:?} cache_k={:?}",
                        self.layer_idx,
                        q.device().location(),
                        cache_k.device().location()
                    );
                }
                if !q.device().same_device(cache_v.device()) {
                    candle_core::bail!(
                        "qwen3.5_moe layer {} kv_cache value device mismatch: q={:?} cache_v={:?}",
                        self.layer_idx,
                        q.device().location(),
                        cache_v.device().location()
                    );
                }
                Sdpa.run_attention(
                    &q,
        ...
        let gate = if gate.device().same_device(y.device()) {
            gate
        } else {
            gate.to_device(y.device())?
        };
        if !y.device().same_device(gate.device()) {
            candle_core::bail!(
                "qwen3.5_moe layer {} attention output gate mul device mismatch: y={:?} gate={:?}",
                self.layer_idx,
                y.device().location(),
        ...
        let mut res = engine_quant::MatMul.qmethod_matmul(&y, &*self.o_proj)?;
        if self.q_proj.quantized_act_type().is_some() {
...
impl Mlp {
    fn new(
        ...
        quant_config: &Option<engine_quant::QuantizedConfig>,
        act_fn: crate::layers::Activation,
        comm: &Arc<engine_quant::Comm>,
    ) -> Result<Self> {
    ...
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        let gate = engine_quant::MatMul.qmethod_matmul(&xs, &*self.gate_proj)?;
        let up = engine_quant::MatMul.qmethod_matmul(&xs, &*self.up_proj)?;
        if !gate.device().same_device(up.device()) {
            candle_core::bail!(
                "qwen3.5_moe shared_expert activation mul device mismatch: gate={:?} up={:?}",
                gate.device().location(),
                up.device().location()
            );
        }
        let activated = crate::ops::mul_and_act(&gate, &up, self.act_fn)?;
        let mut res = engine_quant::MatMul.qmethod_matmul(&activated, &*self.down_proj)?;
...
impl SparseMoeBlock {
    ...
    fn new(
        ...
        layer_idx: usize,
        loading_isq: bool,
        moe_policy: crate::moe::MoEExecutionPolicy,
        comm: &Arc<engine_quant::Comm>,
        real_device: Device,
    ) -> Result<Self> {
        let layer_device = mapper
        ...
        let experts = MoEExperts::new(
            &moe_cfg,
            // Expert weights must target the repeating layer device even when
            // the outer VarBuilder stays on the non-repeating device during ISQ setup.
            vb.clone().set_device(layer_device.clone()),
            layer_device.clone(),
            comm,
            loading_isq,
            &cfg.quantization_config,
    ...
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        if !xs_flat.device().same_device(self.gate.weight().device()) {
            candle_core::bail!(
        ...
        let mut y = self
            .experts
            .forward(xs, topk_weights, &topk_ids)
            .map_err(|err| {
                err.with_path(format!("qwen3.5_moe layer {} experts", self.layer_idx))
            })?;
        y = y.reshape((b_size, seq_len, hidden_dim))?;

        let shared_out = self.shared_expert.forward(xs).map_err(|err| {
            err.with_path(format!(
                "qwen3.5_moe layer {} shared_expert",
                self.layer_idx
            ))
        })?;
        if !xs_flat
        ...
        if !shared_out.device().same_device(shared_gate.device()) {
            candle_core::bail!(
                "qwen3.5_moe layer {} shared_out gate mul device mismatch: shared_out={:?} shared_gate={:?}",
                self.layer_idx,
                shared_out.device().location(),
                shared_gate.device().location()
            );
        }
...
impl DecoderLayer {
    ...
    fn forward_linear(&self, x: &Tensor, cache: &mut GdnLayerCache) -> Result<Tensor> {
        ...
        if !x
            .device()
            .same_device(self.input_layernorm.weight().device())
        {
            candle_core::bail!(
                "qwen3.5_moe linear input_layernorm device mismatch: input={:?} weight={:?}",
                x.device().location(),
                self.input_layernorm.weight().device().location()
            );
        }
        let x = self.input_layernorm.forward(x)?;
        let gdn_out = gdn.forward(&x, cache)?;
        if !gdn_out.device().same_device(residual.device()) {
            candle_core::bail!(
                "qwen3.5_moe linear residual add device mismatch: gdn_out={:?} residual={:?}",
                gdn_out.device().location(),
                residual.device().location()
            );
        }
        ...
        if !x
            .device()
            .same_device(self.post_attention_layernorm.weight().device())
        {
            candle_core::bail!(
                "qwen3.5_moe linear post_attention_layernorm device mismatch: input={:?} weight={:?}",
                x.device().location(),
                self.post_attention_layernorm.weight().device().location()
            );
        }
        let normed = self.post_attention_layernorm.forward(&x)?;
        let ffn_out = self.moe.forward(&normed)?;
...
impl Qwen3_5MoeTextModel {
    ...
    pub fn new(
        ...
        let runtime_max_seq_len = Self::runtime_max_seq_len(cfg, &normal_loading_metadata);
        ...
        if !cfg.mlp_only_layers.is_empty() {
            candle_core::bail!(
                "Qwen3.5 MoE `mlp_only_layers` is not implemented yet in engine.rs."
            );
        }
        ...
        .par_iter_if_isq(|layer_idx| {
                ...
                LayerType::FullAttention => {
                    ...
                    let paged_attn = match &attention_mechanism {
                        ...
                        AttentionImplementation::PagedAttention => Some(PagedAttention::new(
                            ...
                            device,
                            None,
                            normal_loading_metadata.paged_attn_cache_type,
                        )?),
                    };
            ...
            let moe = SparseMoeBlock::new(
                ...
                layer_idx,
                normal_loading_metadata.loading_isq,
                normal_loading_metadata.runtime_policy.moe,
                &comm,
