...
impl FullAttention {
    ...
    fn load(
        ...
        let vb_sa = if loading_isq {
            // Immediate/staged ISQ keeps the outer builder CPU-backed. Attention
            // projections still need their real layer target so quantized weights
            // do not resolve onto CPU and fail the first live forward pass.
        ...
        Ok(Self {
            ...
            q_norm,
            k_norm,
            num_heads: engine_quant::shard_bounds(num_heads, comm.rank(), comm.world_size()).1,
            num_kv_heads: engine_quant::shard_bounds(num_kv_heads, comm.rank(), comm.world_size())
                .1
                .max(1),
            head_dim,
            rotary_emb,
    ...
    pub fn new(
        ...
        .par_iter_if_isq(|layer_idx| {
            ...
            let mlp = Mlp::new(
                if normal_loading_metadata.loading_isq {
                    ...
                    mapper.set_device(layer_idx, vb_l.pp(layer_idx).pp("mlp"), false)
                },
                cfg.hidden_size,
        ...
        Ok(Self {
            ...
            cfg: ModelConfigMetadata {
                ...
                num_attn_heads: engine_quant::shard_bounds(
                    cfg.num_attention_heads,
                    mapper.get_comm_for(0)?.rank(),
                    mapper.get_comm_for(0)?.world_size(),
                )
                .1,
                num_kv_heads: engine_quant::shard_bounds(
