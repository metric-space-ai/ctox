...
fn sanitize_generation_logits(logits: &Tensor) -> Result<Tensor> {
    ...
    logits_f32.clamp(-1e4, 1e4)?.to_dtype(original_dtype)
}
...
impl Attention {
    ...
    fn new(
        ...
        let layer_device = mapper
            ...
            .cloned()
            .or_else(|| mapper.device_for(layer_idx, true).cloned())
            .unwrap_or(Device::Cpu);
        let vb = if vb.device().is_cpu() {
            vb.set_device(layer_device.clone())
        } else {
            vb
        };
        ...
        Ok(Self {
            ...
            paged_attn,
            sdpa_params: SdpaParams {
                n_kv_groups: engine_quant::compute_n_kv_groups(
                    num_kv_heads,
...
impl DecoderLayer {
    ...
    fn new(
        ...
        let layer_device = mapper
            ...
            .unwrap_or(Device::Cpu);
        let quantized_layer_vb = |vb: ShardedVarBuilder| {
            if loading_isq {
                vb.set_device(layer_device.clone())
            } else {
        ...
        let self_attn = Attention::new(
            ...
            layer_idx,
            mapper,
            quantized_layer_vb(vb.pp("self_attn")),
            paged_attn,
            comm,
            kv_shared_layer_index,
        )?;
        ...
        let (
            ...
            let layer_device = mapper
                ...
                .cloned()
                .unwrap_or(Device::Cpu);
            let moe_vb = vb.pp(moe_prefix).set_device(layer_device.clone());
            let force_fast_immediate_isq =
                layer_device.is_cuda() && loading_isq && cfg.quantization_config.is_none();
            let moe = if force_fast_immediate_isq {
                // Gemma4 on discrete CUDA uses immediate ISQ specifically to avoid
                // raw-weight load spikes. Keep the experts path on the gather-based
                // fast backend instead of falling back to the slow packed-experts path.
        ...
        {
            let gate = engine_quant::linear_no_bias(
                cfg.hidden_size,
                ple_dim,
                &None,
                quantized_layer_vb(vb.pp("per_layer_input_gate")),
            )?;
            let proj = engine_quant::linear_no_bias(
                ple_dim,
                cfg.hidden_size,
                &None,
                quantized_layer_vb(vb.pp("per_layer_projection")),
            )?;
...
impl TextModel {
    pub fn new(
        ...
        let (embed_tokens_per_layer, per_layer_model_projection, per_layer_projection_norm) =
            if ple_dim > 0 {
                ...
                let ple_proj = engine_quant::linear_no_bias(
                    cfg.hidden_size,
    ...
    pub fn forward_embeds(
        ...
        for (i, layer) in self.layers.iter().enumerate() {
            ...
            let layer_metadata = metadata.as_ref().map(|(kv_cache, metadata)| {
                let cache_idx = if layer.self_attn.paged_attn.is_some() {
                    i
                } else {
                    layer.self_attn.kv_shared_layer_index.unwrap_or(i)
                };
                if std::env::var_os("ENGINE_GEMMA4_CACHE_DEBUG").is_some() {
                    if let Ok((kb, kh, kp, ks, kx)) = kv_cache[cache_idx].0.dims5() {
                        if let Ok((vb, vh, vp, vs, vx)) = kv_cache[cache_idx].1.dims5() {
                            tracing::info!(
                                target: "engine_core::vision_models::gemma4::cache_debug",
                                "layer={i} cache_idx={cache_idx} sliding={} kv_shared={:?} q_head_dim={} cache_k=[{kb},{kh},{kp},{ks},{kx}] cache_v=[{vb},{vh},{vp},{vs},{vx}]",
                                layer.self_attn.is_sliding,
                                layer.self_attn.kv_shared_layer_index,
