...
/// Slice/view variant of [`launch_mmvq_iq4_xs_q8_1_f32`] — same kernel,
...
pub fn launch_mmvq_iq4_xs_q8_1_f32_view(
    device: &Arc<DeviceContext>,
    a: &CudaTensor<i8>,
    k: usize,
    n: usize,
    ...
    // here. Everything else lines up with validate_mmvq_iq4_xs_shapes().
    if k == 0 || n == 0 {
        return Err(anyhow!(
            "mmvq_iq4_xs: k and n must be nonzero (k={}, n={})",
            k,
    ...
    if x_q8_1_row.len() < needed_bytes {
        return Err(anyhow!(
            "mmvq_iq4_xs: x_q8_1 view len {} < required {} for k={}",
            x_q8_1_row.len(),
            needed_bytes,
            k
        ));
    ...
    unsafe { launcher.launch(mmvq_launch_cfg(n)) }.map_err(|e| {
        anyhow!(
            "mmvq_iq4_xs_q8_1_f32_view launch (k={} n={}): {:?}",
            k,
            n,
            e
        )
