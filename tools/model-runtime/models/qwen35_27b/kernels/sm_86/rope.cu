...
__global__ void rope_multi(const T *            x,
                                  T *                  dst,
                                  const int            ne00,
                                  const int            ne01,
                                  const int            ne02,
...
// -----------------------------------------------------------------------------
// ctox additions below — NOT in upstream ggml-cuda/rope.cu.
//
// Force PTX emission of the two rope_multi specializations we load from Rust.
// Taking the address of a template __global__ instance in a __device__
// variable forces nvcc to materialize it as a .visible .entry in PTX.
//
// Specializations:
//   rope_multi<forward=true, has_ff=false, T=__nv_bfloat16>  — Qwen3.5 bf16
//     Q/K MRoPE path. No YaRN freq-factor table.
//   rope_multi<forward=true, has_ff=false, T=float>          — f32 fallback.
//
// __nv_bfloat16 has an implicit float conversion (CUDA 11+), so the
// upstream kernel's  pattern compiles unchanged.
// -----------------------------------------------------------------------------
namespace {
using ctox_rope_multi_bf16_t =
    void (*)(const __nv_bfloat16 *, __nv_bfloat16 *,
             int, int, int,
             int, int, int,
             int, int, int,
             int,
...
__device__ ctox_rope_multi_bf16_t ctox_keep_rope_multi_bf16 =
    &rope_multi<true, false, __nv_bfloat16>;
__device__ ctox_rope_multi_float_t ctox_keep_rope_multi_float =
