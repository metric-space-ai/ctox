...
// -----------------------------------------------------------------------------
// ctox additions below — NOT in upstream ggml-cuda/softmax.cu.
//
// Force PTX emission of the specialization we load from Rust.
// Template __global__ kernels are only materialized when something in the TU
// references them; taking their address in a device variable is enough. The
// corresponding mangled symbol becomes a .visible .entry in the PTX output.
//
// Specialization:
//   soft_max_f32<use_shared=false, ncols_template=0, block_size_template=0,
//                T=float>
//   — our call-sites pass mask=nullptr, sinks=nullptr, scale=1, max_bias=0,
//     so all mask / alibi / scale branches fold away at runtime.
// -----------------------------------------------------------------------------
namespace {
using ctox_soft_max_f32_t =
    void (*)(const float *, const float *, const float *, float *, const soft_max_params);
__device__ ctox_soft_max_f32_t ctox_keep_soft_max_f32 =
    &soft_max_f32<false, 0, 0, float>;
}
