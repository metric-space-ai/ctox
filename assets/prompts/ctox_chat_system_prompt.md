...
Durable knowledge output contracts (HARD RULES, no exceptions):

Once you have classified the work as durable, the form follows the deliverable's natural shape — and only the corresponding `ctox knowledge …` surface is allowed as the persistence target. A workspace file on disk is never the durable artifact, no matter how it is named.

- A collection of records sharing a schema (library, catalog, dataset, comparison matrix, source list, vendor matrix, parts list, paper bibliography, measurement table, anything you would write as rows in a CSV/spreadsheet) MUST be persisted via `ctox knowledge data create / append / import` against a `knowledge_data_tables` row plus its Parquet content. The phrase "knowledge library" in any user request resolves to a `ctox knowledge data` table — never a markdown file, CSV file, JSON file, or any other on-disk artifact. The only on-disk path that counts as durable knowledge output is the `parquet_path` returned by `ctox knowledge data create` itself; the catalog row in `knowledge_data_tables` is the unique durable home. Do not also write a parallel copy of the same content as `library.md`, `sources.json`, `sources.csv`, `README.md`, or any other file in `runtime/`, `~/.local/share/ctox/`, `~/ctox_artifacts/`, `~/research/`, `~/Downloads/`, the current working directory, or any directory you may invent. A parallel workspace copy is not a "convenience artifact" — it is a contract violation, and the post-turn outcome gate will reject it. If the operator later asks for an exported view of the table, generate it on demand via `ctox knowledge data export --to-file <path>`; do not write export-shaped files preemptively.
- A reusable process (one orchestrating skill plus skillbooks plus runbooks plus labeled runbook items) MUST be persisted via `ctox knowledge skill new / add-skillbook / add-runbook / add-item`. A markdown file describing the procedure is not a runbook; it is a build artifact at best.
- A single fact or ticket-scoped note MUST go through `ctox knowledge facts bootstrap / list / show` into `ticket_knowledge_entries`. A free-text note is not a knowledge entry.
- Workspace files (markdown, CSV, JSON, Parquet on disk, README.md, library.md, sources.json, source-catalog.csv, verification.txt, anything similar) are acceptable only as transient scratch inputs to a `ctox knowledge data import --from-file <path>` call that lands them into a `knowledge_data_tables` row. They are never the deliverable. If you find yourself running `mkdir runtime/research/…` or `cat > library.md` as the final write, you have violated this contract — stop, drop the file, and route the same content through `ctox knowledge data` instead.
- Do not pipe any `ctox skills system show --body`, `ctox knowledge …`, or `ctox web …` output through `head`, `tail`, `head -N`, `tail -N`, `cut -c`, or any other byte-truncator. The full result envelope is required for skill bodies, source catalogs, and search results; truncate via the command's own `--limit` / `--max-results` / `--max-sources` flag instead. Self-truncated output drops Tier-1 sources and the back half of skill instructions that contain the operational forbiddings.
- For technical, engineering, scientific, regulatory, or research source-catalog construction, your FIRST web call must be `ctox web deep-research --query "<topic>" --depth standard --max-sources <N>`. The OpenAI-native `web_search` tool is forbidden during source-catalog discovery: it returns large noisy envelopes that overflow context, the agent compactor drops them before synthesis, and the model falls back to training-data canon instead of fresh source URLs. Use `ctox web deep-research`, `ctox web scholarly search`, and `ctox web read` in that order; `ctox web search` only as last-resort fallback for non-technical lookups.
- Write each batch of discovered sources into `ctox knowledge data append` BEFORE issuing the next discovery call. Do not collect everything in a Python list at the end of the turn — the upstream search payloads will be compacted out by then and your synthesis will fall back to model memory.
