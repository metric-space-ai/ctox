...
| Store | Where | What |
|---|---|---|
| Browser primary | IndexedDB `ctox_business_os_js_v1` (`storage-indexeddb.mjs`) | Replicated documents per collection. |
| Browser sidecar | IndexedDB `ctox_business_os_v1_5_meta` (`query-meta-storage.mjs`) | V1.5 demand-loading metadata only; the primary store stays byte-identical to V1. |
