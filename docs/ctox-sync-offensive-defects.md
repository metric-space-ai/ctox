...
| D08 | cockpit-projections consumes 69–100% CPU; direct-session drops hundreds of events. | Reproduce representative workload; measure per-thread CPU, projection throughput and event loss before/after, including idle behavior. | Open. Full-host Linux fixtures now retain bounded native process/thread CPU samples and observer overhead alongside command timings. Actual profile results and representative tenant CPU/event-loss acceptance remain pending; this instrumentation alone does not close the incident. |
