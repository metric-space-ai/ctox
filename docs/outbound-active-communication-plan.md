...
Status: Planungsstand 2026-06-05
...
| 4. Campaign Settings, Strategie & Sequenz-Simulation | 10% | In Arbeit | 95% |
| 5. CTOX Skills, Runbooks & Draft Automation | 10% | In Arbeit | 65% |
...
| 10. End-to-End Hardening, Observability & Release Gates | 5% | In Arbeit | 99% |
| **Gesamt** | **100%** | **In Arbeit** | **86%** |
...
| 2026-06-05 | Welle 4 auf 95%, Welle 5 auf 65%, Welle 10 auf 99%: natuerliche Campaign-Briefings und Template-gestuetzter CTOX-Task-Spawn | Campaign-Erstellung/-Bearbeitung brauchte eine zentrale Freitext-Anweisung mit DE/EN-Templates, durfte aber keine lokalen Heuristiken oder HTTP-Sonderkanaele einfuehren; der erste Save-Flow hing zudem an zu striktem Sync-/Projection-Warten | Outbound bietet 20 deutsche und 20 englische natuerliche Campaign-Ideen im Editor an, speichert das Briefing editierbar auf der Campaign und reiht `outbound.campaign.briefing.update` direkt ueber RxDB `business_commands` ein; danach wird ueber das normale `ctox-business-os-chat-submit`-Prinzip ein `business_os.chat.task` mit Skill `business-os-outbound-campaign-setup` und Writeback-Vertrag `outbound.campaign.apply_setup` erzeugt; nativer RxDB-Upsert schreibt schema-tolerant `lastWriteTime`; Browser-Evidence: `output/outbound-production-readiness/2026-06-05-deploy-template-e2e.json` und `output/outbound-production-readiness/2026-06-05-deploy-save-flow-e2e.json`; Guard `assert-rxdb-only`, `outbound.test.mjs` und `cargo build --bin ctox` sind gruen; Gesamtfortschritt steigt auf 86% |
