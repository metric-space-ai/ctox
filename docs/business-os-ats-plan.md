...
- [x] **Retention erasure + Art. 15 export** *(commits 23d38c7c, a758b579)*: ATS-record PII lives in `payload_json`, which `ats.retention.purge` overwrites with a non-PII tombstone before tombstoning the row (real erasure, not soft-delete); and `ats.subject.export` gathers every PII record + audit row for a subject across the ATS collections for an Art. 15 access request (chef/admin gated, self-audited). Remaining for full closure: an Art. 17 erasure *report* artifact, and confirming desktop-file chunks/embeddings tied to a candidate are swept (file-store concern, not the ATS records).
