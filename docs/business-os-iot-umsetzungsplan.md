...
**P1 Codegen-Seam ✓ (Generierung modell-gegated):** `compile_trigger`/`generate_render` reihen einen **durable Codegen-Auftrag** ein (`mission::channels::create_queue_task`); ein modell-fähiger Agent least ihn und schreibt den Code per `ctox iot widget upsert --trigger-code/--render-code` zurück. Das Rückschreiben **validiert** (`watcher::validate_program`) → `armed`/`needs_attention` (Self-Repair). Kein synchroner Modell-Call im Command-Pfad; die eigentliche Generierung braucht das Chat-Modell (`Qwen/Qwen3.5-27B`, CUDA-only) → Lauf auf `gpu1-a6000`/API. CLI nimmt von CTOX/Hand geliefertes `trigger_code`/`render_code` direkt entgegen.

**Webhooks ✓ (Kern):** `webhook ingest` (HTTP-Payload → Signal-Datenpunkt über den vollen Inbound-Pfad inkl. Wächter-Tick) + `webhook send` (Outbound-POST, Auth-Secret aus dem Secret-Store) — getestet inkl. E2E „Webhook → Wächter feuert". Die dünne **Inbound-HTTP-Route** im Daemon (`service.rs` → `webhook::ingest`) ist ein abgegrenzter, netzwerk-/auth-sensibler Folgeschritt.
