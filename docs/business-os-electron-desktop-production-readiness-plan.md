...
| 2026-06-24 | Security-Hardening-Welle aus Code-Review (`src/apps/business-os-desktop`) in 9 atomaren Commits (`78ef9ca1`..`29a99e48`): alle 6 High- und 11 Medium-Befunde plus die guenstigen Low-Punkte behoben. Kern: HTTP-Datengrenze auf Default-Deny (xhr/fetch/ws zum Launch-Host, plus ws/wss-Filter und case-insensitive Pfade); `sandbox:true` + Deny-by-default Permission-Handler auf allen Renderern; Login-/Shell-Fenster-Navigation abgeriegelt und `shell.openExternal` nur http/https/mailto; ctox.dev `launchConfigUrl` origin-gepinnt (SSRF), webrtc-Flags erzwungen, Base-URL auf `*.ctox.dev` gepinnt; Deep-Link `pair`/`instance` nur mit Bestaetigungsdialog + Fensterfokus; Session-Partition aus Hash der exakten `(source,id)` (keine Cross-Tenant-Kollision, `raw.sessionPartition` ignoriert); Registry atomar + korruptionssicher; gemeinsamer breiterer Secret-Key-Matcher; app-eigene `known_hosts` verdrahtet; unverifizierter SSH-Dev-`curl|bash`-Installer hinter expliziter Quittung; `package-lock.json` mit Integrity neu erzeugt (0/325 -> 296/296) plus `release:check`-Gate; CSP gehaertet; Windows-Authenticode-Erkennung im Artefakt-Smoke (`--require-signature`). Gruen: `npm test` 134/134, `npm run check`, `npm run release:check`, alle 5 Electron-Smokes. Bewusst dokumentiert-offen (extern/inhärent): signierter Tag-Run + Apple/CSC-Secrets (D10-1), Live-Nicht-Owner-Revocation (D10-3), publisherName-Pin (D8-4), Host-Key-TOFU-Bestaetigungs-UI (D4-3), Remote-Content-CSP-Injection (D9-5). `release.yml` bewusst nicht angefasst (paralleler Agent); nur der `check-release-config`-Guard nachgezogen. |
