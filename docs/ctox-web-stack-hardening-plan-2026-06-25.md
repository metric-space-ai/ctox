...
### WS2-01 ◐ Rechtsgrundlage/Retention an Personen-Persistenz hängen — **HIGH (H2): Stamp umgesetzt, Erasure-Sweep offen**
...
- **Umgesetzt (2026-06-25) — Stamp:** Wichtige Erkenntnis: die Kontakt-Zeilen landen via `append_knowledge_rows` im **generischen JSON-Store `knowledge_data_rows`**, nicht in einem getypten RxDB-Schema → **kein Schema-Hash-Regen nötig**. `outbound_contact_research` stempelt jetzt **jede** persistierte Personen-Zeile mit `legal_basis="legitimate_interest"`, `basis_evidence` (Operator-LIA-Pflicht), `purpose="outbound_b2b_contact_research"`, `granted_at_ms`, `expires_at_ms` (now + 180 Tage) und `subject_key` (`outbound_contact_subject_key`: E-Mail-first, sonst Name+Firma, sha256-gehasht). Feldnamen spiegeln die CONSENT-1-Form in `ats_gates.rs`. Unit-Test `subject_key_is_stable_and_email_first`. **Verifiziert:** core `cargo check` + `check --tests` grün, `node run-all.mjs` 44 passed, `rxdb`-Crate 277 passed.
- **Offen (Folgeticket):** (1) `subject_key`/`expires_at_ms` aktiv in den ATS-`subject.export/erase`-Sweep einhängen, damit Art.17-Löschung die `knowledge_data_rows` tatsächlich trifft; (2) Retention-Purge-Job, der über `expires_at_ms` hinaus aufräumt; (3) Basis/Zweck/Retention über `runtime_config` operator-überschreibbar machen. Der Stamp deckt Accountability/Storage-Limitation-**Dokumentation** ab; die aktive Lösch-/Purge-Mechanik ist das verbleibende Stück.
