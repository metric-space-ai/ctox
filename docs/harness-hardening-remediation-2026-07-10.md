...
| Owner-gebundene / nicht auslaufende Leases | verified_fixed | Routing-State hat additive Lease-/Retry-Felder, Leases laufen nach 15 Minuten owner-unabhängig aus und aktive Worker erneuern alle 60 Sekunden. Business-Command-Übergänge `leased`/`running` bewahren die vom Router gesetzten Owner-/Zeit-/Expiry-Felder. Unvollständige ownerlose Leases werden sofort als ungültig erkannt, der verknüpfte Command geht nach `retry_wait` und dieselbe Queue-Zeile nach `pending`. Spill-Restore verwendet durable Blocked-/Pending-Gründe statt permanenter Lease. |
