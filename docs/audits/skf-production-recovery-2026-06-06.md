...
| 2026-06-09 04:09 CEST | Phase 90 deployment and runtime cleanup completed. Commit `4c521fe6` (`fix: project terminal business commands to rxdb`) was pushed to `main` and deployed through the required `ctox upgrade --dev` path as release `branch-main-20260609T020246Z`; previous release was `branch-main-20260608T235303Z`; upgrade backup is `/home/ubuntu/.local/state/ctox/backups/update-20260609T020250Z`. The first upgrade attempt failed with `No space left on device`; only automatic old `update-*` backups and build/cache artifacts were pruned, leaving active runtime DBs, active releases, artifacts, and manual backups untouched. Post-cleanup `/` has 121G free. Server verification: `current` resolves to `/home/ubuntu/.local/lib/ctox/releases/branch-main-20260609T020246Z`, `ctox.service` and `ctox-business-os-web.service` are active, `ctox status --json` reports Business OS `ok=true`, `sync.transport=webrtc`, native RxDB peer running with fresh heartbeat, `http_bridge_available=false`, and the active release source contains regression test `queue_worker_success_marks_active_rxdb_business_command_completed`. Production repair `ctox business-os repair queue-projections --apply` updated stale terminal `business_commands` projections, including `cmd_ab1c0428-e3bf-44e2-8abb-03f263d7745f`, which now reads `status=completed` and `task_status=completed` in active RxDB. Server SQLite verification shows the real `drone_bearing_design` Knowledge tables in active RxDB: `source_catalog=322`, `measured_load_points=816`, and `load_data_library=192`; no HTTP bridge is available. Production remains not ready until fresh clean-browser E2E proves the same through the UI, including Chat task completion after this deploy. |
| 2026-06-09 05:29 CEST | Phase 91 local WebRTC responder regression fix implemented. Fresh clean-browser E2E after Phase 90 first proved login, authenticated reload, tenant scope, and visible Research data through the UI (`Drohnen-Lagerbelastungsdaten`, `drone_bearing_design · 1.330 rows`, `Sources (322)`, `Measurements (816)`, `Knowledge (192)`, no fake `9/12/5` data). The same run submitted a visible CTOX Chat marker; server proof showed a real RxDB command and single CTOX queue task which completed through `https://llm.ctox.dev/v1`. A repeat fresh profile then reproduced the intermittent hard blocker before module load: `business_module_catalog` and `ctox_runtime_settings` initial replication stayed pending, browser WebRTC showed `ICE connected` but DataChannel `connecting`, repeated `handshake-timeout`, zero local shell-critical rows, and no HTTP bridge/fallback. Code reading isolated the missing native cleanup: renewed browser offers could reuse an existing Rust responder PeerConnection whose DataChannel had never opened. Local fix in `src/core/rxdb/src/plugins/replication_webrtc/connection_handler_rs.rs` tracks `data_channel_open`; a renewed inbound browser offer drops only an existing unopened responder before answering, while already-open channels are preserved. Gates passed: `cargo fmt`, `cargo fmt --check`, `git diff --check`, `cargo test --manifest-path src/core/rxdb/Cargo.toml inbound_offer_rebuilds_only_unopened_responder_peer`, and `cargo test --manifest-path src/core/rxdb/Cargo.toml connection_handler_rs` (19/19). Production remains not ready until this is pushed, deployed via `ctox upgrade --dev`, and repeated clean-browser E2E passes Research, Documents, Knowledge, Chat queue completion, result visibility, logout, tenant scope, and console/network health. |
